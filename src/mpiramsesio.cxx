...
/*!
   ...
   least reasonable domain decomposition
    ...
    NOTE: pkdgrav uses orthoganal recursive bisection along with kd-tree, gadget-2 uses peno-hilbert curve to map
            ...
            if (opt.partsearchtype == PSTGAS || opt.partsearchtype == PSTALL)
            {
                for (i = 0; i < opt.num_files; i++)
                    if (ireadfile[i])
                    {
                        ...
                        // then apparently read ngridlevels, which appears to be an array storing the number of grids at
                        // a given level
                        ...
                        for (k = 0; k < header[i].nboundary + 1; k++)
                        {
                            for (j = 0; j < header[i].nlevelmax; j++)
                            {
                                ...
                                if (chunksize > 0)
                                {
                                    ...
                                    // read son index to determine if a cell in a specific grid is at the highest
                                    // resolution and needs to be represented by a particle
                                ...
                                RAMSES_fortran_skip(Fhydro[i]);
                                // then read hydro for other variables (first is density, then velocity, then pressure,
                                // then metallicity )
                                if (chunksize > 0)
                                {
                                    ...
                                    for (idim = 0; idim < header[i].twotondim; idim++)
                                    {
                                        for (ivar = 0; ivar < header[i].nvarh; ivar++)
                                        {
                                            for (igrid = 0; igrid < chunksize; igrid++)
                                            {
                                                ...
                                                {
                                                    // if cell has no internal cells or at maximum level produce a
                                                    // particle
                                                    if (icellchunk[idim * chunksize + igrid] == 0 ||
                                                        j == header[i].nlevelmax - 1)
                                                    {
                                                        // first suggestion is to add some jitter to the particle
                                                        // positions
                                                        double dx = pow(0.5, j);
                                                        int    ix, iy, iz;
                                                        // below assumes three dimensions with 8 corners (? maybe cells)
                                                        // per grid
                                                        iz = idim / 4;
                                                        iy = (idim - (4 * iz)) / 2;
                                                        ix = idim - (2 * iy) - (4 * iz);
                                                        // Calculate absolute coordinates + jitter, and generate
                                                        // particle
                                                        xtemp[0] = ((((float)rand() / (float)RAND_MAX) *
                                                                     header[i].BoxSize * dx) +
                                                                    (header[i].BoxSize *
                                                                     (xtempchunk[igrid] + (double(ix) - 0.5) * dx)) -
                                                                    (header[i].BoxSize * dx / 2.0));
                                                        xtemp[1] =
                                                            ((((float)rand() / (float)RAND_MAX) * header[i].BoxSize *
                                                              dx) +
                                                        ...
                                                        xtemp[2] =
                                                            ((((float)rand() / (float)RAND_MAX) * header[i].BoxSize *
                                                             ...
                                                             (header[i].BoxSize * (xtempchunk[igrid + 2 * chunksize] +
                                                                                   (double(iz) - 0.5) * dx)) -
                                                             (header[i].BoxSize * dx / 2.0));
                                                        // determine processor this particle belongs on based on its
                                                        // spatial position
                                                        ibuf = MPIGetParticlesProcessor(xtemp[0], xtemp[1], xtemp[2]);
                                                        Nbuf[ibuf]++;
                                                    }
