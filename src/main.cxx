...
int main(int argc, char **argv)
{
    ...
    cout << "Built with SWIFT interface enabled when running standalone VELOCIraptor. Should only be enabled when "
            "running VELOCIraptor as a library from SWIFT. Exiting..."
         << endl;
    exit(0);
#endif
#ifdef USEMPI
    ...
    // Each thread will call MPI routines, but these calls will be coordinated to occur only one at a time within a
    // process.
    ...
    if (provided < required)
    {
        ...
        if (ThisTask == 0)
            cout << "Warning: This MPI implementation provides insufficient threading support. Required was "
                 << required << " but provided was " << provided << endl;
    ...
    if (opt.iSingleHalo)
        MinNumOld = opt.MinSize;
    else
        MinNumOld = opt.HaloMinSize;
#endif
    ...
    if (ThisTask == 0)
    {
        cout << "There are " << nbodies << " particles in total that require "
             << nbodies * sizeof(Particle) / 1024. / 1024. / 1024. << "GB of memory " << endl;
        if (opt.iBaryonSearch > 0)
            cout << "There are " << nbaryons << " baryon particles in total that require "
                 << nbaryons * sizeof(Particle) / 1024. / 1024. / 1024. << "GB of memory " << endl;
    }

    // note that for nonmpi particle array is a contiguous block of memory regardless of whether a separate baryon
    // search is required
#ifndef USEMPI
    Nlocal = nbodies;
...
#else
    // for mpi however, it is not possible to have a simple contiguous block of memory IFF a separate baryon search is
    // required. for the simple reason that the local number of particles changes to ensure large fof groups are local to
    // an mpi domain however, when reading data, it is much simplier to have a contiguous block of memory, sort that
    // memory (if necessary) and then split afterwards the dm particles and the baryons
    if (NProcs == 1)
    {
        Nlocal = Nmemlocal = nbodies;
        NExport = NImport = 1;
    }
    else
    {
        ...
        if (opt.iBaryonSearch > 0)
            cout << ThisTask << "There are " << Nlocalbaryon[0]
                 << " baryon particles and have allocated enough memory for " << Nmemlocalbaryon << " requiring "
                 << Nmemlocalbaryon * sizeof(Particle) / 1024. / 1024. / 1024. << "GB of memory " << endl;
#else
        ...
        cout << ThisTask << " Have allocated enough memory for " << Nmemlocal << " requiring "
             << Nmemlocal * sizeof(Particle) / 1024. / 1024. / 1024. << "GB of memory " << endl;
        if (opt.iBaryonSearch > 0)
            cout << " Have allocated enough memory for " << Nmemlocalbaryon << " baryons particles requiring "
                 << Nmemlocalbaryon * sizeof(Particle) / 1024. / 1024. / 1024. << "GB of memory " << endl;
#endif
    }
    cout << ThisTask
...
#ifdef USEMPI
    // if mpi and want separate baryon search then once particles are loaded into contigous block of memory and sorted
    // according to type order, allocate memory for baryons
    if (opt.iBaryonSearch > 0 && opt.partsearchtype != PSTALL)
    {
    ...
    // write out the configuration used by velociraptor having read in the data (as input data can contain cosmological
    // information)
    ...
    if (opt.smname != NULL) sprintf(fname4, "%s", opt.smname);
        ...
        //(and if STRUCDEN flag or HALOONLYDEN is set then only calculate the velocity density function for objects
        //within a structure as found by SearchFullSet)
#if defined(STRUCDEN) || defined(HALOONLYDEN)
#else
    if (opt.iSubSearch == 1)
    {
        time1 = MyGetTime();
        if (FileExists(fname4))
            ReadLocalVelocityDensity(opt, nbodies, Part);
        else
        {
        ...
        time1 = MyGetTime() - time1;
        cout << "TIME::" << ThisTask << " took " << time1 << " to analyze/read local velocity density for " << Nlocal
             << " with " << nthreads << endl;
    }
#endif
    ...
    // From here can either search entire particle array for "Halos" or if a single halo is loaded, then can just search
    // for substructure
    if (!opt.iSingleHalo)
    {
#ifndef USEMPI
...
#else
        // nbodies=Ntotal;
        ///\todo Communication Buffer size determination and allocation. For example, eventually need something like
        ///FoFDataIn = (struct fofdata_in *) CommBuffer; At the moment just using NExport
        ...
        if (opt.iInclusiveHalo > 0 && opt.iInclusiveHalo < 3)
        {
            ...
            {
                sortvalhalos[i] = pfof[i] * (pfof[i] > 0) + nbodies * (pfof[i] == 0);
                originalID[i]   = Part[i].GetID();
                Part[i].SetID(i);
            }
            Int_t *noffsethalos = BuildNoffset(nbodies, Part.data(), nhalos, numinhalos, sortvalhalos);
            /// here if inclusive halo flag is 3, then S0 masses are calculated after substructures are found for field
            /// objects and only calculate FOF masses. Otherwise calculate inclusive masses at this moment.
    ...
    else
    {
        ...
        // build grid using leaf nodes of tree (which is guaranteed to be adaptive and have maximum number of particles
        // in cell of tree bucket size)
        tree  = InitializeTreeGrid(opt, nbodies, Part.data());
        ngrid = tree->GetNumLeafNodes();
        cout << "Given " << nbodies << " particles, and max cell size of " << opt.Ncell << " there are " << ngrid
             << " leaf nodes or grid cells, with each node containing ~" << nbodies / ngrid << " particles" << endl;
        ...
        opt.HaloSigmaV = 0;
        for (int j = 0; j < ngrid; j++) opt.HaloSigmaV += pow(gveldisp[j].Det(), 1. / 3.);
        opt.HaloSigmaV /= (double)ngrid;

        // now that have the grid cell volume quantities and local volume density
        // can determine the logarithmic ratio between the particle velocity density and that predicted by the
        // background velocity distribution
        ...
        if (nsubset == 0)
        {
            ...
            cout << nsubset << " above threshold of " << opt.ellthreshold << " to be searched" << endl;
#ifndef USEMPI
    ...
    if (opt.iBaryonSearch > 0)
    {
        ...
        else if (opt.iSubSearch == 1)
        {
            ...
            Pbaryons = NULL;
            SearchBaryons(opt, nbaryons, Pbaryons, ndark, Part, pfof, ngroup, nhalos, opt.iseparatefiles,
                          opt.iInclusiveHalo, pdata);
        }
        time1 = MyGetTime() - time1;
    ...
    if (opt.iseparatefiles)
    {
        if (nhalos > 0)
        {
            pglist = SortAccordingtoBindingEnergy(opt, Nlocal, Part.data(), nhalos, pfof, numingroup,
    ...
    if (opt.iseparatefiles)
    {
        ...
        // alter index point to just output sublevels (assumes no reordering and assumes no change in nhalos as a result
        // of unbinding in SubSubSearch)
        indexii = nhalos;
        ng      = ngroup - nhalos;
    }
    ...
    if (ng > 0)
    {
        pglist = SortAccordingtoBindingEnergy(opt, nbodies, Part.data(), ng, pfof, &numingroup[indexii],
                                              &pdata[indexii], indexii); // alters pglist so most bound particles first
