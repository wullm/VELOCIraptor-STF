...
void ReadHDF(Options &opt, vector<Particle> &Part, const Int_t nbodies, Particle *&Pbaryons, Int_t nbaryons)
{
    ...
    /// Since Illustris contains an unused type of particles (2) and tracer particles (3) really not useful to iterate
    /// over all particle types in loops
    ...
    if (ThisTask == 0 && opt.iverbose > 1)
    {
        ...
        for (i = 0; i < nusetypes; i++)
            cout << "Particle " << usetypes[i] << " with name " << hdf_gnames.part_names[usetypes[i]] << endl;
        if (opt.partsearchtype == PSTDARK && opt.iBaryonSearch)
        {
            cout << "Additionally, as full separate baryon search , expecting " << nbusetypes << " baryon particles"
                 << endl;
            for (i = 1; i <= nbusetypes; i++)
                cout << "Particle " << usetypes[i] << " with name " << hdf_gnames.part_names[usetypes[i]] << endl;
        }
    }

    // if MPI is used, read processors (all tasks with task numbers less than the number of snapshots) opens the file
    // and loads the data into a particle buffer this particle buffer is used to broadcast data to the appropriate
    // processor
#ifdef USEMPI
    ...
    if (ThisTask == 0)
        cout << "There are " << opt.nsnapread << " threads reading " << opt.num_files << " files " << endl;
    ...
    if (ireadtask[ThisTask] >= 0)
    {
        ...
        for (i = 0; i < opt.num_files; i++)
            ...
            {
                if (opt.num_files > 1)
                    sprintf(buf, "%s.%d.hdf5", opt.fname, (int)i);
                else
                    sprintf(buf, "%s.hdf5", opt.fname);
                ...
                // try
                {
                    ...
                    hdf_header_info[i].BoxSize =
                        read_attribute<double>(Fhdf[i], hdf_header_info[i].names[hdf_header_info[i].IBoxSize]);
                    ...
                    {
                        vlongbuff = read_attribute_v<long long>(
                            Fhdf[i], hdf_header_info[i].names[hdf_header_info[i].INuminFile]);
                        for (k = 0; k < NHDFTYPE; k++) hdf_header_info[i].npart[k] = vlongbuff[k];
                    }
                    else
                    {
                        vuintbuff = read_attribute_v<unsigned int>(
                            Fhdf[i], hdf_header_info[i].names[hdf_header_info[i].INuminFile]);
                        for (k = 0; k < NHDFTYPE; k++) hdf_header_info[i].npart[k] = vuintbuff[k];
                    }
                    vuintbuff =
                        read_attribute_v<unsigned int>(Fhdf[i], hdf_header_info[i].names[hdf_header_info[i].INumTot]);
                    for (k = 0; k < NHDFTYPE; k++) hdf_header_info[i].npartTotal[k] = vuintbuff[k];
                    vuintbuff =
                        read_attribute_v<unsigned int>(Fhdf[i], hdf_header_info[i].names[hdf_header_info[i].INumTotHW]);
                    for (k = 0; k < NHDFTYPE; k++) hdf_header_info[i].npartTotalHW[k] = vuintbuff[k];
                    hdf_header_info[i].Omega0 =
                        read_attribute<double>(Fhdf[i], hdf_header_info[i].names[hdf_header_info[i].IOmega0]);
                    hdf_header_info[i].OmegaLambda =
                        read_attribute<double>(Fhdf[i], hdf_header_info[i].names[hdf_header_info[i].IOmegaL]);
                    hdf_header_info[i].redshift =
                        read_attribute<double>(Fhdf[i], hdf_header_info[i].names[hdf_header_info[i].IRedshift]);
                    hdf_header_info[i].time =
                        read_attribute<double>(Fhdf[i], hdf_header_info[i].names[hdf_header_info[i].ITime]);
                    hdf_header_info[i].HubbleParam =
                        read_attribute<double>(Fhdf[i], hdf_header_info[i].names[hdf_header_info[i].IHubbleParam]);
                    hdf_header_info[i].num_files =
                        read_attribute<int>(Fhdf[i], hdf_header_info[i].names[hdf_header_info[i].INumFiles]);
                }
                /*catch(GroupIException &error)
                {
                  HDF5PrintError(error);
                  cerr<<"Error in group might suggest config file has the incorrect HDF naming convention. ";
                  cerr<<"Check HDF_name_convetion or add new naming convention updating hdfitems.h in the source code.
        "<<endl; Fhdf[i].close(); #ifdef USEMPI MPI_Abort(MPI_COMM_WORLD,8); #else exit(8); #endif
                }
                ...
                {
                  HDF5PrintError(error);
                  cerr<<"Error in data type might suggest need to update hdfio.cxx in the source code to read correct
        format"<<endl; Fhdf[i].close(); #ifdef USEMPI MPI_Abort(MPI_COMM_WORLD,8); #else exit(8); #endif
                }
                */
            }
        ...
        if (opt.icosmologicalin)
        {
            ...
            // if opt.virlevel<0, then use virial overdensity based on Bryan and Norman 1997 virialization level is
            // given by
        ...
        if (opt.ihdfnameconvention == HDFSWIFTEAGLENAMES)
        {
            mscale  = opt.massinputconversion;
            lscale  = opt.lengthinputconversion * aadjust;
            lvscale = opt.lengthinputconversion * opt.a;
        }
        else
        {
            mscale  = opt.massinputconversion / opt.h;
            lscale  = opt.lengthinputconversion / opt.h * aadjust;
            lvscale = opt.lengthinputconversion / opt.h * opt.a;
        }
        ...
        if (ThisTask == 0)
        {
            ...
            cout << "Particle system contains " << nbodies << " particles and is at time " << opt.a
                 << " in a box of size " << opt.p << endl;
        }
    ...
    for (i = 0; i < opt.num_files; i++)
    {
        ...
        // try
        {
            ...
            for (j = 0; j < nusetypes; j++)
            {
                k = usetypes[j];
                if (ThisTask == 0 && opt.iverbose > 1)
                    cout << "Opening group " << hdf_gnames.part_names[k] << ": Data set " << hdf_parts[k]->names[itemp]
                         << endl;
            ...
            if (opt.partsearchtype == PSTDARK && opt.iBaryonSearch)
                ...
                {
                    k = usetypes[j];
                    partsdataset[i * NHDFTYPE + k] =
                        HDF5OpenDataSet(partsgroup[i * NHDFTYPE + k], hdf_parts[k]->names[0]);
                    partsdataspace[i * NHDFTYPE + k] = HDF5OpenDataSpace(partsdataset[i * NHDFTYPE + k]);
                }
            count  = count2;
            bcount = bcount2;
            for (j = 0; j < nusetypes; j++)
            {
                ...
                if (hdf_header_info[i].npart[k] < chunksize)
                    nchunk = hdf_header_info[i].npart[k];
                else
                    nchunk = chunksize;
                for (n = 0; n < hdf_header_info[i].npart[k]; n += nchunk)
                {
                    ...
                    HDF5ReadHyperSlabReal(doublebuff, partsdataset[i * NHDFTYPE + k], partsdataspace[i * NHDFTYPE + k],
                                          1, 3, nchunk, n);
                    for (int nn = 0; nn < nchunk; nn++)
                        Part[count++].SetPosition(doublebuff[nn * 3], doublebuff[nn * 3 + 1], doublebuff[nn * 3 + 2]);
                }
            }
            if (opt.partsearchtype == PSTDARK && opt.iBaryonSearch)
            {
                for (j = 1; j <= nbusetypes; j++)
                {
                    ...
                    for (n = 0; n < hdf_header_info[i].npart[k]; n += nchunk)
                    {
                        if (hdf_header_info[i].npart[k] - n < chunksize && hdf_header_info[i].npart[k] - n > 0)
                            nchunk = hdf_header_info[i].npart[k] - n;
                        // //setup hyperslab so that it is loaded into the buffer
                        HDF5ReadHyperSlabReal(doublebuff, partsdataset[i * NHDFTYPE + k],
                                              partsdataspace[i * NHDFTYPE + k], 1, 3, nchunk, n);
                        for (int nn = 0; nn < nchunk; nn++)
                            Pbaryons[bcount++].SetPosition(doublebuff[nn * 3], doublebuff[nn * 3 + 1],
                                                           doublebuff[nn * 3 + 2]);
                    }
                }
            }
            ...
            for (j = 0; j < nusetypes; j++)
            {
                ...
                if (ThisTask == 0 && opt.iverbose > 1)
                    cout << "Opening group " << hdf_gnames.part_names[k] << ": Data set " << hdf_parts[k]->names[itemp]
                         << endl;
                partsdataset[i * NHDFTYPE + k] =
                    HDF5OpenDataSet(partsgroup[i * NHDFTYPE + k], hdf_parts[k]->names[itemp]);
                partsdataspace[i * NHDFTYPE + k] = HDF5OpenDataSpace(partsdataset[i * NHDFTYPE + k]);
            }
            if (opt.partsearchtype == PSTDARK && opt.iBaryonSearch)
                for (j = 1; j <= nbusetypes; j++)
            ...
            for (j = 0; j < nusetypes; j++)
            {
                ...
                for (n = 0; n < hdf_header_info[i].npart[k]; n += nchunk)
                {
                    ...
                    HDF5ReadHyperSlabReal(doublebuff, partsdataset[i * NHDFTYPE + k], partsdataspace[i * NHDFTYPE + k],
                                          1, 3, nchunk, n);
                    for (int nn = 0; nn < nchunk; nn++)
                        Part[count++].SetVelocity(doublebuff[nn * 3], doublebuff[nn * 3 + 1], doublebuff[nn * 3 + 2]);
                }
            }
            if (opt.partsearchtype == PSTDARK && opt.iBaryonSearch)
            {
                for (j = 1; j <= nbusetypes; j++)
                {
                    ...
                    for (n = 0; n < hdf_header_info[i].npart[k]; n += nchunk)
                    {
                        ...
                        HDF5ReadHyperSlabReal(doublebuff, partsdataset[i * NHDFTYPE + k],
                                              partsdataspace[i * NHDFTYPE + k], 1, 3, nchunk, n);
                        for (int nn = 0; nn < nchunk; nn++)
                            Pbaryons[bcount++].SetVelocity(doublebuff[nn * 3], doublebuff[nn * 3 + 1],
                                                           doublebuff[nn * 3 + 2]);
                    }
                }
            }
            ...
            for (j = 0; j < nusetypes; j++)
            {
                ...
                for (n = 0; n < hdf_header_info[i].npart[k]; n += nchunk)
                {
                    if (hdf_header_info[i].npart[k] - n < chunksize && hdf_header_info[i].npart[k] - n > 0)
                        nchunk = hdf_header_info[i].npart[k] - n;
                    // setup hyperslab so that it is loaded into the buffer
                    HDF5ReadHyperSlabInteger(longbuff, partsdataset[i * NHDFTYPE + k], partsdataspace[i * NHDFTYPE + k],
                                             1, 1, nchunk, n);
                    ...
                    for (int nn = 0; nn < nchunk; nn++)
                    {
                        ...
                        if (k == HDFGASTYPE)
                            Part[count].SetType(GASTYPE);
                        else if (k == HDFDMTYPE)
                            Part[count].SetType(DARKTYPE);
                        else if (k == HDFSTARTYPE)
                            Part[count].SetType(STARTYPE);
                        else if (k == HDFBHTYPE)
                            Part[count].SetType(BHTYPE);
            ...
            if (opt.partsearchtype == PSTDARK && opt.iBaryonSearch)
            {
                for (j = 1; j <= nbusetypes; j++)
                {
                    ...
                    for (n = 0; n < hdf_header_info[i].npart[k]; n += nchunk)
                    {
                        if (hdf_header_info[i].npart[k] - n < chunksize && hdf_header_info[i].npart[k] - n > 0)
                            nchunk = hdf_header_info[i].npart[k] - n;
                        HDF5ReadHyperSlabInteger(longbuff, partsdataset[i * NHDFTYPE + k],
                                                 partsdataspace[i * NHDFTYPE + k], 1, 1, nchunk, n);
                        ...
                        for (int nn = 0; nn < nchunk; nn++)
                        {
                            ...
                            if (k == HDFGASTYPE)
                                Pbaryons[bcount].SetType(GASTYPE);
                            else if (k == HDFSTARTYPE)
                                Pbaryons[bcount].SetType(STARTYPE);
                            else if (k == HDFBHTYPE)
                                Pbaryons[bcount].SetType(BHTYPE);
#ifdef EXTRAINPUTINFO
                            if (opt.iextendedoutput)
                            {
            ...
            for (j = 0; j < nusetypes; j++)
            {
                ...
                {
                    for (int nn = 0; nn < hdf_header_info[i].npart[k]; nn++)
                        Part[count++].SetMass(hdf_header_info[i].mass[k]);
                }
            }
            if (opt.partsearchtype == PSTDARK && opt.iBaryonSearch)
            {
                for (j = 1; j <= nbusetypes; j++)
                {
                    ...
                    {
                        for (int nn = 0; nn < hdf_header_info[i].npart[k]; nn++)
                            Pbaryons[bcount++].SetMass(hdf_header_info[i].mass[k]);
                    }
                }
            }
            // close data spaces
            ...
            if (!(opt.partsearchtype == PSTDARK && opt.iBaryonSearch == 0))
            {
                ...
                for (j = 0; j < nusetypes; j++)
                {
                    ...
                    {
                        if (ThisTask == 0 && opt.iverbose > 1)
                            cout << "Opening group " << hdf_gnames.part_names[k] << ": Data set "
                                 << hdf_parts[k]->names[5] << endl;
                        partsdataset[i * NHDFTYPE + k] =
                            HDF5OpenDataSet(partsgroup[i * NHDFTYPE + k], hdf_parts[k]->names[5]);
                        partsdataspace[i * NHDFTYPE + k] = HDF5OpenDataSpace(partsdataset[i * NHDFTYPE + k]);
                    }
                }
                if (opt.partsearchtype == PSTDARK && opt.iBaryonSearch)
                ...
                for (j = 0; j < nusetypes; j++)
                {
                    ...
                    if (k == HDFGASTYPE)
                    {
                        ...
                        for (n = 0; n < hdf_header_info[i].npart[k]; n += nchunk)
                        {
                            ...
                            // partsdataspace[i*NHDFTYPE+k].selectHyperslab(H5S_SELECT_SET, filespacecount,
                            // filespaceoffset);
                ...
                for (j = 0; j < nusetypes; j++)
                {
                    ...
                    if (k == HDFGASTYPE)
                    {
                        if (ThisTask == 0 && opt.iverbose > 1)
                            cout << "Opening group " << hdf_gnames.part_names[k] << ": Data set "
                                 << hdf_parts[k]->names[6] << endl;
                        partsdataset[i * NHDFTYPE + k] =
                            HDF5OpenDataSet(partsgroup[i * NHDFTYPE + k], hdf_parts[k]->names[6]);
                        partsdataspace[i * NHDFTYPE + k] = HDF5OpenDataSpace(partsdataset[i * NHDFTYPE + k]);
                    }
                }
                if (opt.partsearchtype == PSTDARK && opt.iBaryonSearch)
                ...
                for (j = 0; j < nusetypes; j++)
                {
                    ...
                    if (k == HDFGASTYPE)
                    {
                        if (ThisTask == 0 && opt.iverbose > 1)
                            cout << "Opening group " << hdf_gnames.part_names[k] << ": Data set "
                                 << hdf_parts[k]->names[hdf_parts[k]->propindex[HDFGASIMETAL]] << endl;
                        partsdataset[i * NHDFTYPE + k] = HDF5OpenDataSet(
                            partsgroup[i * NHDFTYPE + k], hdf_parts[k]->names[hdf_parts[k]->propindex[HDFGASIMETAL]]);
                        partsdataspace[i * NHDFTYPE + k] = HDF5OpenDataSpace(partsdataset[i * NHDFTYPE + k]);
                    }
                    if (k == HDFSTARTYPE)
                    {
                        if (ThisTask == 0 && opt.iverbose > 1)
                            cout << "Opening group " << hdf_gnames.part_names[k] << ": Data set "
                                 << hdf_parts[k]->names[hdf_parts[k]->propindex[HDFSTARIMETAL]] << endl;
                        partsdataset[i * NHDFTYPE + k] = HDF5OpenDataSet(
                            partsgroup[i * NHDFTYPE + k], hdf_parts[k]->names[hdf_parts[k]->propindex[HDFSTARIMETAL]]);
                        partsdataspace[i * NHDFTYPE + k] = HDF5OpenDataSpace(partsdataset[i * NHDFTYPE + k]);
                    }
                }
                if (opt.partsearchtype == PSTDARK && opt.iBaryonSearch)
                    for (j = 1; j <= nbusetypes; j++)
                    {
                        ...
                        {
                            partsdataset[i * NHDFTYPE + k] =
                                HDF5OpenDataSet(partsgroup[i * NHDFTYPE + k],
                                                hdf_parts[k]->names[hdf_parts[k]->propindex[HDFGASIMETAL]]);
                            partsdataspace[i * NHDFTYPE + k] = HDF5OpenDataSpace(partsdataset[i * NHDFTYPE + k]);
                        }
                        if (k == HDFSTARTYPE)
                        {
                            partsdataset[i * NHDFTYPE + k] =
                                HDF5OpenDataSet(partsgroup[i * NHDFTYPE + k],
                                                hdf_parts[k]->names[hdf_parts[k]->propindex[HDFSTARIMETAL]]);
                            partsdataspace[i * NHDFTYPE + k] = HDF5OpenDataSpace(partsdataset[i * NHDFTYPE + k]);
                        }
                    }
                count  = count2;
                ...
                for (auto &hidval : partsdataspace) HDF5CloseDataSpace(hidval);
                for (auto &hidval : partsdataset) HDF5CloseDataSet(hidval);
                // then get star formation time, must also adjust so that if tage<0 this is a wind particle in Illustris
                // so change particle type
                for (j = 0; j < nusetypes; j++)
                {
                    k = usetypes[j];
                    if (k == HDFSTARTYPE)
                    {
                        if (ThisTask == 0 && opt.iverbose > 1)
                            cout << "Opening group " << hdf_gnames.part_names[k] << ": Data set "
                                 << hdf_parts[k]->names[hdf_parts[k]->propindex[HDFSTARIAGE]] << endl;
                        partsdataset[i * NHDFTYPE + k] = HDF5OpenDataSet(
                            partsgroup[i * NHDFTYPE + k], hdf_parts[k]->names[hdf_parts[k]->propindex[HDFSTARIAGE]]);
                        partsdataspace[i * NHDFTYPE + k] = HDF5OpenDataSpace(partsdataset[i * NHDFTYPE + k]);
                    }
                }
                if (opt.partsearchtype == PSTDARK && opt.iBaryonSearch)
                    for (j = 1; j <= nbusetypes; j++)
                    {
                        ...
                        if (k == HDFSTARTYPE)
                        {
                            partsdataset[i * NHDFTYPE + k] =
                                HDF5OpenDataSet(partsgroup[i * NHDFTYPE + k],
                                                hdf_parts[k]->names[hdf_parts[k]->propindex[HDFSTARIAGE]]);
                            partsdataspace[i * NHDFTYPE + k] = HDF5OpenDataSpace(partsdataset[i * NHDFTYPE + k]);
                        }
                    }
                count  = count2;
                ...
                for (j = 0; j < nusetypes; j++)
                {
                    ...
                    if (k == HDFSTARTYPE)
                    {
                        ...
                        for (n = 0; n < hdf_header_info[i].npart[k]; n += nchunk)
                        {
                            ...
                            for (int nn = 0; nn < nchunk; nn++)
                            {
                                if (doublebuff[nn] < 0) Part[count].SetType(WINDTYPE);
                                Part[count++].SetTage(doublebuff[nn]);
                            }
                        }
                    }
                    else
                    ...
                    for (j = 1; j <= nbusetypes; j++)
                    {
                        ...
                        if (k == HDFGASTYPE || k == HDFSTARTYPE)
                        {
                            ...
                            for (n = 0; n < hdf_header_info[i].npart[k]; n += nchunk)
                            {
                                ...
                                for (int nn = 0; nn < nchunk; nn++)
                                {
                                    if (doublebuff[nn] < 0) Pbaryons[bcount].SetType(WINDTYPE);
                                    Pbaryons[bcount++].SetTage(doublebuff[nn]);
                                }
                            }
                        }
                        else
        ...
        catch( DataTypeIException error )
        {
            HDF5PrintError(error);
            cerr<<"Error in data type might suggest need to update hdfio.cxx in the source code to read correct
        format"<<endl; Fhdf[i].close(); exit(8);
        }
        */
        // Fhdf[i].close();
    ...
    for (i = 0; i < nbodies; i++)
    {
        ...
        for (int j = 0; j < 3; j++)
            Part[i].SetVelocity(j, Part[i].GetVelocity(j) * vscale + Hubbleflow * Part[i].GetPosition(j));
        for (int j = 0; j < 3; j++) Part[i].SetPosition(j, Part[i].GetPosition(j) * lscale);
#ifdef GASON
        if (Part[i].GetType() == GASTYPE)
            Part[i].SetU(Part[i].GetU() * opt.velocityinputconversion * opt.velocityinputconversion);
#endif
    }
    if (Pbaryons != NULL && opt.iBaryonSearch == 1)
    {
        for (i = 0; i < nbaryons; i++)
        {
            ...
            for (int j = 0; j < 3; j++)
                Pbaryons[i].SetVelocity(j,
                                        Pbaryons[i].GetVelocity(j) * vscale + Hubbleflow * Pbaryons[i].GetPosition(j));
...
#else
    // for all mpi threads that are reading input data, open file load access to data structures and begin loading into
    // either local buffer or temporary buffer to be send to non-read threads
    if (ireadtask[ThisTask] >= 0)
    {
        inreadsend = 0;
        count2 = bcount2 = 0;
            ...
            if (ireadfile[i])
            {
                ...
                // try
                {
                    ...
                    for (itemp = 0; itemp < NHDFDATABLOCKALL; itemp++)
                    {
                        ...
                        if (itemp != 3)
                        {
                            for (j = 0; j < nusetypes; j++)
                            {
                                ...
                                if (ThisTask == 0 && opt.iverbose > 1)
                                    cout << "Opening group " << hdf_gnames.part_names[k] << ": Data set "
                                         << hdf_parts[k]->names[itemp] << endl;
                                partsdatasetall[i * NHDFTYPE * NHDFDATABLOCK + k * NHDFDATABLOCK + itemp] =
                                    HDF5OpenDataSet(partsgroup[i * NHDFTYPE + k], hdf_parts[k]->names[itemp]);
                                partsdataspaceall[i * NHDFTYPE * NHDFDATABLOCK + k * NHDFDATABLOCK + itemp] =
                                    HDF5OpenDataSpace(
                                        partsdatasetall[i * NHDFTYPE * NHDFDATABLOCK + k * NHDFDATABLOCK + itemp]);
                            }
                            if (opt.partsearchtype == PSTDARK && opt.iBaryonSearch)
                    ...
                    if (!(opt.partsearchtype == PSTDARK && opt.iBaryonSearch == 0))
                    {
                        ...
                        // then get star formation time, must also adjust so that if tage<0 this is a wind particle in
                        // Illustris so change particle type
                        itemp++;
                        for (j = 0; j < nusetypes; j++)
                        {
                            k = usetypes[j];
                    ...
                    for (j = 0; j < nusetypes; j++)
                    {
                        ...
                        for (n = 0; n < hdf_header_info[i].npart[k]; n += nchunk)
                        {
                            ...
                            itemp = 0;
                            // set hyperslab
                            HDF5ReadHyperSlabReal(
                                doublebuff, partsdatasetall[i * NHDFTYPE * NHDFDATABLOCK + k * NHDFDATABLOCK + itemp],
                                partsdataspaceall[i * NHDFTYPE * NHDFDATABLOCK + k * NHDFDATABLOCK + itemp], 1, 3,
                                nchunk, n);
                            // velocities
                            itemp++;
                            HDF5ReadHyperSlabReal(
                            ...
                            // ids
                            itemp++;
                            HDF5ReadHyperSlabInteger(
                                longbuff, partsdatasetall[i * NHDFTYPE * NHDFDATABLOCK + k * NHDFDATABLOCK + itemp],
                                partsdataspaceall[i * NHDFTYPE * NHDFDATABLOCK + k * NHDFDATABLOCK + itemp], 1, 1,
                                nchunk, n);
                            ...
                            for (int nn = 0; nn < nchunk; nn++)
                            {
                                ibuf      = MPIGetParticlesProcessor(doublebuff[nn * 3], doublebuff[nn * 3 + 1],
                                                                doublebuff[nn * 3 + 2]);
...
#ifdef BHON
#endif
                                Pbuf[ibufindex].SetPosition(doublebuff[nn * 3], doublebuff[nn * 3 + 1],
                                                            doublebuff[nn * 3 + 2]);
                                Pbuf[ibufindex].SetVelocity(veldoublebuff[nn * 3], veldoublebuff[nn * 3 + 1],
                                                            veldoublebuff[nn * 3 + 2]);
                                if (hdf_header_info[i].mass[k] == 0)
                                    Pbuf[ibufindex].SetMass(massdoublebuff[nn]);
                                else
                                    Pbuf[ibufindex].SetMass(hdf_header_info[i].mass[k]);
                                Pbuf[ibufindex].SetPID(longbuff[nn]);
                                Pbuf[ibufindex].SetID(nn);
                                if (k == HDFGASTYPE)
                                    Pbuf[ibufindex].SetType(GASTYPE);
                                else if (k == HDFDMTYPE)
                                    Pbuf[ibufindex].SetType(DARKTYPE);
#ifdef HIGHRES
...
#ifdef HIGHRES
                                if (k == HDFDMTYPE && MP_DM > Pbuf[ibufindex].GetMass())
                                    MP_DM = Pbuf[ibufindex].GetMass();
#endif
#ifdef GASON
                                if (k == HDFGASTYPE)
                                {
...
#endif
                                Nbuf[ibuf]++;
                                MPIAddParticletoAppropriateBuffer(ibuf, ibufindex, ireadtask, BufSize, Nbuf, Pbuf,
                                                                  Nlocal, Part.data(), Nreadbuf, Preadbuf);
                            }
                            ninputoffset += nchunk;
                        }
                    }
                            ...
                            for (n = 0; n < hdf_header_info[i].npart[k]; n += nchunk)
                            {
                                ...
                                if (hdf_header_info[i].mass[k] == 0)
                                {
                                    HDF5ReadHyperSlabReal(
                                        massdoublebuff,
                                        partsdatasetall[i * NHDFTYPE * NHDFDATABLOCK + k * NHDFDATABLOCK + itemp],
                                        partsdataspaceall[i * NHDFTYPE * NHDFDATABLOCK + k * NHDFDATABLOCK + itemp], 1,
                                        1, nchunk, n);
                                }
#ifdef GASON
                                // self-energy
                                ...
                                {
                                    if (ifloat_pos)
                                        ibuf = MPIGetParticlesProcessor(floatbuff[nn * 3], floatbuff[nn * 3 + 1],
                                                                        floatbuff[nn * 3 + 2]);
                                    else
                                        ibuf = MPIGetParticlesProcessor(doublebuff[nn * 3], doublebuff[nn * 3 + 1],
                                                                        doublebuff[nn * 3 + 2]);
                                    ...
                                    Nbuf[ibuf]++;
                                    MPIAddParticletoAppropriateBuffer(ibuf, ibufindex, ireadtask, BufSize, Nbuf, Pbuf,
                                                                      Nlocalbaryon[0], Pbaryons, Nreadbuf, Preadbuf);
                                }
                                ninputoffset += nchunk;
                            } // end of chunk
                        }     // end of part type
                ...
                catch(GroupIException error)
                {
                    ...
                    cerr<<"Error in group might suggest config file has the incorrect HDF naming convention. ";
                    cerr<<"Check HDF_name_convetion or add new naming convention updating hdfitems.h in the source code.
                "<<endl; Fhdf[i].close(); MPI_Abort(MPI_COMM_WORLD,8);
                }
                // catch failure caused by the H5File operations
                catch( FileIException error )
                {
                ...
                catch( DataSetIException error )
                {
                    HDF5PrintError(error);
                    cerr<<"Error in data set might suggest config file has the incorrect HDF naming convention. ";
                    cerr<<"Check HDF_name_convetion or update hdfio.cxx in the source code to read correct
                format"<<endl; Fhdf[i].close(); MPI_Abort(MPI_COMM_WORLD,8);
                }
                // catch failure caused by the DataSpace operations
                catch( DataSpaceIException error )
                {
                ...
                */
                ...
                {
                    MPI_Allgather(Nreadbuf, opt.nsnapread, MPI_Int_t, mpi_nsend_readthread, opt.nsnapread, MPI_Int_t,
                                  mpi_comm_read);
                    MPISendParticlesBetweenReadThreads(opt, Preadbuf, Part.data(), ireadtask, readtaskID, Pbaryons,
                                                       mpi_comm_read, mpi_nsend_readthread,
                                                       mpi_nsend_readthread_baryon);
                    inreadsend++;
        ...
        for (ibuf = 0; ibuf < NProcs; ibuf++)
            if (ireadtask[ibuf] < 0)
            {
                MPI_Ssend(&Nbuf[ibuf], 1, MPI_Int_t, ibuf, ibuf + NProcs, MPI_COMM_WORLD);
                if (Nbuf[ibuf] > 0)
                {
                    MPI_Ssend(&Pbuf[ibuf * BufSize], sizeof(Particle) * Nbuf[ibuf], MPI_BYTE, ibuf, ibuf,
                              MPI_COMM_WORLD);
                    Nbuf[ibuf] = 0;
        ...
        if (opt.nsnapread > 1)
        {
            MPI_Allgather(Nreadbuf, opt.nsnapread, MPI_Int_t, mpi_nsend_readthread, opt.nsnapread, MPI_Int_t,
                          mpi_comm_read);
            MPISendParticlesBetweenReadThreads(opt, Preadbuf, Part.data(), ireadtask, readtaskID, Pbaryons,
                                               mpi_comm_read, mpi_nsend_readthread, mpi_nsend_readthread_baryon);
            inreadsend++;
            for (ibuf = 0; ibuf < opt.nsnapread; ibuf++) Nreadbuf[ibuf] = 0;
        }
    }
    ...
    {
        MPIReceiveParticlesFromReadThreads(opt, Pbuf, Part.data(), readtaskID, irecv, mpi_irecvflag, Nlocalthreadbuf,
                                           mpi_request, Pbaryons);
    }
#endif
    ...
    if (ThisTask == 0)
    {
        ...
        if (opt.ihdfnameconvention == HDFSWIFTEAGLENAMES)
        {
            // adjust period
            if (opt.comove)
                opt.p *= opt.lengthinputconversion;
            else
                opt.p *= opt.lengthinputconversion * opt.a;
        }
        else
        {
            // adjust period
            if (opt.comove)
                opt.p *= opt.lengthinputconversion / opt.h;
            else
                opt.p *= opt.lengthinputconversion / opt.h * opt.a;
        }
...
#endif
#ifdef USEMPI
    if (opt.nsnapread > 1) { MPI_Allreduce(&MP_DM, &MP_DM, 1, MPI_DOUBLE, MPI_MIN, mpi_comm_read); }
...
#endif
    /// if not an individual halo and cosmological and store scale of the highest resolution interparticle spacing to
    /// scale the physical FOF linking length
