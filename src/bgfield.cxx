...
/*!
    ...
    Building a physical tree with shannon entropy ensures that regions have uniform M(R)/r or more specifically uniform
   inter particle spacing \todo need to alter pglist array. Much smoother if use particles themselves to find nearest
   cells Instead of storing pglist which is memory intensive (nbodies*ncells) just calculate it when needed.
*/
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, vel)
    {
#pragma omp for
#endif
        for (i = 0; i < nbodies; i++)
    ...
    {
        if (opt.iverbose >= 2)
            cout << "Building Physical Tree using simple spatial extend as splitting criterion" << endl;
        tree = new KDTree(Part, nbodies, opt.Ncell, tree->TPHYS);
    }
    else if (opt.gridtype == PHYSENGRID)
    {
        if (opt.iverbose >= 2)
            cout << "Building physical Tree using minimum shannon entropy as splitting criterion" << endl;
    ...
    {
        if (opt.iverbose >= 2)
            cout << "Building Phase-space Tree using minimum shannon entropy as splitting criterion" << endl;
        // tree=new KDTree(*S,opt.Ncell,tree->TPHS,tree->KEPAN,100,1,1);//if phase tree, use entropy criterion with
        // anisotropic kernel if phase tree, use entropy criterion with anisotropic kernel
...
// PartCellNum *pglist)
{
    ...
    int       ND;
    if (treetype == tree->TPHYS)
        ND = 3;
    else if (treetype == tree->TPHS)
        ND = 6;

    if (opt.iverbose >= 2) cout << "Filling KD-Tree Grid" << endl;

    // this loop works well for large cells but for small cells may have to replace this with a for loop which goes
    // through every particle and stores nid and size of node, then builds grid based on these values and then places
    // the particles in the cells. for leaf nodes, use starts and ends indices (ie what particles in the system are in
    // the node)
    while (ncount < nbodies)
    {
        ...
        for (Int_t k = start, l = 0; k < end; k++, l++)
        {
            ...
            for (int j = 0; j < ND; j++) grid[gridcount].xm[j] += Part[k].GetPosition(j) * Part[k].GetMass();
            mtot += Part[k].GetMass();
        }
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, mtot)
    {
#pragma omp for
#endif
        for (i = 0; i < ngrid; i++)
        {
            ...
            {
                for (int k = 0; k < 3; k++)
                    gvel[i][k] += Part[grid[i].nindex[j]].GetVelocity(k) * Part[grid[i].nindex[j]].GetMass();
            }
...
#pragma omp for
        ...
        {
            for (int k = 0; k < 3; k++)
                for (int l = 0; l < 3; l++) gveldisp[i](k, l) = 0.;
            for (int j = 0; j < grid[i].nparts; j++)
            {
                for (int k = 0; k < 3; k++)
                    for (int l = 0; l < 3; l++)
                        gveldisp[i](k, l) += (Part[grid[i].nindex[j]].GetVelocity(k) - gvel[i][k]) *
                                             (Part[grid[i].nindex[j]].GetVelocity(l) - gvel[i][l]) *
                                             Part[grid[i].nindex[j]].GetMass();
            }
            mtot = 1.0 / grid[i].mass;
            for (int k = 0; k < 3; k++)
                for (int l = 0; l < 3; l++) gveldisp[i](k, l) *= mtot;
        }
#ifdef USEOPENMP
    }
#endif
