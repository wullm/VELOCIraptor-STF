...
/// reads a gadget file. If cosmological simulation uses cosmology (generally assuming LCDM or small deviations from
/// this) to estimate the mean interparticle spacing and scales physical linking length passed by this distance. Also
/// reads header and over rides passed cosmological parameters with ones stored in header.
...
#define SKIPM2 Fgadmass[i].read((char *)&dummy, sizeof(dummy));
        ...
        for (i = 0; i < opt.num_files; i++)
            ...
            {
                if (opt.num_files > 1)
                    sprintf(buf, "%s.%d", opt.fname, i);
                else
                    sprintf(buf, "%s", opt.fname);
...
#ifdef GADGET2FORMAT
                SKIP2;
                Fgad[i].read((char *)&DATA[0], sizeof(char) * 4);
                DATA[4] = '\0';
                SKIP2;
                SKIP2;
#endif
        ...
        cout << "Particle system contains " << nbodies << " particles at is at time " << opt.a << " in a box of size "
             << opt.p << endl;
...
#ifdef NOMASS
        for (k = 0; k < NGTYPE; k++)
            if (header[ifirstfile].mass[k] != 0)
            {
                opt.MassValue = header[ifirstfile].mass[k] * mscale;
                break;
            }
#endif
        ...
        for (i = 0, count = 0, bcount = 0, pc = 0; i < opt.num_files;
             i++, pc = pc_new, count = count2, bcount = bcount2)
        {
#ifdef GADGET2FORMAT
            SKIP2;
            Fgad[i].read((char *)&DATA[0], sizeof(char) * 4);
            ...
            for (k = 0, ntot_withmasses = 0; k < NGTYPE; k++)
                if (header[i].mass[k] == 0) ntot_withmasses += header[i].npart[k];
#endif
            for (k = 0, Ntotfile = 0; k < NGTYPE; k++) Ntotfile += header[i].npart[k];
            // and read positions, velocities, ids, masses, etc
            SKIP2;
            if (dummy / Ntotfile / 3 != sizeof(FLOAT))
            {
                cout << " mismatch in position type size, file has " << dummy / Ntotfile / 3 << " but using "
                     << sizeof(FLOAT) << endl;
                exit(9);
            }
            ...
            if (dummy / Ntotfile / 3 != sizeof(FLOAT))
            {
                cout << " mismatch in velocity type size, file has " << dummy / Ntotfile / 3 << " but using "
                     << sizeof(FLOAT) << endl;
                exit(9);
            }
            for (k = 0, count2 = count, bcount2 = bcount, pc_new = pc; k < NGTYPE; k++)
...
#endif
            SKIP2;
            if (dummy / Ntotfile != sizeof(idval))
            {
                cout << " mismatch in ID type size, file has " << dummy / Ntotfile << " but using " << sizeof(idval)
                     << endl;
                exit(9);
            }
            for (k = 0, count2 = count, bcount2 = bcount, pc_new = pc; k < NGTYPE; k++)
                    ...
                    else if (opt.partsearchtype == PSTDARK)
                    {
                        ...
                        else
                        {
                            if (opt.iBaryonSearch == 1 && (k == GGASTYPE || k == GSTARTYPE || k == GBHTYPE))
                            {
                                ...
                                Pbaryons[bcount2].SetType(STARTYPE * (k == GSTARTYPE) + GASTYPE * (k == GGASTYPE) +
                                                          BHTYPE * (k == GBHTYPE));
#ifdef EXTRAINPUTINFO
                                if (opt.iextendedoutput)
                                {
            ...
            {
                SKIP2;
                if (dummy / ntot_withmasses != sizeof(REAL))
                {
                    cout << " mismatch in mass type size, file has " << dummy / ntot_withmasses << " but using "
                         << sizeof(REAL) << endl;
                    exit(9);
                }
            }
            ...
            if (header[i].npartTotal[GGASTYPE] > 0)
            {
...
#endif
                SKIP2;
                if (dummy / header[i].npart[GGASTYPE] != sizeof(FLOAT))
                {
                    cout << " mismatch in SPH type size, file has " << dummy / header[i].npart[GGASTYPE]
                         << " but using " << sizeof(FLOAT) << endl;
                    exit(9);
                }
                for (k = 0, count2 = count, bcount2 = bcount, pc_new = pc; k < NGTYPE; k++)
                {
                    if (k == GGASTYPE)
                    {
                        if ((opt.partsearchtype == PSTALL || opt.partsearchtype == PSTGAS ||
                             (opt.partsearchtype == PSTDARK && opt.iBaryonSearch == 1)))
                        {
                            for (n = 0; n < header[i].npart[k]; n++)
                            {
                ...
                // next skip N gas blocks where, typically N is 4 for Ne, Nh, HSML, SFR. Note that if header indicates
                // SFR block, data is kept
            ...
            if (header[i].npartTotal[GSTARTYPE] > 0)
            {
...
#endif
                SKIP2;
                if (dummy / header[i].npart[GSTARTYPE] != sizeof(FLOAT))
                {
                    cout << " mismatch in Star type size, file has " << dummy / header[i].npart[GSTARTYPE]
                         << " but using " << sizeof(FLOAT) << endl;
                    exit(9);
                }
                for (k = 0, count2 = count, bcount2 = bcount, pc_new = pc; k < NGTYPE; k++)
                {
                    for (n = 0; n < header[i].npart[k]; n++)
                    {
                        ...
                        {
                            if (!(k == GGASTYPE || k == GSTARTYPE || k == GBHTYPE)) { count2++; }
                            else
                            {
                                if (opt.iBaryonSearch == 1 && (k == GGASTYPE || k == GSTARTYPE))
                                {
...
#endif
                SKIP2;
                if (dummy / (header[i].npart[GSTARTYPE] + header[i].npart[GGASTYPE]) != sizeof(FLOAT))
                {
                    cout << " mismatch in SPH+STAR type size, file has "
                         << dummy / (header[i].npart[GSTARTYPE] + header[i].npart[GGASTYPE]) << " but using "
                         << sizeof(FLOAT) << endl;
                    exit(9);
                }
                for (k = 0, count2 = count, bcount2 = bcount, pc_new = pc; k < NGTYPE; k++)
...
#endif
                    SKIP2;
                    if (dummy / header[i].npart[GBHTYPE] != sizeof(FLOAT))
                    {
                        cout << " mismatch in BH type size, file has " << dummy / header[i].npart[GBHTYPE]
                             << " but using " << sizeof(FLOAT) << endl;
                        exit(9);
                    }
    ...
    for (i = 0; i < opt.num_files; i++)
        if (ireadfile[i])
        {
            ...
            count = 0;
            for (k = 0; k < NGTYPE; k++) count += header[i].npart[k];
            ...
            for (int sphblocks = 0; sphblocks < NUMGADGETSPHBLOCKS; sphblocks++)
                Fgadsph[i + sphblocks].open(buf, ios::in);
            for (int starblocks = 0; starblocks < NUMGADGETSTARBLOCKS; starblocks++)
                Fgadstar[i + starblocks].open(buf, ios::in);
            ...
            Fgadvel[i].read((char *)&DATA[0], sizeof(char) * 4);
            DATA[4] = '\0';
            Fgadvel[i].read((char *)&dummy, sizeof(dummy));
            Fgadvel[i].read((char *)&dummy, sizeof(dummy));

            Fgadid[i].read((char *)&dummy, sizeof(dummy));
            Fgadid[i].read((char *)&DATA[0], sizeof(char) * 4);
            DATA[4] = '\0';
            Fgadid[i].read((char *)&dummy, sizeof(dummy));
            Fgadid[i].read((char *)&dummy, sizeof(dummy));

            Fgadmass[i].read((char *)&dummy, sizeof(dummy));
            Fgadmass[i].read((char *)&DATA[0], sizeof(char) * 4);
            DATA[4] = '\0';
            Fgadmass[i].read((char *)&dummy, sizeof(dummy));
            Fgadmass[i].read((char *)&dummy, sizeof(dummy));
#endif
            ...
            for (int sphblocks = 0; sphblocks < NUMGADGETSPHBLOCKS; sphblocks++)
            {
                ...
                Fgadsph[i + sphblocks].read((char *)&DATA[0], sizeof(char) * 4);
                DATA[4] = '\0';
                Fgadsph[i + sphblocks].read((char *)&dummy, sizeof(dummy));
                Fgadsph[i + sphblocks].read((char *)&dummy, sizeof(dummy));
#endif
            ...
            for (int sphstart = 1; sphstart < NUMGADGETSPHBLOCKS; sphstart++)
            {
                for (int sphblocks = 0; sphblocks < sphstart; sphblocks++)
                {
                    ...
                    Fgadsph[i + sphstart].read((char *)&DATA[0], sizeof(char) * 4);
                    DATA[4] = '\0';
                    Fgadsph[i + sphstart].read((char *)&dummy, sizeof(dummy));
                    Fgadsph[i + sphstart].read((char *)&dummy, sizeof(dummy));
#endif
            ...
            for (int starblocks = 0; starblocks < NUMGADGETSTARBLOCKS; starblocks++)
            {
                ...
                Fgadstar[i + starblocks].read((char *)&DATA[0], sizeof(char) * 4);
                DATA[4] = '\0';
                Fgadstar[i + starblocks].read((char *)&dummy, sizeof(dummy));
                Fgadstar[i + starblocks].read((char *)&dummy, sizeof(dummy));
#endif
            ...
            for (int starstart = 1; starstart < NUMGADGETSTARBLOCKS; starstart++)
            {
                for (int starblocks = 0; starblocks < starstart; starblocks++)
                {
                    ...
                    Fgadstar[i + starstart].read((char *)&DATA[0], sizeof(char) * 4);
                    DATA[4] = '\0';
                    Fgadstar[i + starstart].read((char *)&dummy, sizeof(dummy));
                    Fgadstar[i + starstart].read((char *)&dummy, sizeof(dummy));
#endif
    ...
    for (i = 0, count = 0, pc = 0; i < opt.num_files; i++, pc = pc_new, count = count2)
        if (ireadfile[i])
        {
            ...
            for (k = 0, count2 = count, bcount2 = bcount, pc_new = pc; k < NGTYPE; k++)
                if (header[i].npart[k] > 0)
                {
                    // data loaded into memory in chunks
                    if (header[i].npart[k] < chunksize)
                        nchunk = header[i].npart[k];
                    else
                        nchunk = chunksize;
                    ninputoffset = 0;
                    for (n = 0; n < header[i].npart[k]; n += nchunk)
                    {
                        if (header[i].npart[k] - n < chunksize && header[i].npart[k] - n > 0)
                            nchunk = header[i].npart[k] - n;
                        ...
                        for (int sphblocks = 0; sphblocks < NUMGADGETSPHBLOCKS; sphblocks++)
                        {
                            Fgadsph[i + sphblocks].read((char *)&sphtempchunk[sphblocks * nchunk],
                                                        sizeof(FLOAT) * nchunk);
                        }
#endif
#ifdef STARON
                        for (int starblocks = 0; starblocks < NUMGADGETSTARBLOCKS; starblocks++)
                        {
                            Fgadstar[i + starblocks].read((char *)&startempchunk[starblocks * nchunk],
                                                          sizeof(FLOAT) * nchunk);
                        }
#endif
#ifndef NOMASS
                        ...
                        {
                            ctemp[0] = ctempchunk[0 + 3 * nn];
                            ctemp[1] = ctempchunk[1 + 3 * nn];
                            ctemp[2] = ctempchunk[2 + 3 * nn];
                            vtemp[0] = vtempchunk[0 + 3 * nn];
                            vtemp[1] = vtempchunk[1 + 3 * nn];
                            vtemp[2] = vtempchunk[2 + 3 * nn];
                            idval    = idvalchunk[nn];
                            for (int kk = 0; kk < 3; kk++)
                            {
                                ctemp[kk] = LittleFLOAT(ctemp[kk]);
                                vtemp[kk] = LittleFLOAT(vtemp[kk]);
                            }
#ifdef GASON
                            for (int sphblocks = 0; sphblocks < NUMGADGETSPHBLOCKS; sphblocks++)
                                sphtempchunk[sphblocks * nchunk + nn] =
                                    LittleFLOAT(sphtempchunk[sphblocks * nchunk + nn]);
#endif
#ifdef STARON
                            for (int starblocks = 0; starblocks < NUMGADGETSTARBLOCKS; starblocks++)
                                startempchunk[starblocks * nchunk + nn] =
                                    LittleFLOAT(startempchunk[starblocks * nchunk + nn]);
#endif
                            ...
                            if (k != GGASTYPE && k != GSTARTYPE && k != GBHTYPE && dtemp < MP_DM && dtemp > 0)
                                MP_DM = dtemp;
                            if (k == GGASTYPE && dtemp < MP_B && dtemp > 0) MP_B = dtemp;
                            ...
                            // load the particle into a particle buffer. If the particle belongs on local thread, then
                            // just copy it over to the Part array (or Pbaryons array if the iBaryonSearch is set).
                            // Otherwise, keep adding to the Pbuf array till the array is full and then send messages.
                            if (opt.partsearchtype == PSTALL)
                            {
                                Pbuf[ibufindex] = Particle(
                                    dtemp * mscale, ctemp[0] * lscale, ctemp[1] * lscale, ctemp[2] * lscale,
                            ...
                            else if (opt.partsearchtype == PSTSTAR)
                            {
                                if (k == GSTARTYPE)
                                {
                                    // if using MPI, determine proccessor and place in ibuf, store particle in particle
                                    // buffer and if buffer full, broadcast data unless ibuf is 0, then just store
                                    // locally
                                    Pbuf[ibufindex] = Particle(
                                        dtemp * mscale, ctemp[0] * lscale, ctemp[1] * lscale, ctemp[2] * lscale,
                                        vtemp[0] * opt.velocityinputconversion * sqrt(opt.a) + Hubbleflow * ctemp[0],
    ...
    if (opt.nsnapread > 1)
    {
        MPI_Allgather(Nreadbuf, opt.nsnapread, MPI_Int_t, mpi_nsend_readthread, opt.nsnapread, MPI_Int_t,
                      mpi_comm_read);
        MPISendParticlesBetweenReadThreads(opt, Preadbuf, Part.data(), ireadtask, readtaskID, Pbaryons, mpi_comm_read,
                                           mpi_nsend_readthread, mpi_nsend_readthread_baryon);
    }
#endif
