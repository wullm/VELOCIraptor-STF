...
int xdr_gas(XDR *pxdrs, struct tipsy_gas_particle *ph)
{
    ...
    for (i = 0; i < NCHILADAMAXDIM; i++)
        if (!xdr_float(pxdrs, &ph->pos[i])) return 0;
    for (i = 0; i < NCHILADAMAXDIM; i++)
        if (!xdr_float(pxdrs, &ph->vel[i])) return 0;
...
void xdr_NC_Open(XDR *xdrs, enum xdr_op op, FILE **fp, char *achInFile, long lstart)
{
    ...
    else if (op == XDR_ENCODE && lstart)
        *fp = fopen(achInFile, "a");
    else if (op == XDR_ENCODE && lstart == 0)
        *fp = fopen(achInFile, "w");
    assert(*fp);
    fseek(*fp, lstart, 0);
...
Int_t Nchilada_get_nbodies(char *fname, int ptype, Options &opt)
{
    ...
    */
    ...
    if (nbodies == 0)
    {
        cout << "Error. Zero particles of type " << opt.partsearchtype << " found. Either 0 nor can't find file"
             << filename << endl;
#ifdef USEMPI
        MPI_Abort(MPI_COMM_WORLD, 9);
#else
...
void ReadNchilada(Options &opt, vector<Particle> &Part, const Int_t nbodies, Particle *&Pbaryons, Int_t nbaryons)
{
    ...
    // counters
    Int_t count, countsph, countstar, countbh, count2, bcount, bcount2, pc, pc_new, Ntot, indark, ingas, instar, inbh,
        Ntotfile;
    ...
    */
    ...
    if (opt.partsearchtype == PSTALL)
    {
        // lets assume there are dm/stars/gas.
        nusetypes   = 3;
        usetypes[0] = NCHILADAGASTYPE;
        usetypes[1] = NCHILADADMTYPE;
        usetypes[2] = NCHILADASTARTYPE;
    }
    else if (opt.partsearchtype == PSTDARK)
    {
        nusetypes   = 1;
        usetypes[0] = NCHILADADMTYPE;
        if (opt.iBaryonSearch)
        {
            nbusetypes  = 2;
            usetypes[1] = NCHILADAGASTYPE;
            usetypes[2] = NCHILADASTARTYPE;
        }
    }
    else if (opt.partsearchtype == PSTGAS)
    ...
    if (ThisTask == 0 && opt.iverbose > 1)
    {
        ...
        // for (i=0;i<nusetypes;i++) cout<<"Particle "<<usetypes[i]<<" with name
        // "<<hdf_gnames.part_names[usetypes[i]]<<endl;
        if (opt.partsearchtype == PSTDARK && opt.iBaryonSearch)
        {
            cout << "Additionally, as full separate baryon search , expecting " << nbusetypes << " baryon particles"
                 << endl;
            // for (i=1;i<=nbusetypes;i++) cout<<"Particle "<<usetypes[i]<<" with name
            // "<<hdf_gnames.part_names[usetypes[i]]<<endl;
        }
    }
    ...
    */
    ...
    if (ireadtask[ThisTask] >= 0)
    {
        ...
        if (opt.partsearchtype==PSTALL || (opt.partsearchtype==PSTDARK && opt.iBaryonSearch>=1) ||
        opt.partsearchtype==PSTGAS) { fgasu.open((string(opt.fname)+
        nchilada_part_name.part_names[NCHILADAGASTYPE]+string("U")).c_str()); fgassfr.open((string(opt.fname)+
        nchilada_part_name.part_names[NCHILADAGASTYPE]+string("SFR")).c_str()); fgasz.open((string(opt.fname)+
        nchilada_part_name.part_names[NCHILADAGASTYPE]+string("Z")).c_str());
        }
        if (opt.partsearchtype==PSTALL || (opt.partsearchtype==PSTDARK && opt.iBaryonSearch>=1) ||
        opt.partsearchtype==PSTGAS) { fstartage.open((string(opt.fname)+
        nchilada_part_name.part_names[NCHILADASTARTYPE]+string("U")).c_str()); fstarz.open((string(opt.fname)+
        nchilada_part_name.part_names[NCHILADASTARTYPE]+string("Z")).c_str());
        }
        */
        for (j = 0; j < nusetypes; j++)
        {
            fpos[j] =
                fopen((string(opt.fname) + nchilada_part_name.part_names[usetypes[j]] + string("pos")).c_str(), "rb");
            fvel[j] =
                fopen((string(opt.fname) + nchilada_part_name.part_names[usetypes[j]] + string("vel")).c_str(), "rb");
            fmass[j] =
                fopen((string(opt.fname) + nchilada_part_name.part_names[usetypes[j]] + string("mass")).c_str(), "rb");
        }
#ifdef GASON
        if (opt.partsearchtype == PSTALL || (opt.partsearchtype == PSTDARK && opt.iBaryonSearch >= 1) ||
        ...
        {
            fgasu =
                fopen((string(opt.fname) + nchilada_part_name.part_names[NCHILADAGASTYPE] + string("U")).c_str(), "rb");
#ifdef STARON
            fgassfr = fopen(
                (string(opt.fname) + nchilada_part_name.part_names[NCHILADAGASTYPE] + string("SFR")).c_str(), "rb");
            fgasz =
                fopen((string(opt.fname) + nchilada_part_name.part_names[NCHILADAGASTYPE] + string("Z")).c_str(), "rb");
#endif
        }
#endif
        ...
        {
            fstartage = fopen(
                (string(opt.fname) + nchilada_part_name.part_names[NCHILADASTARTYPE] + string("U")).c_str(), "rb");
            fstarz = fopen((string(opt.fname) + nchilada_part_name.part_names[NCHILADASTARTYPE] + string("Z")).c_str(),
                           "rb");
        }
#endif
        ...
        for (j = 0; j < nusetypes; j++)
        {
            ...
            if (fhpos.code == float32)
                posfloatbuff = (float *)posdata;
            else
                posdoublebuff = (double *)posdata;
            veldata = readFieldData(fvel[j], fhvel, 3, numParticles, startParticle);
            if (fhvel.code == float32)
                velfloatbuff = (float *)veldata;
            else
                veldoublebuff = (double *)veldata;
            massdata = readFieldData(fmass[j], fhmass, 1, numParticles, startParticle);
            if (fhmass.code == float32)
                massfloatbuff = (float *)massdata;
            else
                massdoublebuff = (double *)massdata;
            iddata = readFieldData(fid[j], fhid, 1, numParticles, startParticle);
            if (fhid.code == int32)
                intbuff = (int *)iddata;
            else if (fhid.code == int64)
                longbuff = (long long *)iddata;
            else if (fhid.code == uint32)
                uintbuff = (unsigned int *)iddata;
            else if (fhid.code == uint64)
                ulongbuff = (unsigned long long *)iddata;
#ifdef GASON
            gasudata = readFieldData(fgasu, fhgasu, 1, numParticles, startParticle);
            if (fhgasu.code == float32)
                gasufloatbuff = (float *)gasudata;
            else
                gasudoublebuff = (double *)gasudata;
#ifdef STARON
            if (usetypes[j] == NCHILADAGASTYPE)
            {
                gaszdata = readFieldData(fgasz, fhgasz, 1, numParticles, startParticle);
                if (fhgasz.code == float32)
                    gaszfloatbuff = (float *)gaszdata;
                else
                    gaszdoublebuff = (double *)gaszdata;
                gassfrdata = readFieldData(fgassfr, fhgassfr, 1, numParticles, startParticle);
                if (fhgassfr.code == float32)
                    gassfrfloatbuff = (float *)gassfrdata;
                else
                    gassfrdoublebuff = (double *)gassfrdata;
            }
#endif
#endif
#ifdef STARON
            if (usetypes[j] == NCHILADASTARTYPE)
            {
                ...
                if (fhstarz.code == float32)
                    starzfloatbuff = (float *)starzdata;
                else
                    starzdoublebuff = (double *)starzdata;
                startagedata = readFieldData(fstartage, fhstartage, 1, numParticles, startParticle);
                if (fhstartage.code == float32)
                    startagefloatbuff = (float *)startagedata;
                else
                    startagedoublebuff = (double *)startagedata;
            }
#endif
            for (i = 0; i < fhpos.nbodies; i++)
            {
#ifdef USEMPI
                if (fhpos.code == float32)
                    ibuf = MPIGetParticlesProcessor(posfloatbuff[i], posfloatbuff[i + nbodies],
                                                    posfloatbuff[i + nbodies * 2]);
                else
                    ibuf = MPIGetParticlesProcessor(posdoublebuff[i], posdoublebuff[i + nbodies],
                                                    posdoublebuff[i + nbodies * 2]);
                if (fhpos.code == float32)
                {
                    Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetPosition(posfloatbuff[i], posfloatbuff[i + nbodies],
                                                                  posfloatbuff[i + nbodies * 2]);
                    Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetVelocity(velfloatbuff[i], velfloatbuff[i + nbodies],
                                                                  velfloatbuff[i + nbodies * 2]);
                    Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetMass(massfloatbuff[i]);
                }
                else
                {
                    Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetPosition(posdoublebuff[i], posdoublebuff[i + nbodies],
                                                                  posdoublebuff[i + nbodies * 2]);
                    Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetVelocity(veldoublebuff[i], veldoublebuff[i + nbodies],
                                                                  veldoublebuff[i + nbodies * 2]);
                    Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetMass(massdoublebuff[i]);
                }
                if (fhid.code == uint32)
                    Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetPID(intbuff[i]);
                else
                    Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetPID(longbuff[i]);
                Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetID(i);
                if (k == NCHILADAGASTYPE)
                    Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetType(GASTYPE);
                else if (k == NCHILADADMTYPE)
                    Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetType(DARKTYPE);
                else if (k == NCHILADASTARTYPE)
                {
#ifdef BHON
                    if (fhstartage.code == float32)
                        tageval = startagefloatbuff[i];
                    else
                        tageval = startagedoublebuff[i];
                    if (tageval > 0)
                        Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetType(STARTYPE);
                    else
                        Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetType(BHTYPE);
#else
                    Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetType(STARTYPE);
#endif
                }
                ...
                {
                    if (fhgasu.code == float32)
                        Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetU(gasufloatbuff[i]);
                    else
                        Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetU(gasudoublebuff[i]);
#ifdef STARON
                    if (fhgassfr.code == float32)
                        Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetSFR(gassfrfloatbuff[i]);
                    else
                        Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetSFR(gassfrdoublebuff[i]);
                    if (fhgasz.code == float32)
                        Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetZmet(gaszfloatbuff[i]);
                    else
                        Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetZmet(gaszdoublebuff[i]);
#endif
                }
#endif
#ifdef STARON
...
#endif
                    if (fhstarz.code == float32)
                        Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetZmet(starzfloatbuff[i]);
                    else
                        Pbuf[ibuf * BufSize + Nbuf[ibuf]].SetZmet(starzdoublebuff[i]);
                ...
                else
                {
                    ...
                    if (Nbuf[ibuf] == BufSize && ireadtask[ibuf] < 0)
                    {
                        ...
                        MPI_Send(&Pbuf[ibuf * BufSize], sizeof(Particle) * Nbuf[ibuf], MPI_BYTE, ibuf, ibuf,
                                 MPI_COMM_WORLD);
                        Nbuf[ibuf] = 0;
                    }
...
// start of non mpi section
            ...
            if (fhid.code == uint32)
                Part[i].SetPID(intbuff[i]);
            else
                Part[i].SetPID(longbuff[i]);
            Part[i].SetID(i);
            if (k == NCHILADAGASTYPE)
                Part[i].SetType(GASTYPE);
            else if (k == NCHILADADMTYPE)
            ...
            {
                if (fhgasu.code == float32)
                    Part[i].SetU(gasufloatbuff[i]);
                else
                    Part[i].SetU(gasudoublebuff[i]);
#ifdef STARON
                if (fhgassfr.code == float32)
                    Part[i].SetSFR(gassfrfloatbuff[i]);
                else
                    Part[i].SetSFR(gassfrdoublebuff[i]);
                if (fhgasz.code == float32)
                    Part[i].SetZmet(gaszfloatbuff[i]);
                else
                    Part[i].SetZmet(gaszdoublebuff[i]);
#endif
            }
#endif
#ifdef STARON
...
#endif
                if (fhstarz.code == float32)
                    Part[i].SetZmet(starzfloatbuff[i]);
                else
                    Part[i].SetZmet(starzdoublebuff[i]);
    ...
    if (ireadtask[ThisTask] >= 0 && opt.nsnapread > 1)
    {
        ...
        if (Nlocalbuf > 0)
        {
            ...
            // determine offsets
            nreadoffset[0] = 0;
            for (i = 1; i < opt.nsnapread; i++) nreadoffset[i] = nreadoffset[i - 1] + Nreadbuf[i - 1];
        ...
        if (opt.partsearchtype == PSTDARK && opt.iBaryonSearch)
        {
            if (ThisTask < opt.nsnapread)
            {
                for (ibuf = 0; ibuf < opt.nsnapread; ibuf++)
                    if (mpi_nsend[ThisTask * NProcs + ibuf] > 0)
                    {
                        Nbuf[ibuf] = 0;
                        for (i = 0; i < mpi_nsend[ThisTask * NProcs + ibuf]; i++)
                        {
                            k = Pbuf[nreadoffset[ibuf] + i].GetType();
                            if (!(k == GASTYPE || k == STARTYPE || k == BHTYPE))
                                Pbuf[nreadoffset[ibuf] + i].SetID(0);
                            else
                            {
                                if (k == GASTYPE)
                                    Pbuf[nreadoffset[ibuf] + i].SetID(1);
                                else if (k == STARTYPE)
                                    Pbuf[nreadoffset[ibuf] + i].SetID(2);
                                else if (k == BHTYPE)
                                    Pbuf[nreadoffset[ibuf] + i].SetID(3);
                                Nbuf[ibuf]++;
                            }
                        }
                        qsort(&Pbuf[nreadoffset[ibuf]], mpi_nsend[ThisTask * NProcs + ibuf], sizeof(Particle),
                              IDCompare);
                    }
            }
        ...
        MPISendParticlesBetweenReadThreads(opt, Pbuf, Part.data(), nreadoffset, ireadtask, readtaskID, Pbaryons,
                                           mpi_nsend_baryon);
        if (ireadtask[ThisTask] >= 0)
        {
            delete[] Pbuf;
            ...
            if (opt.iBaryonSearch)
                for (i = 0; i < Nlocalbaryon[0]; i++) Pbaryons[i].SetID(i + Nlocal);
        } // end of read tasks
    }
#endif
    ...
    if (opt.Neff == -1)
    {
        ...
        LN = pow(((MP_DM + MP_B) * opt.massinputconversion / opt.h) /
                     (opt.Omega_m * 3.0 * opt.H * opt.h * opt.H * opt.h / (8.0 * M_PI * opt.G)),
                 1. / 3.) *
             opt.a;
    }
    else
...
#endif
    /// if not an individual halo, assume cosmological and store scale of the highest resolution interparticle spacing
    /// to scale the physical FOF linking length
    ...
    for (i = 0; i < Nlocal; i++)
    {
        ...
        for (int j = 0; j < 3; j++)
            Part[i].SetVelocity(j, Part[i].GetVelocity(j) * opt.velocityinputconversion * sqrt(opt.a) +
                                       Hubbleflow * Part[i].GetPosition(j));
    ...
    if (Pbaryons != NULL && opt.iBaryonSearch == 1)
    {
        for (i = 0; i < Nlocalbaryon[0]; i++)
        {
            ...
            for (int j = 0; j < 3; j++)
                Pbaryons[i].SetVelocity(j, Pbaryons[i].GetVelocity(j) * opt.velocityinputconversion * sqrt(opt.a) +
                                               Hubbleflow * Pbaryons[i].GetPosition(j));
