...
void ReadTipsy(Options &opt, vector<Particle> &Part, const Int_t nbodies, Particle *&Pbaryons, Int_t nbaryons)
{
    ...
    if (ThisTask == 0)
    {
        ...
        z      = 1. / opt.a - 1.;
        Hubble = opt.h * opt.H *
                 sqrt((1.0 - opt.Omega_m - opt.Omega_Lambda) * pow(1.0 + z, 2.0) + opt.Omega_m * pow(1.0 + z, 3.0) +
                      opt.Omega_Lambda);
        opt.rhobg    = 3. * Hubble * Hubble / 8.0 / M_PI / opt.G * opt.Omega_m;
        Double_t bnx = -((1 - opt.Omega_m - opt.Omega_Lambda) * pow(aadjust, -2.0) + opt.Omega_Lambda) /
                       ((1 - opt.Omega_m - opt.Omega_Lambda) * pow(aadjust, -2.0) + opt.Omega_m * pow(aadjust, -3.0) +
                        opt.Omega_Lambda);
        ...
        cout << "System to be searched contains " << nbodies << " particles of type " << opt.partsearchtype
             << " at time " << opt.a << endl;
        ...
        if (opt.p > 0)
        {
            ...
            for (Int_t i = 0; i < ngas; i++)
            {
                ...
                {
                    posfirst[0] = gas.pos[0];
                    posfirst[1] = gas.pos[1];
                    posfirst[2] = gas.pos[2];
                    count++;
                    break;
                }
            }
            if (count == 0)
            {
                for (Int_t i = 0; i < ndark; i++)
                {
                    ...
                    {
                        posfirst[0] = dark.pos[0];
                        posfirst[1] = dark.pos[1];
                        posfirst[2] = dark.pos[2];
                        count++;
                        break;
                    }
                }
            ...
            if (count == 0)
            {
                for (Int_t i = 0; i < nstar; i++)
                {
                    ...
                    {
                        posfirst[0] = star.pos[0];
                        posfirst[1] = star.pos[1];
                        posfirst[2] = star.pos[2];
                        count++;
                        break;
                    }
                }
        ...
        for (Int_t i = 0; i < ngas; i++)
        {
            ...
            if (opt.p > 0.0)
            {
                for (int j = 0; j < 3; j++)
                {
                    if (gas.pos[j] - posfirst[j] > opt.p / 2.0)
                        gas.pos[j] -= opt.p;
                    else if (gas.pos[j] - posfirst[j] < -opt.p / 2.0)
                        gas.pos[j] += opt.p;
                }
            }
...
#ifndef USEMPI
                Part[count] =
                    Particle(gas.mass * mscale, gas.pos[0] * lscale, gas.pos[1] * lscale, gas.pos[2] * lscale,
                             gas.vel[0] * opt.velocityinputconversion + Hubbleflow * gas.pos[0],
                             gas.vel[1] * opt.velocityinputconversion + Hubbleflow * gas.pos[1],
                             gas.vel[2] * opt.velocityinputconversion + Hubbleflow * gas.pos[2], count, GASTYPE);
#else
...
#ifndef USEMPI
                Part[count] =
                    Particle(dark.mass * mscale, dark.pos[0] * lscale, dark.pos[1] * lscale, dark.pos[2] * lscale,
                             dark.vel[0] * opt.velocityinputconversion + Hubbleflow * dark.pos[0],
                             dark.vel[1] * opt.velocityinputconversion + Hubbleflow * dark.pos[1],
                             dark.vel[2] * opt.velocityinputconversion + Hubbleflow * dark.pos[2], count, DARKTYPE);
#else
        ...
        for (Int_t i = 0; i < nstar; i++)
        {
            ...
            if (opt.p > 0.0)
            {
                for (int j = 0; j < 3; j++)
                {
                    if (star.pos[j] - posfirst[j] > opt.p / 2.0)
                        star.pos[j] -= opt.p;
                    else if (star.pos[j] - posfirst[j] < -opt.p / 2.0)
                        star.pos[j] += opt.p;
                }
            }
...
#ifndef USEMPI
                Part[count] =
                    Particle(star.mass * mscale, star.pos[0] * lscale, star.pos[1] * lscale, star.pos[2] * lscale,
                             star.vel[0] * opt.velocityinputconversion + Hubbleflow * star.pos[0],
                             star.vel[1] * opt.velocityinputconversion + Hubbleflow * star.pos[1],
                             star.vel[2] * opt.velocityinputconversion + Hubbleflow * star.pos[2], count, STARTYPE);
#else
    ...
    if (opt.Neff == -1)
    {
        ...
        LN = pow(((MP_DM)*opt.massinputconversion) /
                     (opt.Omega_cdm * 3.0 * opt.H * opt.h * opt.H * opt.h / (8.0 * M_PI * opt.G)),
                 1. / 3.) *
             opt.a;
    }
    else
