 ...
 *  \brief this file contains routines that allow the velociraptor library to interface with the swift N-body code from
 * within swift.
 */
...
inline int ConfigCheckSwift(Options &opt, Swift::siminfo &s)
{
    ...
    {
        if (ThisTask == 0)
            cerr << "Conflict in config file: both gas/star/etc particle type search AND the separate baryonic "
    ...
    {
        if (ThisTask == 0)
            cerr << "Conflict in config file: Asking for Bound Field objects but also asking to keep the 3DFOF/then "
    ...
    {
        if (ThisTask == 0)
            cerr << "Conflict in config file: simulation contains no dark matter but searching only dark matter. This "
                    "is incompatible. Check config\n";
        return SWIFTCONFIGOPTCONFLICT;
    }
    if (s.idarkmatter == false && opt.partsearchtype == PSTALL && opt.iBaryonSearch)
    {
        if (ThisTask == 0)
            cerr << "Conflict in config file: simulation contains no dark matter but using dark matter to define links "
                    "when searching all particles . This is incompatible. Check config\n";
        return SWIFTCONFIGOPTCONFLICT;
    }
    if (s.igas == false && opt.partsearchtype == PSTGAS)
    {
        if (ThisTask == 0)
            cerr << "Conflict in config file: simulation contains no gas but searching only gas. This is incompatible. "
                    "Check config\n";
        return SWIFTCONFIGOPTCONFLICT;
    }
    ...
    {
        if (ThisTask == 0)
            cerr << "Conflict in config file: simulation contains no gas but searching only stars or using stars as "
                    "basis for links. This is incompatible. Check config\n";
        return SWIFTCONFIGOPTCONFLICT;
    }
    ...
    {
        if (ThisTask == 0)
            cerr << "Code not compiled with ADIOS output enabled. Recompile with this enabled or change "
                    "Binary_output.\n";
        return SWIFTCONFIGOPTERROR;
    }
#endif
...
int InitVelociraptor(char *configname, unitinfo u, siminfo s, const int numthreads)
{
    ...
    // write velociraptor configuration info, appending .configuration to the input config file and writing every config
    // option
...
void SetVelociraptorSimulationState(cosmoinfo c, siminfo s)
{
    ...
    if (libvelociraptorOpt.iSingleHalo)
        MinNumOld = libvelociraptorOpt.MinSize;
    else
        MinNumOld = libvelociraptorOpt.HaloMinSize;
...
groupinfo *InvokeVelociraptor(const int snapnum, char *outputname, cosmoinfo c, siminfo s,
                              const size_t num_gravity_parts, const size_t num_hydro_parts, const size_t num_star_parts,
                              struct swift_vel_part *swift_parts, int *cell_node_ids, const int numthreads,
                              const int ireturngroupinfoflag, int *const numpartingroups)
{
#ifndef GASON
    cout << "Gas has not been turned on in VELOCIraptor. Set GASON in Makefile.config and recompile VELOCIraptor."
         << endl;
    return 0;
#endif
#ifdef USEMPI
...
#pragma omp parallel
    ...
    /// If we are performing a baryon search, sort the particles so that the DM particles are at the start of the array
    /// followed by the gas particles.
    ...
    if (libvelociraptorOpt.iBaryonSearch > 0 && libvelociraptorOpt.partsearchtype != PSTALL)
    {
        ...
        for (auto i = 0; i < Nlocal; i++)
        {
            ...
            else
            {
                ...
                {
                    cout << "Unknown particle type found: index=" << i << " type=" << swift_parts[i].type
                         << " while treating baryons differently. Exiting..." << endl;
                    return NULL;
                }
            }
        }
    ...
    else
    {
        for (auto i = 0; i < Nlocal; i++)
        {
            if (swift_parts[i].type != DARKTYPE && swift_parts[i].type != GASTYPE && swift_parts[i].type != STARTYPE &&
                swift_parts[i].type != BHTYPE)
            {
// if high res then particle is also allowed to be type 2, a DARK2TYPE
...
#endif
                    cout << "Unknown particle type found: index=" << i << " type=" << swift_parts[i].type
                         << " when loading particles. Exiting..." << endl;
                    return NULL;
#ifdef HIGHRES
                }
#endif
    ...
    cout << "TIME::" << ThisTask << " took " << time1 << " to copy " << Nlocal
         << " particles from SWIFT to a local format. Out of " << Ntotal << endl;
    cout << ThisTask << " There are " << Nlocal << " particles and have allocated enough memory for " << Nmemlocal
         << " requiring " << Nmemlocal * sizeof(Particle) / 1024. / 1024. / 1024. << "GB of memory " << endl;
    if (libvelociraptorOpt.iBaryonSearch > 0)
        cout << ThisTask << "There are " << Nlocalbaryon[0] << " baryon particles and have allocated enough memory for "
             << Nmemlocalbaryon << " requiring " << Nmemlocalbaryon * sizeof(Particle) / 1024. / 1024. / 1024.
             << "GB of memory " << endl;
    cout << ThisTask
         << " will also require additional memory for FOF algorithms and substructure search. Largest mem needed for "
            "preliminary FOF search. Rough estimate is "
         << Nlocal * (sizeof(Int_tree_t) * 8) / 1024. / 1024. / 1024. << "GB of memory" << endl;
    ...
    if (libvelociraptorOpt.iInclusiveHalo > 0 && libvelociraptorOpt.iInclusiveHalo < 3)
    {
        ...
        for (Int_t i = 0; i < Nlocal; i++)
        {
            ...
            originalID[i]   = parts[i].GetID();
            parts[i].SetID(i);
        }
        Int_t *noffsethalos = BuildNoffset(Nlocal, parts.data(), nhalos, numinhalos, sortvalhalos);
        GetInclusiveMasses(libvelociraptorOpt, Nlocal, parts.data(), nhalos, pfof, numinhalos, pdatahalos,
                           noffsethalos);
    ...
    if (libvelociraptorOpt.iSubSearch)
    {
        ...
        time1 = MyGetTime() - time1;
        cout << "TIME::" << ThisTask << " took " << time1 << " to search for substructures " << Nlocal << " with "
             << nthreads << endl;
    }
    ...
    // if only searching initially for dark matter groups, once found, search for associated baryonic structures if
    // requried
    if (libvelociraptorOpt.iBaryonSearch > 0)
    {
        time1 = MyGetTime();
        ...
        {
            pfofall     = SearchBaryons(libvelociraptorOpt, nbaryons, pbaryons, Nlocal, parts, pfof, ngroup, nhalos,
                                    libvelociraptorOpt.iseparatefiles, libvelociraptorOpt.iInclusiveHalo, pdata);
            pfofbaryons = &pfofall[Nlocal];
        }
        // if FOF search overall particle types then running sub search over just dm and need to associate baryons to
        // just dm particles must determine number of baryons, sort list, run search, etc but only need to run search if
        // substructure has been searched
        else if (libvelociraptorOpt.iSubSearch == 1)
        {
            ...
            for (Int_t i = 0; i < Nlocal; i++)
            {
                if (parts[i].GetType() == DARKTYPE)
                    ndark++;
                else
                    nbaryons++;
            }
            pbaryons = NULL;
            SearchBaryons(libvelociraptorOpt, nbaryons, pbaryons, ndark, parts, pfof, ngroup, nhalos,
                          libvelociraptorOpt.iseparatefiles, libvelociraptorOpt.iInclusiveHalo, pdata);
        }
        time1 = MyGetTime() - time1;
    ...
    pglist     = SortAccordingtoBindingEnergy(libvelociraptorOpt, Nlocal, parts.data(), ngroup, pfof, numingroup,
                                          pdata); // alters pglist so most bound particles first
...
#endif
    // if returning to swift as swift is writing a snapshot, then write for the groups where the particles are found in
    // a file assuming that the swift task and swift index can be used to determine where a particle will be written.
    ...
    {
        if (pfof[i] > 0)
            parts[i].SetPID((pfof[i] + ngoffset) + libvelociraptorOpt.snapshotvalue);
        else
            parts[i].SetPID(0);
    }
#ifdef USEMPI
...
void CheckSwiftTasks(string message, const Int_t n, Particle *p)
{
    ...
    for (auto i = 0; i < n; i++)
    {
        ...
        {
            cerr << ThisTask
                 << " has odd swift particle with nonsensical swift task (cur index, swift index, swift task)=(" << i
                 << "," << index << "," << task << ")" << endl;
            numbad++;
        }
