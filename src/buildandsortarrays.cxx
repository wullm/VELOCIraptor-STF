...
Int_t *BuildNumInGroupTyped(const Int_t nbodies, const Int_t numgroups, Int_t *pfof, Particle *P, int type)
{
    ...
    for (Int_t i = 0; i <= numgroups; i++) numingroup[i] = 0;
    for (Int_t i = 0; i < nbodies; i++)
        if (pfof[i] > 0 && P[i].GetType() == type) numingroup[pfof[i]]++;
    return numingroup;
}
...
Int_t **BuildPGListTyped(const Int_t nbodies, const Int_t numgroups, Int_t *numingroup, Int_t *pfof, Particle *P,
                         int type)
{
    Int_t **pglist = new Int_t *[numgroups + 1];
    Int_t   pid;
...
Int_tree_t *BuildGroupTailArray(const Int_t nbodies, const Int_t numgroups, Int_t *numingroup, Int_t **pglist)
{
    ...
    for (Int_t i = 1; i <= numgroups; i++) { GTail[i] = pglist[i][numingroup[i] - 1]; }
    return GTail;
}
...
/// sort particles according to some quantity which is stored in particle type and build an array for a sorted particle
/// list remember this reorders the particle array!
                    ...
                    Int_t ioffset)
{
    ...
    for (Int_t i = 0; i < nbodies; i++)
    {
        ...
        if (sortval[Part[i].GetID()] > ioffset)
            Part[i].SetType(sortval[Part[i].GetID()]);
        else
            Part[i].SetType(nbodies + 1); // here move all particles not in groups to the back of the particle array
    }
...
///\todo must alter so that after pfof is reorderd, so is numingroup array and pglist so that do not have to reconstruct
///this list after reordering if numgroups==newnumgroups (ie, list has not shrunk)
                            ...
                            Int_t **pglist, Int_t *value)
{
    ...
    Int_t          groupid;
    for (Int_t i = 1; i <= numgroups; i++)
        if (numingroup[i] > 0) pq->Push(i, value[i]);
    for (Int_t i = 1; i <= newnumgroups; i++)
    {
        groupid = pq->TopQueue();
        pq->Pop();
