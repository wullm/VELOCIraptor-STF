...
OMP_Domain *OpenMPBuildDomains(Options &opt, const Int_t numompregions, KDTree *&tree, const Double_t rdist)
{
    ...
    for (auto i = 0; i < numompregions; i++)
    {
        for (auto k = 0; k < 3; k++)
        {
            xsearch[k][0] = ompdomain[i].bnd[k][0] - rdist;
            xsearch[k][1] = ompdomain[i].bnd[k][1] + rdist;
        }
        for (auto j = 0; j < numompregions; j++)
            if (j != i)
            {
...
#pragma omp for schedule(dynamic) nowait
        ...
        {
            tree3dfofomp[i] = new KDTree(&Part.data()[ompdomain[i].noffset], ompdomain[i].ncount, opt.Bsize,
                                         tree3dfofomp[i]->TPHYS, tree3dfofomp[i]->KEPAN, 100, 0, 0, 0, period);
        }
    }
    return tree3dfofomp;
}
...
int OpenMPSearchForOverlap(Double_t xsearch[3][2], Double_t bnd[3][2], Double_t period)
{
    ...
    if (!((bnd[0][1] < xsearch[0][0]) || (bnd[0][0] > xsearch[0][1]) || (bnd[1][1] < xsearch[1][0]) ||
          (bnd[1][0] > xsearch[1][1]) || (bnd[2][1] < xsearch[2][0]) || (bnd[2][0] > xsearch[2][1])))
        return 1;
    else
    {
        if (period == 0)
        ...
        else
        {
            for (int j = 0; j < 3; j++)
            {
                xsearchp[j][0] = xsearch[j][0];
                xsearchp[j][1] = xsearch[j][1];
            }
            for (int j = 0; j < 3; j++)
            {
                if (!((bnd[j][1] < xsearch[j][0] + period) || (bnd[j][0] > xsearch[j][1] + period)))
                {
                    xsearchp[j][0] += period;
                    xsearchp[j][1] += period;
                }
                else if (!((bnd[j][1] < xsearch[j][0] - period) || (bnd[j][0] > xsearch[j][1] - period)))
                {
                    xsearchp[j][0] -= period;
                    xsearchp[j][1] -= period;
                }
            }
            if (!((bnd[0][1] < xsearchp[0][0]) || (bnd[0][0] > xsearchp[0][1]) || (bnd[1][1] < xsearchp[1][0]) ||
                  (bnd[1][0] > xsearchp[1][1]) || (bnd[2][1] < xsearchp[2][0]) || (bnd[2][0] > xsearchp[2][1])))
                return 1;
            else
                return 0;
        }
...
int OpenMPSearchForOverlap(Particle &Part, Double_t bnd[3][2], Double_t rdist, Double_t period)
{
    ...
    for (auto k = 0; k < 3; k++)
    {
        xsearch[k][0] = Part.GetPosition(k) - rdist;
        xsearch[k][1] = Part.GetPosition(k) + rdist;
    }
...
{
    return ((xsearch[0][0] > bnd[0][0]) && (xsearch[0][1] < bnd[0][1]) && (xsearch[1][0] > bnd[1][0]) &&
            (xsearch[1][1] < bnd[1][1]) && (xsearch[2][0] > bnd[2][0]) && (xsearch[2][1] < bnd[2][1]));
}
...
Int_t OpenMPLocalSearch(Options &opt, const Int_t nbodies, vector<Particle> &Part, Int_t *&pfof, Int_t *&storeorgIndex,
                        ...
                        const Double_t rdist, const Int_t ompminsize, FOFcompfunc fofcmp, const Int_t numompregions,
...
#pragma omp                              parallel default(shared) private(i, p3dfofomp, orgIndex, ng)
    {
#pragma omp for schedule(dynamic) nowait reduction(+ : ngtot)
        for (i = 0; i < numompregions; i++)
        {
            if (opt.partsearchtype == PSTALL && opt.iBaryonSearch > 1)
                p3dfofomp = tree3dfofomp[i]->FOFCriterionSetBasisForLinks(fofcmp, param, ng, ompminsize, 0, 0,
                                                                          FOFchecktype, &Head[ompdomain[i].noffset],
                                                                          &Next[ompdomain[i].noffset]);
            else
                p3dfofomp = tree3dfofomp[i]->FOF(rdist, ng, ompminsize, 0, &Head[ompdomain[i].noffset],
                                                 &Next[ompdomain[i].noffset]);
            if (ng > 0)
            {
...
OMP_ImportInfo *OpenMPImportParticles(Options &opt, const Int_t nbodies, vector<Particle> &Part, Int_t *&pfof,
                                      Int_t *&storeorgIndex, const Int_t numompregions, OMP_Domain *&ompdomain,
...
void OpenMPLinkAcross(Options &opt, Int_t nbodies, vector<Particle> &Part, Int_t *&pfof, Int_t *&storeorgIndex,
...
#pragma omp            parallel default(shared) private(i, orgIndex, curIndex, x, nt, Pval, pfofcomp)
        {
            ...
            for (i = 0; i < numompregions; i++)
            {
                for (auto j = 0; j < omp_nrecv_total[i]; j++)
                {
                    ...
                    for (auto k = 0; k < nt; k++)
                    {
                        ...
                        if (pfof[orgIndex] > 0)
                        {
                            // only change if both particles are appropriate type and group ids indicate local needs to
                            // be exported
                            if (opt.partsearchtype == PSTALL && opt.iBaryonSearch > 1)
                                if (!(fofcheck(Part[curIndex], param) == 0 && fofcheck(*Pval, param) == 0)) continue;
                            ...
                            if (pfof[orgIndex] > pfofcomp)
                            {
                                ...
                                {
                                    orgIndex =
                                        storeorgIndex[Part[ss + ompdomain[i].noffset].GetID() + ompdomain[i].noffset];
...
#pragma omp parallel default(shared) private(i, orgIndex, curIndex, curTask)
        {
#pragma omp for nowait
        ...
        numloops++;
        if (opt.iverbose > 1)
            cout << ThisTask << " linking across at loop " << numloops << " having found " << omp_links_across_total
                 << " links" << endl;
...
Int_t OpenMPResortParticleandGroups(Int_t nbodies, vector<Particle> &Part, Int_t *&pfof, Int_t minsize)
{
    ...
    for (auto i = 0, start = 0; i < nbodies; i++)
    {
        if (Part[i].GetID() != Part[start].GetID())
        {
            ...
            if ((i - start) < minsize)
                for (Int_t j = start; j < i; j++) Part[j].SetID(0);
            else
                ngroups++;
            start = i;
        }
