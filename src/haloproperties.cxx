 ...
 *  \brief this file contains routines to characterize the bulk properties of the halo IFF one needs to scale linking
 lengths
...
/*! The FoF linking terms are adjusted by determining the physical extent of the halo and Rvir, Vcirc(Rvir), Mvir, Renc
   where Menclosed is [20%, 50%, 80%] of mass \n Units should be V=100 km/s, L=kpc, M=2.32e9 Msun so that G=1 and
   rhoc=3*Ho^2/8piG which gives 1.19e-7. These quantites, such as average spacing and ciricular velocity at virial
   radius, are stored in \ref Options.ellxscale and \ref Options.ellvscale.
*/
                         ...
                         Double_t Mtot)
{
    ...
    {
        rlim[1]  = Renc[2];
        MaxVcirc = sqrt(opt.G * Menc[2] * Mtot / Renc[2]);
    }
    else
    {
        rlim[1]  = Rvir;
        MaxVcirc = sqrt(opt.G * Mvir / Rvir);
    }
...
void AdjusttoCM(const Int_t nbodies, Particle *Part, Coordinate &cm, Coordinate &cmvel, const Double_t Mtot,
                Double_t *rlim, Double_t &MaxVcirc, Double_t tol)
{
    Int_t      i;
    int        nthreads;
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, r)
    {
#pragma omp for
#endif
        for (i = 1; i < nbodies; i++)
...
#pragma omp for reduction(+ : EncMass, Ninside, cmx, cmy, cmz)
        ...
        for (int j = 1; j < 3; j++)
            if (fabs((cm[j] - cmold[j]) / cmold[j]) > change) change = fabs((cm[j] - cmold[j]) / cmold[j]);
...
#pragma omp for
    ...
    {
        rmin[j] = rmax[j] = r;
        rave[j]           = rmin[j] * Part[0].GetMass();
        vcmax[j]          = vc;
    }

#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, r, vc)
    {
#pragma omp for
#endif
        for (i = 1; i < nbodies; i++)
    ...
    rlim[0] *= 0.99;
    rlim[2] *= 1.01;
    cout << "System (halo) has a radial extent of (min ave max)=(";
    for (int j = 0; j < 3; j++) cout << rlim[j] << " ";
    cout << ") Lunits, and a maximum circular velocity of " << MaxVcirc << " Vunits." << endl;
}

/*! Get virial quantities such as mass, radius by binnning system radial about cm
    and finding radius at which enclosed mass is some specific value or the average density is some specific value (in
   this case virial density)
*/
...
#pragma omp parallel
    ...
    for (int j = 0; j < nthreads; j++)
        for (int k = 0; k < nbins; k++) mbin[j][k] = 0.0;
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i)
    {
#pragma omp for
    ...
    for (int j = 0; j < nbins - 1; j++)
    {
        ...
        {
            Renc[itemp] =
                pow(10.0, (Menc[itemp] - Mencbin[j] / Mtot) / (Mencbin[j + 1] / Mtot - Mencbin[j] / Mtot) * deltalogr +
                              (logrmin + deltalogr * (j + 1.0)));
            if (itemp == 0) jtemp = j;
            itemp++;
            if (itemp == nenc) break;
        }
    ...
    for (int j = nbins - 2; j >= 0; j--)
        ...
        {
            Rvir = pow(10.0,
                       (1.0 - rhoavebin[j] / rhovir) / (rhoavebin[j + 1] / rhovir - rhoavebin[j] / rhovir) * deltalogr +
                           (logrmin + deltalogr * (j + 1.0)));
            Mvir = Mencbin[j] +
                   (Mencbin[j + 1] - Mencbin[j]) / deltalogr * (log10(Rvir) - (logrmin + deltalogr * (j + 1.0)));
            break;
        }
    delete[] Mbin;
    delete[] Mencbin;
    ...
    cout << "And (Mfrac,Rmfrac) of ";
    for (int j = 0; j < nenc; j++) cout << "(" << Menc[j] << "," << Renc[j] << ") ";
    cout << endl;
}
//@}
