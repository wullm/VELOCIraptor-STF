            ...
            for(k=0;k<6;k++)
            {
                for(n=0;n<header[i].npart[k];n++)
                {
                    ...
                    if (opt.partsearchtype==PSTALL)
                        for (m=0;m<3;m++) {if (ctemp[m]<mpi_xlim[m][0]) mpi_xlim[m][0]=ctemp[m];if
    (ctemp[m]>mpi_xlim[m][1]) mpi_xlim[m][1]=ctemp[m];} else if (opt.partsearchtype==PSTDARK) if
    (!k==GASTYPE||k==STARTYPE) for (m=0;m<3;m++) {if (ctemp[m]<mpi_xlim[m][0]) mpi_xlim[m][0]=ctemp[m];if
    (ctemp[m]>mpi_xlim[m][1]) mpi_xlim[m][1]=ctemp[m];} else if (opt.partsearchtype==PSTSTAR) if (k==STARTYPE) for
    (m=0;m<3;m++) {if (ctemp[m]<mpi_xlim[m][0]) mpi_xlim[m][0]=ctemp[m];if (ctemp[m]>mpi_xlim[m][1])
    mpi_xlim[m][1]=ctemp[m];} else if (opt.partsearchtype==PSTGAS) if (k==GASTYPE) for (m=0;m<3;m++) {if
    (ctemp[m]<mpi_xlim[m][0]) mpi_xlim[m][0]=ctemp[m];if (ctemp[m]>mpi_xlim[m][1]) mpi_xlim[m][1]=ctemp[m];}
                }
            }
        }
        */
        // There may be issues with particles exactly on the edge of a domain so before expanded limits by a small
        // amount now only done if a specific compile option passed
#ifdef MPIEXPANDLIM
        for (int j = 0; j < 3; j++)
        {
    ...
    for (i=0;i<opt.num_files; i++)
    {
        ...
        for (k=0;k<6;k++)
        {
            for (n=0;n<header[i].npart[k];n++)
            {
                ...
                for (m=0;m<3;m++) ctemp[m]=LittleFloat(ctemp[m]);
                for (m=0;m<3;m++)
ibin[m]=(int)floor((ctemp[m]-mpi_xlim[m][0])/(mpi_xlim[m][1]-mpi_xlim[m][0])*(Double_t)nbins1d); if
(opt.partsearchtype==PSTALL) for (m=0;m<3;m++) { mean[m]+=ctemp[m]; var[m]+=ctemp[m]*ctemp[m]; histo1d[m][ibin[m]]++;
                    }
                else if (opt.partsearchtype==PSTDARK)
...
//                double value=mpi_xlim[j][0]+((mpi_xlim[j][1]-mpi_xlim[j][0])/(Double_t)nbins1d)*(i-0.5);
                ...
                double
value=mpi_xlim[j][0]+((mpi_xlim[j][1]-mpi_xlim[j][0])/(Double_t)nbins1d)*(i-1)+((mpi_xlim[j][1]-mpi_xlim[j][0])/(Double_t)nbins1d)/(log(histo1d[j][i]))*(log(sum)-log(Ntot*nfac*kbnd));
...
(Ntot/(Double_t)mpi_nxsplit[ix]-binsum[0])*(histo3dbinval[n1+1][0]-histo3dbinval[n1][0])/(histo1d[0][n1+1]);
        ...
        for (j=0;j<mpi_nxsplit[iy];j++) {
            ...
            for (int n2=start[1];n2<nbins3d;n2++){
                ...
                binsum[1]+=lastbin;
                if
(binsum[1]>=Ntot/(Double_t)(mpi_nxsplit[ix]*mpi_nxsplit[iy])&&binsum[1]-lastbin<Ntot/(Double_t)(mpi_nxsplit[ix]*mpi_nxsplit[iy])){
                    end[1]=n2;
            ...
            for (k=0;k<mpi_nxsplit[iz];k++) {
                ...
                for (int n3=start[2];n3<nbins3d;n3++){
                    //lastbin=0; for (int n1=start[0];n1<end[0];n1++)for (int n2=start[1];n2<end[1];n2++)
lastbin+=histo3d[n1][n2][n3]; lastbin=0; for (int n1=start[0];n1<end[0];n1++)for (int n2=start[1];n2<end[1];n2++)
lastbin+=histo3d[n1*nbins3d2+n2*nbins3d+n3]; binsum[2]+=lastbin; if
(binsum[2]>=Ntot/(Double_t)(mpi_nxsplit[ix]*mpi_nxsplit[iy]*mpi_nxsplit[iz])&&binsum[2]-lastbin<Ntot/(Double_t)(mpi_nxsplit[ix]*mpi_nxsplit[iy]*mpi_nxsplit[iz])){
                        end[2]=n3;
                ...
                if (ireadfile[i])
                {
                    ...
                    for (k = 0; k < NGTYPE; k++)
                    {
                        for (n = 0; n < header[i].npart[k]; n++)
                        {
                            ...
                            if (opt.partsearchtype == PSTALL) { Nbuf[ibuf]++; }
                            else if (opt.partsearchtype == PSTDARK)
                            {
                                if (!(k == GGASTYPE || k == GSTARTYPE || k == GBHTYPE)) { Nbuf[ibuf]++; }
                                else
                                {
                                    if (opt.iBaryonSearch) { Nbaryonbuf[ibuf]++; }
                                }
                            }
                            else if (opt.partsearchtype == PSTSTAR)
                            {
                                if (k == STARTYPE) { Nbuf[ibuf]++; }
                            }
                            else if (opt.partsearchtype == PSTGAS)
                            {
                                if (k == GASTYPE) { Nbuf[ibuf]++; }
                            }
                        }
                    }
                    Fgad[i].close();
