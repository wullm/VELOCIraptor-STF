 ...
 *  \brief this file contains routines to check if groups are self-bound and if not unbind them as requried
    ...
    \todo Need to improve the gravity calculation (ie: for tree potential use something other than just monopole and
 also apply corrections if necessary). \todo Need to clean up unbind proceedure, ensure its mpi compatible and can be
 combined with a pglist output easily
 */
...
void GetNodeList(Node *np, Int_t &ncell, Node **nodelist, const Int_t bsize)
{
    ...
    {
        ncell++;
        GetNodeList(((SplitNode *)np)->GetLeft(), ncell, nodelist, bsize);
        ncell++;
        GetNodeList(((SplitNode *)np)->GetRight(), ncell, nodelist, bsize);
    }
    // else ncell++;
}
...
inline void MarkCell(Node *np, Int_t *marktreecell, Int_t *markleafcell, Int_t &ntreecell, Int_t &nleafcell,
                     Double_t *r2val, const Int_t bsize, Double_t *cR2max, Coordinate *cm, Double_t *cmtot,
                     Coordinate xpos, Double_t eps2)
{
    Int_t    nid = np->GetID();
    Double_t r2;
    r2 = 0;
    ...
    if (r2 < cR2max[nid])
    {
        if (np->GetCount() > bsize)
        {
            MarkCell(((SplitNode *)np)->GetLeft(), marktreecell, markleafcell, ntreecell, nleafcell, r2val, bsize,
                     cR2max, cm, cmtot, xpos, eps2);
            MarkCell(((SplitNode *)np)->GetRight(), marktreecell, markleafcell, ntreecell, nleafcell, r2val, bsize,
                     cR2max, cm, cmtot, xpos, eps2);
        }
        else
            markleafcell[nleafcell++] = nid;
    }
    else
    {
...
inline bool CheckGroupForBoundness(Options &opt, Double_t &Efrac, Double_t &maxE, Int_t ning)
{
    ...
    {
        if (((Efrac < opt.uinfo.minEfrac) || (maxE > 0)) && (ning >= opt.MinSize))
            unbindcheck = true;
        else
            unbindcheck = false;
    }
...
inline void FillUnboundArrays(Options &opt, int maxunbindsize, Int_t ning, Particle *&groupPart, const Double_t &Efrac,
...
inline void UpdateCMForUnboundParticles(Options &opt, Double_t &mass, Coordinate &cmvel, Int_t &nig,
                                        Particle *&groupPart, Int_t &nEplus, Int_t *&nEplusid, int *&Eplusflag)
{
...
{
    if (unbindloops > 0)
        for (auto j = 0; j < ning; j++) pglist[j] = groupPart[j].GetPID();
}

inline void GetBoundFractionAndMaxE(Options &opt, Int_t &ning, Particle *&groupPart, Coordinate &cmvel, Double_t &Efrac,
                                    Double_t &maxE, Int_t &nunbound, bool sortflag = true)
{
...
#pragma omp parallel for \
...
#endif
        v2 = 0.0;
        for (auto k = 0; k < 3; k++) v2 += pow(groupPart[j].GetVelocity(k) - cmvel[k], 2.0);
        Ti = 0.5 * mass * v2;
#ifdef GASON
    ...
    {
        sort(groupPart, groupPart + ning,
             [](const Particle &a, const Particle &b) { return a.GetDensity() < b.GetDensity(); });
    }
}
...
//@{
/*!
    Interface for unbinding proceedure. Unbinding routine requires several arrays, such as numingroup, pglist,gPart,ids,
   etc This arrays may have been constructed prior to the unbinding call and so can be passed to the routine if this is
   called it uses Particle array then deletes it.
*/
int CheckUnboundGroups(Options opt, const Int_t nbodies, Particle *Part, Int_t &ngroup, Int_t *&pfof, Int_t *numingroup,
                       Int_t **pglist, int ireorder, Int_t *groupflag)
{
    ...
    {
        storeval1[i] = Part[i].GetPID();
        storeval2[i] = Part[i].GetID();
        storeden[i]  = Part[i].GetDensity();
        Part[i].SetPID(i);
        Part[i].SetID(pfof[i] + nbodies * (pfof[i] == 0));
    }
    ...
    if (groupflag != NULL)
    {
        for (Int_t i = 0; i <= ng; i++)
            if (numingroup[i] == 0) groupflag[i] = 0;
        // and reorder it if reordering group ids
        if (ireorder)
            ReorderGroupIDsAndArraybyValue(const Int_t numgroups, const Int_t newnumgroups, Int_t *numingroup,
                                           Int_t *pfof, Int_t **pglist, Int_t *value, Int_t *gdata)
    }
    // reset order
    qsort(Part, nbodies, sizeof(Particle), PIDCompare);
    for (Int_t i = 0; i < nbodies; i++)
    {
        Part[i].SetPID(storeval1[i]);
        Part[i].SetID(storeval2[i]);
        Part[i].SetDensity(storeden[i]);
    }
    // and alter pglist
    ...
    if (ireorder == 1 && iflag && ngroup > 0)
    {
        if (groupflag != NULL)
        {
            ...
            for (Int_t i = 0; i <= ng; i++)
            {
                ...
                {
                    if (groupflag[i] == 1)
                        groupvalue[i] = numingroup[i];
                    else
                        groupvalue[i] = 1.0 / (Double_t)numingroup[i];
                }
            }
            // and reorder it if reordering group ids
    ...
    if (groupflag != NULL)
        iflag = Unbind(opt, gPart, ngroup, numingroup, pfof, pglist, 0);
    else
    ...
    for (Int_t i = 1; i <= ng; i++) delete[] gPart[i];
    delete[] gPart;
#endif
    if (pglistflag)
    {
    ...
    if (opt.iverbose)
        cout << ThisTask << " Done. Number of groups remaining " << ngroup << " in" << MyGetTime() - time1 << endl;

    return iflag;
}

/*!
    Unbinding algorithm that checks to see if a group is self-bound. For small groups the potential is calculated using
   a PP algorithm, for large groups a tree-potential using kd-tree and monopole is calculated. \n There are several ways
   a group can be defined as bound, it total energy is negative, its least bound particle has negative energy, or both.
   Also one can have different kinetic reference frames. By default, uses the CM velocity of the total system BUT could
   also use the velocity of a region centred on the minimum potential well. \n

    If a group is not self bound, the least bound particle is removed from the group. Then
    \arg if CM reference frame, the centre-of-mass velocity is recalculated and kinetic energies are recalculated (if it
   has changed enough). \n \arg the potential can be left unchanged or if one ignores the background unbound but linked
   particles, the energies are adjusted for the loss of the particle from the group. NOTE that for groups where the
   tree-potential was calculated, at the moment, the subtracted energy corresponds to the PP calculation which can lead
   to decrepancies. However, unless one is worried about the exact details of when an object is self-bound, this is not
   an issue. \n

    Finally, this routines assumes that the pglist passed to the routine is for a gPart array that was build in id order
   from pfof and a local particle array.
*/
int Unbind(Options &opt, Particle **gPart, Int_t &numgroups, Int_t *numingroup, Int_t *pfof, Int_t **pglist,
           int ireorder)
{
    ...
    // recalculate the entire potential using a Tree code than it is removing the contribution of each removed particle
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, n, r2, poti)
        {
                ...
                if (numingroup[i] <= UNBINDNUM)
                {
                    ...
                    for (j = 0; j < numingroup[i]; j++)
                    {
                        for (k = j + 1; k < numingroup[i]; k++)
                        {
                            r2 = 0.;
                            for (n = 0; n < 3; n++)
                                r2 += pow(gPart[i][j].GetPosition(n) - gPart[i][k].GetPosition(n), 2.0);
                            r2 += eps2;
                            r2           = 1.0 / sqrt(r2);
                            Double_t pot = -opt.G * (gPart[i][j].GetMass() * gPart[i][k].GetMass()) * r2;
                            poti         = gPart[i][j].GetPotential() + pot;
                            gPart[i][j].SetPotential(poti);
                            poti = gPart[i][k].GetPotential() + pot;
                            gPart[i][k].SetPotential(poti);
                        }
                    }
#ifdef NOMASS
...
#pragma omp master
        ...
        {
            if (numingroup[i] < 0) continue;
            if (numingroup[i] > UNBINDNUM) { Potential(opt, numingroup[i], gPart[i]); }
        }
...
#pragma omp for schedule(dynamic, 1) nowait
    ...
    {
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, npot, menc, potpos, storeval)
        {
#pragma omp for schedule(dynamic, 1) nowait
#endif
            for (i = 1; i <= numgroups; i++)
    ...
    {
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, npot, menc, potmin, ipotmin, potpos, storeval)
        {
#pragma omp for schedule(dynamic, 1) nowait
#endif
            for (i = 1; i <= numgroups; i++)
    ...
    for (i = 1; i <= numgroups; i++)
        if (numingroup[i] >= ompunbindnum)
        {
            ...
            {
                // determine if any particle  number of particle with positive energy upto
                // opt.uinfo.maxunbindfrac*numingroup+1
                ...
                FillUnboundArrays(opt, maxunbindsize, numingroup[i], gPart[i], Efrac, nEplusid, Eplusflag, nEplus,
                                  unbindcheck);
                while (unbindcheck)
                {
                    iunbindflag += 1;
                    unbindloops++;
                    UpdateCMForUnboundParticles(opt, gmass[i], cmvel[i], numingroup[i], gPart[i], nEplus, nEplusid,
                                                Eplusflag);
                    ...
                    // Here we set the limit at 2*log(numingroup[i]) to account for overhead in producing tree and
                    // calculating new potential
                    iunbindsizeflag = (nEplus < 2.0 * log((double)numingroup[i]));
                    if (opt.uinfo.bgpot == 0)
                    {
                        if (iunbindsizeflag == 0)
                        ...
                        else
                        {
                            ...
                            {
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(j, r2, poti)
                                {
#pragma omp for schedule(dynamic) nowait
                                    ...
                                    for (j = 0; j < numingroup[i]; j++)
                                    {
                                        if (j != nEplusid[k])
                                        {
                                            r2 = 0.;
                                            for (n = 0; n < 3; n++)
                                                r2 += pow(gPart[i][nEplusid[k]].GetPosition(n) -
                                                              gPart[i][j].GetPosition(n),
                                                          2.0);
                                            r2 += eps2;
                                            r2 = 1.0 / sqrt(r2);
                                            poti =
                                                gPart[i][j].GetPotential() +
                                                opt.G * (gPart[i][nEplusid[k]].GetMass() * gPart[i][j].GetMass()) * r2;
#ifdef NOMASS
                                            poti *= mw2;
#endif
                                            gPart[i][j].SetPotential(poti);
                    ...
                    else
                    {
                        ...
                        maxunbindsize = (Int_t)(opt.uinfo.maxunbindfrac * nunbound + 1);
                        unbindcheck   = CheckGroupForBoundness(opt, Efrac, maxE, numingroup[i]);
                        FillUnboundArrays(opt, maxunbindsize, numingroup[i], gPart[i], Efrac, nEplusid, Eplusflag,
                                          nEplus, unbindcheck);
                    }
                }
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, n, maxE, maxunbindsize, nEplus, nEplusid, Eplusflag, v2, Ti,  \
                                             unbindcheck, Efrac, nEfrac, nunbound, r2, poti, unbindloops, sortflag, \
                                             oldnumingroup)
    {
#pragma omp for schedule(dynamic) nowait reduction(+ : iunbindflag)
#endif
        for (i = 1; i <= numgroups; i++)
            if (numingroup[i] <= ompunbindnum && numingroup[i] > 0)
            {
                maxE        = -MAXVALUE;
                nEplus      = 0;
                Efrac       = 0.;
                ...
                else
                {
                    ...
                    while (unbindcheck)
                    {
                            ...
                            for (k = 0; k < nEplus; k++)
                            {
                                for (j = 0; j < numingroup[i]; j++)
                                {
                                    if (j != nEplusid[k])
                                    {
                                        r2 = 0.;
                                        for (n = 0; n < 3; n++)
                                            r2 += pow(gPart[i][nEplusid[k]].GetPosition(n) - gPart[i][j].GetPosition(n),
                                                      2.0);
                                        r2 += eps2;
                                        r2 = 1.0 / sqrt(r2);
#ifdef NOMASS
                                        poti = gPart[i][j].GetPotential() +
                                               opt.G * (gPart[i][nEplusid[k]].GetMass() * gPart[i][j].GetMass()) * r2 *
                                                   mv2;
#else
                                    poti = gPart[i][j].GetPotential() +
                        ...
                        RemoveUnboundParticles(i, pfof, numingroup[i], pglist[i], gPart[i], nEplus, nEplusid,
                                               Eplusflag);
                        // if number of particles remove with positive energy is near to the number allowed to be
                        // removed must recalculate kinetic energies and check if maxE>0 otherwise, end unbinding.
                        if (nEplus < opt.uinfo.maxallowedunboundfrac * numingroup[i]) { unbindcheck = false; }
                        else
                        {
                            sortflag = false;
                            if ((oldnumingroup - numingroup[i]) > opt.uinfo.maxallowedunboundfrac * oldnumingroup)
...
int Unbind(Options &opt, Particle *&gPart, Int_t &numgroups, Int_t *&numingroup, Int_t *&noffset, Int_t *&pfof)
{
    ...
    Double_t maxE, totT, v2, r2, poti, Ti, eps2 = opt.uinfo.eps * opt.uinfo.eps, mv2 = opt.MassValue * opt.MassValue,
                                           Efrac;
    Double_t *     gmass, *totV;
    PriorityQueue *pq;
    ...
    for (i = 1; i <= numgroups; i++)
    {
        ...
        {
            for (j = 0; j < numingroup[i]; j++)
                gPart[noffset[i] + j].SetPotential(gPart[noffset[i] + j].GetGravityPotential());
        }
#endif
    }
                ...
                if (numingroup[i] <= UNBINDNUM)
                {
                    for (j = 0; j < numingroup[i]; j++)
                    {
                        for (k = j + 1; k < numingroup[i]; k++)
                        {
                            r2 = 0.;
                            for (n = 0; n < 3; n++)
                                r2 += pow(gPart[noffset[i] + j].GetPosition(n) - gPart[noffset[i] + k].GetPosition(n),
                                          2.0);
                            r2 += eps2;
                            r2 = 1.0 / sqrt(r2);
                            Double_t pot =
                                -opt.G * (gPart[noffset[i] + j].GetMass() * gPart[noffset[i] + k].GetMass()) * r2;
                            poti = gPart[noffset[i] + j].GetPotential() + pot;
                            gPart[noffset[i] + j].SetPotential(poti);
                            poti = gPart[noffset[i] + k].GetPotential() + pot;
                            gPart[noffset[i] + k].SetPotential(poti);
                        }
                    }
#ifdef NOMASS
                    for (j = 0; j < numingroup[i]; j++)
                        gPart[noffset[i] + j].SetPotential(gPart[noffset[i] + j].GetPotential() * mv2);
#endif
...
#pragma omp master
        ...
        for (i = 1; i <= numgroups; i++)
        {
            if (numingroup[i] > UNBINDNUM)
            {
                ...
                {
                    marktreecell[j] = new Int_t[ncell];
                    markleafcell[j] = new Int_t[ncell];
                }
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(j, k, n)
                {
#pragma omp for schedule(dynamic, 1) nowait
#endif
                    for (j = 0; j < ncell; j++)
                    {
                        ...
                        {
                            for (n = 0; n < 3; n++)
                                cellcm[j][n] += gPart[noffset[i] + k].GetPosition(n) * gPart[noffset[i] + k].GetMass();
                ...
                // for marked cells calculate pp, for every other cell just use the CM of the cell to calculate the
                // potential.
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(j, k, l, n, ntreecell, nleafcell, r2, poti)
                {
#pragma omp for schedule(dynamic, 1) nowait
#endif
                    for (j = 0; j < numingroup[i]; j++)
                    ...
                    tid = 0;
                        ...
                        MarkCell(npomp[tid], marktreecell[tid], markleafcell[tid], ntreecell, nleafcell, r2val[tid],
                                 opt.uinfo.BucketSize, cR2max, cellcm, cmtot, xpos, eps2);
                        poti = 0;
                        for (k = 0; k < ntreecell; k++) { poti += -gPart[noffset[i] + j].GetMass() * r2val[tid][k]; }
                        for (k = 0; k < nleafcell; k++)
                        {
                            for (l = start[markleafcell[tid][k]]; l < end[markleafcell[tid][k]]; l++)
                            {
                                if (j != l)
                                {
                                    r2 = 0.;
                                    for (n = 0; n < 3; n++)
                                        r2 += pow(gPart[noffset[i] + j].GetPosition(n) -
                                                      gPart[noffset[i] + l].GetPosition(n),
                                                  (Double_t)2.0);
                                    r2 += eps2;
                                    r2 = 1.0 / sqrt(r2);
                ...
                for (j = 0; j < nthreads; j++)
                {
                    delete[] marktreecell[j];
                    delete[] markleafcell[j];
                    delete[] r2val[j];
                }
            }
        }
...
#pragma omp for schedule(dynamic, 1) nowait
    ...
    for (i = 1; i <= numgroups; i++)
        if (numingroup[i] >= ompunbindnum)
        {
            ...
            Efrac = 0;
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(j, k, v2, Ti, unbindcheck)
            {
#pragma omp for reduction(+ : totT, Efrac)
                ...
                {
                    v2 = 0.0;
                    for (k = 0; k < 3; k++) v2 += pow(gPart[noffset[i] + j].GetVelocity(k) - cmvel[i][k], 2.0);
#ifdef NOMASS
            ...
            // if largest energy is positive load largest energies so as to remove at most pqsize particles (roughly 1%)
            // per removal loop
            if (opt.uinfo.unbindtype == USYSANDPART)
            {
                if (((Efrac < opt.uinfo.minEfrac) || (maxE > 0)) && (numingroup[i] >= opt.MinSize))
                    unbindcheck = true;
                else
                    unbindcheck = false;
            }
            ...
            {
                if ((maxE > 0) && (numingroup[i] >= opt.MinSize))
                    unbindcheck = true;
                else
                    unbindcheck = false;
            }
            if (unbindcheck)
            {
                ...
                for (j = pqsize; j < numingroup[i]; j++)
                    if (gPart[noffset[i] + j].GetDensity() > gPart[noffset[i] + pq->TopQueue()].GetDensity())
                    {
                        pq->Pop();
                        pq->Push(j, gPart[noffset[i] + j].GetDensity());
                    }
                nEplus = 0;
                // if just looking at particle then add to removal list till energy >0
                if (opt.uinfo.unbindtype == UPART)
                {
                    for (j = 0; j < pqsize; j++)
                    {
                        if (gPart[noffset[i] + pq->TopQueue()].GetDensity() > 0)
                        {
                ...
                else if (opt.uinfo.unbindtype == USYSANDPART)
                {
                    ...
                    for (j = 0; j < pqsize; j++)
                    {
                        if (gPart[noffset[i] + pq->TopQueue()].GetDensity() > 0 || nEplus < nEfrac)
                        {
                            nEplusid[nEplus++]        = pq->TopQueue();
                            Eplusflag[pq->TopQueue()] = 1;
                            pq->Pop();
            ...
            while (unbindcheck)
            {
                ...
                if (opt.uinfo.cmvelreftype == CMVELREF)
                {
                    ...
                    {
                        for (k = 0; k < 3; k++)
                            cmvel[i][k] -= gPart[noffset[i] + nEplusid[j]].GetVelocity(k) *
                                           gPart[noffset[i] + nEplusid[j]].GetMass() * temp;
...
#pragma omp for schedule(dynamic) nowait
                                ...
                                for (j = 0; j < numingroup[i]; j++)
                                {
                                    if (j != nEplusid[k])
                                    {
                                        r2 = 0.;
                                        for (n = 0; n < 3; n++)
                                            r2 += pow(gPart[noffset[i] + nEplusid[k]].GetPosition(n) -
                                                          gPart[noffset[i] + j].GetPosition(n),
                                                      2.0);
                                        r2 += eps2;
                                        r2 = 1.0 / sqrt(r2);
#ifdef NOMASS
                                        poti = gPart[noffset[i] + j].GetPotential() +
                                               opt.G *
                                                   (gPart[noffset[i] + nEplusid[k]].GetMass() *
                                                    gPart[noffset[i] + j].GetMass()) *
                                                   r2 * mv2;
#else
                                    poti = gPart[noffset[i] + j].GetPotential() +
                                           opt.G *
                ...
                {
                    if (opt.uinfo.bgpot == 0)
                        for (k = 0; k < nEplus; k++) totV[i] -= 0.5 * gPart[noffset[i] + nEplusid[k]].GetPotential();
                }
                // remove particles with positive energy
                for (j = 0; j < nEplus; j++) pfof[gPart[noffset[i] + nEplusid[j]].GetPID()] = 0;
                k = numingroup[i] - 1;
                for (j = 0; j < nEplus; j++)
                    if (nEplusid[j] < numingroup[i] - nEplus)
                    {
...
#pragma omp for reduction(+ : totT, Efrac)
...
#endif
                            totT += Ti;
                            gPart[noffset[i] + j].SetDensity(opt.uinfo.Eratio * Ti +
                                                             gPart[noffset[i] + j].GetPotential());
                            Efrac += (Ti + gPart[noffset[i] + j].GetPotential() < 0);
                        }
#ifdef USEOPENMP
                    }
                    ...
                    if (unbindcheck)
                    {
                        ...
                        // otherwise, remove all positive energies and also if Efrac< minEfrac, keep adding to removal
                        // list
                        else if (opt.uinfo.unbindtype == USYSANDPART)
                        {
                            nEfrac = 0;
        ...
        // now for small groups loop over groups
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, maxE, pq, pqsize, nEplus, nEplusid, Eplusflag, totT, v2, Ti, \
                                             unbindcheck, Efrac, Ptemp, nEfrac)
    {
#pragma omp for schedule(dynamic) nowait reduction(+ : iunbindflag)
#endif
        for (i = 1; i <= numgroups; i++)
                ...
                while (unbindcheck)
                {
                        ...
                        for (k = 0; k < nEplus; k++)
                        {
                            ...
                            for (j = 0; j < numingroup[i]; j++)
                            {
                                ...
                                poti =
                                    gPart[noffset[i] + j].GetPotential() +
                                    opt.G *
                                        (gPart[noffset[i] + nEplusid[k]].GetMass() * gPart[noffset[i] + j].GetMass()) *
                                        r2;
#endif
                                    gPart[noffset[i] + j].SetPotential(poti);
                                }
                    ...
                    if (nEplus >= 0.1 * pqsize + 0.5)
                    {
                        ...
                        if (unbindcheck)
                        {
                            ...
                            for (j = 0; j < pqsize; j++) pq->Push(j, gPart[noffset[i] + j].GetDensity());
                            for (j = pqsize; j < numingroup[i]; j++)
                                if (gPart[noffset[i] + j].GetDensity() >
                                    gPart[noffset[i] + pq->TopQueue()].GetDensity())
                                {
                                    pq->Pop();
                                    pq->Push(j, gPart[noffset[i] + j].GetDensity());
                                }
                            ...
                            // otherwise, remove all positive energies and also if Efrac< minEfrac, keep adding to
                            // removal list
                            else if (opt.uinfo.unbindtype == USYSANDPART)
                            {
                                nEfrac = 0;
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(j, k, l, n, ntreecell, nleafcell, r2)
    {
#pragma omp for schedule(dynamic, 1) nowait
#endif
        for (j = 0; j < nbodies; j++)
        ...
        tid = 0;
            ...
            MarkCell(npomp[tid], marktreecell[tid], markleafcell[tid], ntreecell, nleafcell, r2val[tid],
                     opt.uinfo.BucketSize, cR2max, cellcm, cmtot, xpos, eps2);
            for (k = 0; k < ntreecell; k++) { potV[Part[j].GetID()] += -Part[j].GetMass() * r2val[tid][k]; }
            for (k = 0; k < nleafcell; k++)
            {
                for (l = start[markleafcell[tid][k]]; l < end[markleafcell[tid][k]]; l++)
                {
                    if (j != l)
                    {
                        r2 = 0.;
                        for (n = 0; n < 3; n++)
                            r2 += pow(Part[j].GetPosition(n) - Part[l].GetPosition(n), (Double_t)2.0);
                        r2 += eps2;
                        r2 = 1.0 / sqrt(r2);
