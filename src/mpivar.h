...
/// Array which is [NProcs*NProcs] that stores number of particles to be exported in FOF search from processor i to j
/// via [i+NProcs*j]
...
///\todo must implement this array to be used in output produced by \ref WritePGListIndex. This index based output can
///be useful.
...
/// array that is used to indicate particle must be sent across an mpi_domain, stores the mpi thread num particle is to
/// be sent to \todo issue is that particles can be sent to more than one mpi thread so have to store Nlocal*NProcs which
///can be too large since the idea is to use GETNUMEXPORT to reduce mem costs
...
/// Particle arrays that allow allocation of memory need when deallocating local particle arrays that then need to be
/// reassigned
