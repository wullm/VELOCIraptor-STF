...
struct gadget_header
{
    ...
    char    fill[256 - 6 * 4 - 6 * 8 - 2 * 8 - 2 * 4 - 6 * 4 - 2 * 4 - 4 * 8 - 2 * 4 - 6 * 4 -
              1 * 4]; /* fills to 256 Bytes, here calculation is based on 6 gadget particle types with default header
                            stuff*/
...
inline Int_t get_nbodies(char *fname, int ptype = -1)
{
...
#ifdef GADGET2FORMAT
    fread(&dummy, sizeof(dummy), 1, fd);
    fread(&DATA[0], sizeof(char) * 4, 1, fd);
    DATA[4] = '\0';
    fread(&dummy, sizeof(dummy), 1, fd);
    fread(&dummy, sizeof(dummy), 1, fd);
#endif
    ...
    for (j = 0, nbodies = 0; j < NGTYPE; j++)
        if (header1.npartTotalHW[j] != 0) ihwflag = 1;
    if (ptype == -1)
    {
        for (j = 0, nbodies = 0; j < NGTYPE; j++)
        {
