...
/*! Calculates the local velocity density function for each particle using a kernel technique
    ...
    2) Or calculate phase space density and physical density and divide phase-space density by physical density. This is
   far more computationally expensive and may not enhance velocity clustering and is just present for testing purposes.
    \todo velocity density function is NOT mass weighted. Might want to alter this.
    \todo there is a seg fault memory error when searching for NN in large sims using \em SINGLEPRECISION flag. I don't
   know why.
*/
void GetVelocityDensity(Options &opt, const Int_t nbodies, Particle *Part, KDTree *tree)
{
    ...
    {
        cout << ThisTask << " "
             << "Using the following parameters to calculate velocity density using sph kernel: ";
        cout << ThisTask << " "
             << "(Nse,Nv)=" << opt.Nsearch << "," << opt.Nvel << endl;
        cout << ThisTask << " "
             << "Get velocity density using a subset of nearby physical or phase-space neighbours" << endl;
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, tid, id, v2, nnids, nnr2, nnidsneighbours, nnr2neighbours, \
                                             weight, pqx, pqv)
    {
#endif
...
#pragma omp for schedule(dynamic)
        ...
        for (i = 0; i < nbodies; i++)
        {
            ...
            if (Part[i].GetType() > 0)
            {
                ...
                // otherwise distinction must be made so that only base calculation on dark matter particles
                    ...
                    tree->FindNearestCriterion(i, FOFPositivetypes, NULL, nnids, nnr2, opt.Nsearch);
#else
        tree->FindNearest(i, nnids, nnr2, opt.Nsearch);
#endif
                // once NN set is found, store maxrdist and see if particle's search radius overlaps with another mpi
                // domain
        ...
        if (MPISearchForOverlap(Part[i], maxrdist[i]) == 0)
        {
                    ...
                    for (j = 0; j < opt.Nsearch; j++)
                    {
                        ...
                        id = nnids[j];
                        for (k = 0; k < 3; k++)
                            v2 += (Part[i].GetVelocity(k) - Part[id].GetVelocity(k)) *
                                  (Part[i].GetVelocity(k) - Part[id].GetVelocity(k));
    ...
    nimport =
        MPIBuildParticleNNImportList(nbodies, tree, Part, (!(opt.iBaryonSearch >= 1 && opt.partsearchtype == PSTALL)));
    ...
    if (nimport > 0)
        ...
        // then run search
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, tid, pid, pid2, v2, nnids, nnr2, nnidsneighbours, \
                                             nnr2neighbours, weight, pqx, pqv)
    {
#endif
        nnids           = new Int_t[opt.Nsearch];
        nnr2            = new Double_t[opt.Nsearch];
...
#pragma omp for
        ...
        tid = 0;
                ...
                if (Part[i].GetDensity() == -1)
                {
                    ...
                    // if not searching all particles in FOF then also doing baryon search then just find nearest
                    // neighbours
                    if (!(opt.iBaryonSearch >= 1 && opt.partsearchtype == PSTALL))
                        tree->FindNearest(i, nnids, nnr2, opt.Nsearch);
                    // otherwise distinction must be made so that only base calculation on dark matter particles
                    ...
                    for (j = 0; j < opt.Nsearch; j++)
                    {
                        ...
                        if (pqx->TopQueue() < nbodies)
                        {
                            pid2 = pqx->TopQueue();
                            for (k = 0; k < 3; k++)
                                v2 += (Part[i].GetVelocity(k) - Part[pid2].GetVelocity(k)) *
                                      (Part[i].GetVelocity(k) - Part[pid2].GetVelocity(k));
                        }
                        else
                        {
                            pid2 = pqx->TopQueue() - nbodies;
                            for (k = 0; k < 3; k++)
                                v2 += (Part[i].GetVelocity(k) - PartDataGet[pid2].GetVelocity(k)) *
                                      (Part[i].GetVelocity(k) - PartDataGet[pid2].GetVelocity(k));
                        }
                        if (v2 < pqv->TopPriority())
                        {
                            pqv->Pop();
...
#pragma omp for schedule(dynamic, minamount) nowait
        for (i = 0; i < nbodies; i++)
        {
            ...
            if (Part[i].GetType() > 0)
            {
                ...
                // if not searching all particles in FOF then also doing baryon search then just find nearest neighbours
                if (!(opt.iBaryonSearch >= 1 && opt.partsearchtype == PSTALL))
                    Part[i].SetDensity(tree->CalcVelDensityParticle(i, opt.Nvel, opt.Nsearch, 1, pqx[tid], pqv[tid],
                                                                    &nnids[tid * opt.Nsearch],
                                                                    &nnr2[tid * opt.Nsearch]));
                // otherwise distinction must be made so that only base calculation on dark matter particles
                else
                {
                    tree->FindNearestCriterion(i, FOFPositivetypes, NULL, &nnids[tid * opt.Nsearch],
                                               &nnr2[tid * opt.Nsearch], opt.Nsearch);
                    for (j = 0; j < opt.Nvel; j++)
                    {
                ...
                if (opt.iverbose)
                    ...
                    {
                        printf("Task %d done %e of its share \n", tid,
                               fracdone[tid] / ((double)nbodies / (double)nthreads));
...
#endif
            Part[i].SetDensity(tree->CalcVelDensityParticle(i, opt.Nvel, opt.Nsearch, 1, pqx[tid], pqv[tid],
                                                            &nnids[tid * opt.Nsearch], &nnr2[tid * opt.Nsearch]));
            fracdone[tid]++;
            if (opt.iverbose)
                if (fracdone[tid] > fraclim[tid])
                {
...
void GetVelocityDensityExact(Options &opt, const Int_t nbodies, Particle *Part, KDTree *tree)
{
    ...
    if (opt.iverbose)
        cout << ThisTask
             << " Calculating the local velocity density by finding EXACT nearest physical neighbours to particles"
             << endl;
    Int_t     i, j, k;
    int       nthreads;
    int       tid, id, pid, pid2, itreeflag = 0;
...
#pragma omp parallel
    ...
    // If so, do not calculate local velocity density and set its velocity density to -1 as a flag
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, tid, id, v2, nnids, nnr2, weight, pqv)
    {
#endif
        nnids  = new Int_t[opt.Nsearch];
        nnr2   = new Double_t[opt.Nsearch];
...
#pragma omp for schedule(dynamic)
    ...
    if (NProcs > 1 && opt.iLocalVelDenApproxCalcFlag == 0)
    {
        ...
        nimport           = MPIBuildParticleNNImportList(nbodies, tree, Part,
                                               (!(opt.iBaryonSearch >= 1 && opt.partsearchtype == PSTALL)));
        int nimportsearch = opt.Nsearch;
        if (nimportsearch > nimport) nimportsearch = nimport;
        if (opt.iverbose) cout << ThisTask << " Searching particles in other domains " << nimport << endl;
        // now with imported particle list and local particle list can run proper NN search
...
void GetVelocityDensityApproximative(Options &opt, const Int_t nbodies, Particle *Part, KDTree *tree)
{
    ...
    if (opt.iverbose)
        cout << ThisTask
             << " Calculating the local velocity density by finding APPROXIMATIVE nearest physical neighbour search "
                "for each particle "
             << endl;
    int      nthreads;
    int      tid, id, pid, pid2, itreeflag = 0;
...
#ifdef USEMPI
            leafnodes[i].searchdist =
                leafnodes[i].size * pow(opt.Nsearch / (float)leafnodes[i].numtot, 1.0 / 3.0) * 1.2;
#endif
        }
#ifdef USEOPENMP
    }
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(id, v2, nnids, nnr2, weight, pqv)
    {
#endif
        nnids  = new Int_t[opt.Nsearch];
        nnr2   = new Double_t[opt.Nsearch];
        ...
        pqv    = new PriorityQueue(opt.Nvel);
#ifdef USEOPENMP
#pragma omp for schedule(dynamic) reduction(+ : nprocessed, ntot)
#endif
        for (auto i = 0; i < numleafnodes; i++)
        {
            ...
            // otherwise distinction must be made so that only base calculation on dark matter particles
                ...
                tree->FindNearestCheck(leafnodes[i].cm, FOFcheckpositivetype, NULL, nnids, nnr2, opt.Nsearch);
#else
        tree->FindNearestPos(leafnodes[i].cm, nnids, nnr2, opt.Nsearch);
#endif
#ifdef USEMPI
...
#ifdef SWIFTINTERFACE
                if (MPISearchForOverlapUsingMesh(libvelociraptorOpt, leafnodes[i].cm, leafnodes[i].searchdist) != 0)
                    continue;
#else
            ...
            for (auto j = leafnodes[i].istart; j < leafnodes[i].iend; j++)
            {
                ...
                for (auto k = 0; k < opt.Nsearch; k++)
                {
                    ...
                    if (id == j) continue;
                    for (auto n = 0; n < 3; n++)
                        v2 += (Part[j].GetVelocity(n) - Part[id].GetVelocity(n)) *
                              (Part[j].GetVelocity(n) - Part[id].GetVelocity(n));
                    if (v2 < pqv->TopPriority())
                    {
                        pqv->Pop();
                        pqv->Push(id, v2);
...
#ifdef USEMPI
    // if search is fully approximative, then since particles have been localized to mpi domains in FOF groups, don't
    // search neighbour mpi domains
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(id, v2, nnids, nnr2, nnidsneighbours, nnr2neighbours, weight, pqx, pqv, \
                                             Pval, pid2)
            {
#endif
                ...
                pqv             = new PriorityQueue(opt.Nvel);
#ifdef USEOPENMP
#pragma omp for schedule(dynamic) reduction(+ : nprocessed)
#endif
                for (auto i = 0; i < numleafnodes; i++)
                {
                    // if there are no active particles in leaf node, do nothing
                ...
                tree->FindNearestPos(leafnodes[i].cm, nnids, nnr2, opt.Nsearch);
                    ...
                    for (auto j = leafnodes[i].istart; j < leafnodes[i].iend; j++)
                    {
                        ...
                        for (auto k = 0; k < opt.Nsearch; k++)
                        {
                            ...
                            v2 = 0;
                            for (auto n = 0; n < 3; n++)
                                v2 += (Part[j].GetVelocity(n) - Pval->GetVelocity(n)) *
                                      (Part[j].GetVelocity(n) - Pval->GetVelocity(n));
                            if (v2 < pqv->TopPriority())
                            {
                                pqv->Pop();
                                pqv->Push(nnids[k], v2);
