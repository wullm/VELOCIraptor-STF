...
///\name to access where extra baryonic properties are located in the HDF_Part_Info structure that code will use to
///calcualte object properties
//@{
...
template <typename ReturnT, typename F, typename... Ts> ReturnT safe_hdf5(F function, Ts... args)
{
...
static inline void get_attribute(vector<hid_t> &ids, const std::string attr_name)
{
    ...
    if (exists == 0) { throw invalid_argument(std::string("attribute not found ") + attr_name); }
    else if (exists < 0)
    {
...
static inline void get_attribute(vector<hid_t> &ids, const std::vector<std::string> &parts)
{
    ...
    if (parts.size() == 1) { get_attribute(ids, parts[0]); }
    else
    {
        H5O_info_t object_info;
        hid_t      newid;
        H5Oget_info_by_name(ids.back(), parts[0].c_str(), &object_info, H5P_DEFAULT);
        if (object_info.type == H5O_TYPE_GROUP) { newid = H5Gopen2(ids.back(), parts[0].c_str(), H5P_DEFAULT); }
...
static inline void close_hdf_ids(vector<hid_t> &ids)
{
    ...
    for (auto &id : ids)
    {
        ...
        if (object_info.type == H5O_TYPE_GROUP) { H5Gclose(id); }
...
static inline hid_t HDF5OpenGroup(const hid_t &file, string name) { return H5Gopen2(file, name.c_str(), H5P_DEFAULT); }
...
static inline void HDF5ReadHyperSlabReal(double *buffer, const hid_t &dataset, const hid_t &dataspace,
...
static inline void HDF5ReadHyperSlabInteger(long long *buffer, const hid_t &dataset, const hid_t &dataspace,
                                            const hsize_t datarank, const hsize_t ndim, int nchunk, int noffset)
{
    // setup hyperslab so that it is loaded into the buffer
    vector<hsize_t> start, count, stride, block, memdims;
...
class H5OutputFile
{
  ...
  public:
    ...
    template <typename T>
    void write_dataset(std::string name, hsize_t len, T *data, hid_t memtype_id = -1, hid_t filetype_id = -1)
    {
        int     rank    = 1;
        hsize_t dims[1] = {len};
    ...
    void write_dataset(string name, hsize_t len, string data)
    {
        ...
        // Create the dataset
        dset_id = H5Dcreate(file_id, name.c_str(), filetype_id, dspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
        // Write the data
    ...
    void write_dataset(string name, hsize_t len, void *data, hid_t memtype_id = -1, hid_t filetype_id = -1)
    {
        int     rank    = 1;
        hsize_t dims[1] = {len};
        if (memtype_id == -1)
    ...
    /// the input data if not explicitly specified with the filetype_id parameter.
    template <typename T>
    void write_dataset_nd(std::string name, int rank, hsize_t *dims, T *data, hid_t memtype_id = -1,
                          hid_t filetype_id = -1)
    {
        ...
        // Create the dataset
        hid_t dset_id = H5Dcreate(file_id, name.c_str(), filetype_id, dspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    ...
    void write_dataset_nd(std::string name, int rank, hsize_t *dims, void *data, hid_t memtype_id = -1,
                          hid_t filetype_id = -1)
    {
        // Get HDF5 data type of the array in memory
        if (memtype_id == -1)
...
/// This structures stores the strings defining the groups of data in the hdf input. NOTE: HERE I show the strings for
/// Illustris format
    ...
    HDF_Part_Info(int PTYPE, int hdfnametype = HDFEAGLENAMES)
    {
        ...
        if (ptype == HDFGASTYPE)
        {
            ...
            if (hdfnametype == HDFEAGLENAMES)
                names[itemp++] = string("Velocity");
            else
                names[itemp++] = string("Velocities");
            names[itemp++] = string("ParticleIDs");
            if (hdfnametype == HDFEAGLENAMES)
                names[itemp++] = string("Mass");
            else
        ...
        if (ptype == HDFDM1TYPE || ptype == HDFDM2TYPE)
        {
            ...
            names[itemp++] = string("ParticleIDs");
            names[itemp++] = string("Masses");
            if (hdfnametype == HDFSIMBANAMES || hdfnametype == HDFMUFASANAMES) { names[itemp++] = string("Potential"); }
        }
...
inline void HDFSetUsedParticleTypes(Options &opt, int &nusetypes, int &nbusetypes, int usetypes[])
{
    ...
    if (opt.partsearchtype == PSTALL)
    {
        ...
        if (opt.iuseextradarkparticles)
        {
            ...
            if (opt.ihdfnameconvention != HDFSWIFTEAGLENAMES) { usetypes[nusetypes++] = HDFDM2TYPE; }
        }
    ...
    else if (opt.partsearchtype == PSTDARK)
    {
        ...
        {
            nbusetypes                         = 1;
            usetypes[nusetypes + nbusetypes++] = HDFGASTYPE;
...
inline Int_t HDF_get_nbodies(char *fname, int ptype, Options &opt)
{
    ...
    // try
    {
        ...
        if (opt.ihdfnameconvention == HDFSWIFTEAGLENAMES)
        {
            ...
            {
                hdf_header_info.iscosmological =
                    read_attribute<int>(Fhdf, hdf_header_info.names[hdf_header_info.IIsCosmological]);

                if (!hdf_header_info.iscosmological && opt.icosmologicalin)
                {
                    cout << "Error: cosmology is turned on in the config file but the snaphot provided is a "
                ...
                {
                    cout << "Error: cosmology is turned off in the config file but the snaphot provided is a "
                            "cosmological run."
                         << endl;
#ifdef USEMPI
                    MPI_Abort(MPI_COMM_WORLD, 8);
            ...
            {
                cout << "SWIFT EAGLE HDF5 naming convention chosen in config file but the snapshot was not produced by "
                        "SWIFT. The string read was: "
                     << stringbuff << endl;
#ifdef USEMPI
                MPI_Abort(MPI_COMM_WORLD, 8);
#else
        ...
        if (opt.partsearchtype == PSTALL)
        {
            if (opt.iusestarparticles && hdf_header_info.npartTotalHW[HDFSTARTYPE] == 0 &&
                hdf_header_info.npartTotal[HDFSTARTYPE] == 0)
            {
                cerr << "Warning: Configured to load star particles but none present" << endl;
                opt.iusestarparticles = 0;
            }
            if (opt.iusesinkparticles && hdf_header_info.npartTotalHW[HDFBHTYPE] == 0 &&
                hdf_header_info.npartTotal[HDFBHTYPE] == 0)
            {
                cerr << "Warning: Configured to load black hole particles but none present" << endl;
                opt.iusesinkparticles = 0;
            }
            if (opt.iusewindparticles && hdf_header_info.npartTotalHW[HDFWINDTYPE] == 0 &&
                hdf_header_info.npartTotal[HDFWINDTYPE] == 0)
            {
                cerr << "Warning: Configured to load wind particles but none present" << endl;
                opt.iusewindparticles = 0;
            }
            if (opt.iusetracerparticles && hdf_header_info.npartTotalHW[HDFTRACERTYPE] == 0 &&
                hdf_header_info.npartTotal[HDFTRACERTYPE] == 0)
            {
            ...
            if (opt.iuseextradarkparticles && hdf_header_info.npartTotalHW[HDFDM1TYPE] == 0 &&
                hdf_header_info.npartTotal[HDFDM1TYPE] == 0 && hdf_header_info.npartTotalHW[HDFDM2TYPE] == 0 &&
