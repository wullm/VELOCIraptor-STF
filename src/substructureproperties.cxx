 ...
 */
void GetCM(Options &opt, const Int_t nbodies, Particle *Part, Int_t ngroup, Int_t *&pfof, Int_t *&numingroup,
           PropData *&pdata, Int_t *&noffset)
{
#ifndef USEMPI
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, Pval, ri, rcmv, r2, cmx, cmy, cmz, EncMass, Ninside, cmold, \
                                             change, tol, x, y, z, vx, vy, vz, vc, rc)
    {
#pragma omp for schedule(dynamic) nowait
#endif
        for (i = 1; i <= ngroup; i++)
            if (numingroup[i] < omppropnum)
            {
                ...
                for (k = 0; k < 3; k++)
                {
                    pdata[i].gcm[k] *= (1.0 / pdata[i].gmass);
                    pdata[i].gcmvel[k] *= (1.0 / pdata[i].gmass);
                }
                ...
                for (j = 0; j < numingroup[i]; j++)
                {
                    ...
                    r2   = 0.0;
                    for (k = 0; k < 3; k++)
                        r2 += (pdata[i].gcm[k] - (*Pval).GetPosition(k)) * (pdata[i].gcm[k] - (*Pval).GetPosition(k));
                ...
                if (numingroup[i] * opt.pinfo.cmadjustfac >= PROPCMMINNUM && opt.iIterateCM)
                {
                    ...
                    pdata[i].gcmvel[0] = cmx;
                    pdata[i].gcmvel[1] = cmy;
                    pdata[i].gcmvel[2] = cmz;
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(j, Pval)
            {
...
#pragma omp for reduction(+ : cmx, cmy, cmz)
            ...
            while (opt.iIterateCM)
            {
                ...
                Ninside         = 0;
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(j, Pval, x, y, z)
                {
...
/*!
    ...
    The overall structure of the code is a bit lengthy simply to break up calculations appropriately for OMP style
   parallization. For small groups it is more efficient to parallize across groups, whereas for large groups containing
   many particles, we loop over the particles to sum quantities.

 */
void GetProperties(Options &opt, const Int_t nbodies, Particle *Part, Int_t ngroup, Int_t *&pfof, Int_t *&numingroup,
                   PropData *&pdata, Int_t *&noffset)
{
#ifndef USEMPI
    int ThisTask = 0, NProcs = 1;
    ...
    for (i = 1; i <= ngroup; i++)
    {
        ...
        if ((opt.iInclusiveHalo > 0 && opt.iInclusiveHalo < 3 && pdata[i].hostid != -1) || opt.iInclusiveHalo == 0 ||
            opt.iInclusiveHalo == 3)
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, Pval, cmref)
    {
#pragma omp for schedule(dynamic) nowait
#endif
        for (i = 1; i <= ngroup; i++)
...
#pragma omp parallel default(shared) private(                                                                        \
    i, j, k, Pval, ri, rcmv, r2, cmx, cmy, cmz, EncMass, Ninside, cmold, change,                                     \
    tol) private(x, y, z, vx, vy, vz, vc, rc, jval, jzval, Rdist, zdist, Ekin, Krot, mval, RV_Ekin, RV_Krot, RV_num, \
                 SFR) private(EncMassSF, EncMassNSF, Krot_sf, Krot_nsf, Ekin_sf, Ekin_nsf)
    {
#pragma omp for schedule(dynamic) nowait
#endif
        for (i = 1; i <= ngroup; i++)
            if (numingroup[i] < omppropnum)
            {
                ...
                {
                    // CalculateSphericalOverdensity(opt, pdata[i], numingroup[i], &Part[noffset[i]], m200val, m200mval,
                    // mBN98val, virval, m500val, SOlgrhovals);
                    CalculateSphericalOverdensitySubhalo(opt, pdata[i], numingroup[i], &Part[noffset[i]], m200val,
                                                         m200mval, mBN98val, virval, m500val, SOlgrhovals);
                ...
                if (opt.iextrahalooutput)
                {
                    ...
                    {
                        CalculateSphericalOverdensityExclusive(opt, pdata[i], numingroup[i], &Part[noffset[i]], m200val,
                                                               m200mval, mBN98val, virval, m500val, SOlgrhovals);
                ...
                EncMass += Pval->GetMass();
                    ...
                    // calculate vc
                    if (rc > 0)
                        if (EncMass > 0) vc = sqrt(opt.G * EncMass * opt.MassValue / rc);
                    ...
                    if (pdata[i].gRhalfmass > 0 && pdata[i].gMassTwiceRhalfmass == 0 && rc >= 0.5 * pdata[i].gRhalfmass)
                        pdata[i].gMassTwiceRhalfmass = EncMass;
                }
                ...
                {
                    pdata[i].glambda_B = pdata[i].gJ200c.Length() /
                                         (pdata[i].gM200c * sqrt(2.0 * opt.G * pdata[i].gM200c * pdata[i].gR200c));
                }
                ...
                {
                    pdata[i].glambda_B = pdata[i].gJ.Length() /
                                         (pdata[i].gM200c * sqrt(2.0 * opt.G * pdata[i].gM200c * pdata[i].gR200c));
                }
                ...
                for (j = 0; j < RV_num; j++)
                {
                    ...
                    pdata[i].RV_J =
                        pdata[i].RV_J + Coordinate(Pval->GetPosition()).Cross(Coordinate(vx, vy, vz)) * Pval->GetMass();
...
#endif
                pdata[i].RV_lambda_B = pdata[i].RV_J.Length() /
                                       (pdata[i].gMmaxvel * sqrt(2.0 * opt.G * pdata[i].gMmaxvel * pdata[i].gRmaxvel));
                for (j = 0; j < RV_num; j++)
                {
                    Pval  = &Part[j + noffset[i]];
                    vx    = (*Pval).Vx() - pdata[i].gcmvel[0];
                ...
                for (j = 0; j < numingroup[i]; j++)
                {
                    ...
                    if (Pval->GetType() == GASTYPE)
                    {
                        ...
                        if (SFR > opt.gas_sfr_threshold)
                            pdata[i].M_gas_sf += Pval->GetMass();
                        else
                            pdata[i].M_gas_nsf += Pval->GetMass();
#endif
                    }
                }
                Ekin = 0;
                ...
                if (pdata[i].n_gas * opt.pinfo.cmadjustfac >= PROPCMMINNUM && opt.iIterateCM)
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                            pdata[i].cm_gas[0] = cmx;
                            pdata[i].cm_gas[1] = cmy;
                            pdata[i].cm_gas[2] = cmz;
                    ...
                    pdata[i].cmvel_gas[0] = cmx;
                    pdata[i].cmvel_gas[1] = cmy;
                    pdata[i].cmvel_gas[2] = cmz;
                ...
                if (pdata[i].n_gas >= PROPMORPHMINNUM)
                    GetGlobalSpatialMorphology(numingroup[i], &Part[noffset[i]], pdata[i].q_gas, pdata[i].s_gas, 1e-2,
                                               pdata[i].eigvec_gas, 0, GASTYPE, 0);
#endif
#ifdef STARON
                for (j = 0; j < numingroup[i]; j++)
                {
                ...
                for (j = 0; j < numingroup[i]; j++)
                {
                    ...
                    if (Pval->GetType() == STARTYPE)
                    {
                        ...
                        pdata[i].L_star =
                            pdata[i].L_star + Coordinate(Pval->GetPosition()).Cross(Coordinate(vx, vy, vz)) * mval;
                ...
                if (pdata[i].n_star * opt.pinfo.cmadjustfac >= PROPCMMINNUM && opt.iIterateCM)
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                            pdata[i].cm_star[0] = cmx;
                            pdata[i].cm_star[1] = cmy;
                            pdata[i].cm_star[2] = cmz;
                    ...
                    pdata[i].cmvel_star[0] = cmx;
                    pdata[i].cmvel_star[1] = cmy;
                    pdata[i].cmvel_star[2] = cmz;
                ...
                if (pdata[i].n_star >= PROPMORPHMINNUM)
                    GetGlobalSpatialMorphology(numingroup[i], &Part[noffset[i]], pdata[i].q_star, pdata[i].s_star, 1e-2,
                                               pdata[i].eigvec_star, 0, STARTYPE, 0);
#endif
                ...
                {
                    Pval = &Part[j + noffset[i]];
                    if (Pval->GetType() == DARK2TYPE || Pval->GetType() == DARK3TYPE ||
                        (Pval->GetType() == DARKTYPE && Pval->GetMass() > opt.zoomlowmassdm))
                    {
                ...
                // morphology calcs
#ifdef NOMASS
                GetGlobalSpatialMorphology(numingroup[i], &Part[noffset[i]], pdata[i].gq, pdata[i].gs, 1e-2,
                                           pdata[i].geigvec, 0);
                // calculate morphology based on particles within RV, the radius of maximum circular velocity
                if (RV_num >= PROPMORPHMINNUM)
                    GetGlobalSpatialMorphology(RV_num, &Part[noffset[i]], pdata[i].RV_q, pdata[i].RV_s, 1e-2,
                                               pdata[i].RV_eigvec, 0);
#else
            GetGlobalSpatialMorphology(numingroup[i], &Part[noffset[i]], pdata[i].gq, pdata[i].gs, 1e-2,
                                       pdata[i].geigvec, 1);
    ...
    for (i = 1; i <= ngroup; i++)
        if (numingroup[i] >= omppropnum)
        {
            ...
            if (opt.iInclusiveHalo == 0 || opt.iInclusiveHalo != 0 && pdata[i].hostid != -1)
            {
                // CalculateSphericalOverdensity(opt, pdata[i], numingroup[i], &Part[noffset[i]], m200val, m200mval,
                // mBN98val, virval, m500val, SOlgrhovals);
                CalculateSphericalOverdensitySubhalo(opt, pdata[i], numingroup[i], &Part[noffset[i]], m200val, m200mval,
                                                     mBN98val, virval, m500val, SOlgrhovals);
                SetSphericalOverdensityMasstoTotalMass(opt, pdata[i]);
            }
            if (opt.iextrahalooutput)
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(j, Pval, rc, x, y, z, vx, vy, vz, J, mval)
            {
#pragma omp for reduction(+ : Jx, Jy, Jz, sxx, sxy, sxz, syy, syz, szz, Ekin)
                ...
                for (j = 0; j < numingroup[i]; j++)
                {
                    ...
                    Jy += J[1];
                    Jz += J[2];
                    if (rc < pdata[i].gR200m) Jx200m += J[0];
                    Jy200m += J[1];
                    Jz200m += J[2];
                    if (rc < pdata[i].gR200c) Jx200c += J[0];
                    Jy200c += J[1];
                    Jz200c += J[2];
                    if (rc < pdata[i].gRBN98) JxBN98 += J[0];
                    JyBN98 += J[1];
                    JzBN98 += J[2];
                    sxx += vx * vx * mval;
                    syy += vy * vy * mval;
            ...
            {
                pdata[i].glambda_B =
                    pdata[i].gJ.Length() / (pdata[i].gM200c * sqrt(2.0 * opt.G * pdata[i].gM200c * pdata[i].gR200c));
            }
            // rotational support calculation
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(j, Pval, x, y, z, vx, vy, vz, jval, jzval, zdist, Rdist)
            {
#pragma omp for reduction(+ : Krot)
            ...
            Ekin = Jx = Jy = Jz = sxx = sxy = sxz = syy = syz = szz = Krot = 0.;
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(j, Pval, x, y, z, vx, vy, vz, J, mval)
            {
#pragma omp for reduction(+ : Jx, Jy, Jz, sxx, sxy, sxz, syy, syz, szz, Ekin)
#endif
                for (j = 0; j < RV_num; j++)
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(j, Pval, x, y, z, vx, vy, vz, J, mval, SFR)
                {
#pragma omp for reduction(+:Jx,Jy,Jz,sxx,sxy,sxz,syy,syz,szz,cmx,cmy,cmz,cmvx,cmvy,cmvz,Tsum,tsum,Zsum,sfrsum,Tmeansum,tmeansum,Zmeansum,sfrmeansum, Tsum_sf,Zsum_sf,Tmeansum_sf,Zmeansum_sf,Tsum_nsf,Zsum_nsf,Tmeansum_nsf,Zmeansum_nsf,sigV_gas_sf,sigV_gas_nsf)
                ...
                pdata[i].cm_gas[1]    = cmy;
                pdata[i].cm_gas[2]    = cmz;
                pdata[i].cmvel_gas[0] = cmvx;
                pdata[i].cmvel_gas[1] = cmvy;
                pdata[i].cmvel_gas[2] = cmvz;
                pdata[i].L_gas[0]     = Jx;
                pdata[i].L_gas[1]     = Jy;
                pdata[i].L_gas[2]     = Jz;
                if (pdata[i].n_gas >= PROPROTMINNUM)
...
#pragma omp for reduction(+:Jx,Jy,Jz,sxx,sxy,sxz,syy,syz,szz,cmx,cmy,cmz,cmvx,cmvy,cmvz,tsum,Zsum,tmeansum,Zmeansum)
                ...
                pdata[i].cm_star[1]    = cmy;
                pdata[i].cm_star[2]    = cmz;
                pdata[i].cmvel_star[0] = cmvx;
                pdata[i].cmvel_star[1] = cmvy;
                pdata[i].cmvel_star[2] = cmvz;
                pdata[i].L_star[0]     = Jx;
                pdata[i].L_star[1]     = Jy;
                pdata[i].L_star[2]     = Jz;
                if (pdata[i].n_star >= PROPROTMINNUM)
...
#pragma omp for schedule(dynamic) nowait
    ...
    {
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, Pval, x, y, z)
        {
#pragma omp for schedule(dynamic) nowait
#endif
            for (i = 1; i <= ngroup; i++)
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, Pval)
    {
#pragma omp for schedule(dynamic) nowait
#endif
        for (i = 1; i <= ngroup; i++)
        ...
        for (i = 1; i <= ngroup; i++)
        {
            ...
            if (opt.iPropertyReferencePosition == PROPREFMBP)
            {
                for (auto k = 0; k < 3; k++)
                {
                    ...
                    if (pdata[i].gposmbp[k] < 0)
                        pdata[i].gposmbp[k] += opt.p;
                    else if (pdata[i].gposmbp[k] > opt.p)
                        pdata[i].gposmbp[k] -= opt.p;
                }
            }
            if (opt.iPropertyReferencePosition == PROPREFMINPOT)
            {
                for (auto k = 0; k < 3; k++)
                {
                    ...
                    if (pdata[i].gposminpot[k] < 0)
                        pdata[i].gposminpot[k] += opt.p;
                    else if (pdata[i].gposminpot[k] > opt.p)
                        pdata[i].gposminpot[k] -= opt.p;
                }
            }
        }
...
void GetMaximumSizes(Options &opt, Int_t nbodies, Particle *Part, Int_t ngroup, Int_t *&numingroup, PropData *&pdata,
                     Int_t *&noffset)
{
    Int_t     i;
    ...
    Particle *Pval;
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, Pval, rcm, rmbp, rminpot)
    {
#pragma omp for nowait
#endif
        for (i = 1; i <= ngroup; i++)
...
#pragma omp for schedule(dynamic) nowait
        ...
        for (i = 1; i <= ngroup; i++)
        {
            ...
            // always possible halo severly truncated before so correct if necessary and also for tidal debris, both
            // vmax concentration pretty meaningless
            if (pdata[i].VmaxVvir2 <= 1.05)
            {
                if (pdata[i].gM200c == 0)
                    pdata[i].cNFW = pdata[i].gsize / pdata[i].gRmaxvel;
                else
                    pdata[i].cNFW = pdata[i].gR200c / pdata[i].gRmaxvel;
            }
            else
            {
                if (numingroup[i] >= PROPNFWMINNUM)
                    CalcConcentration(pdata[i]);
                else
                {
                    if (pdata[i].gM200c == 0)
                        pdata[i].cNFW = pdata[i].gsize / pdata[i].gRmaxvel;
...
/// Get inclusive halo FOF based masses. If requesting spherical overdensity masses then extra computation and search
/// required
void GetInclusiveMasses(Options &opt, const Int_t nbodies, Particle *Part, Int_t ngroup, Int_t *&pfof,
                        Int_t *&numingroup, PropData *&pdata, Int_t *&noffset)
{
    Particle *Pval;
    KDTree *  tree;
    ...
    if (opt.iverbose)
    {
        ...
        if (opt.iInclusiveHalo == 1)
            cout << " with masses based on the FOF envelopes (quicker)" << endl;
...
#pragma omp parallel
    ...
    for (i = 1; i <= ngroup; i++) { pdata[i].Allocate(opt); }

    // first get center of mass and maximum size

    // for small groups loop over groups
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, Pval, ri, rcmv, ri2, r2, cmx, cmy, cmz, EncMass, Ninside, icmv, \
                                             cmold, x, y, z, vx, vy, vz, massval)
    {
#pragma omp for schedule(dynamic) nowait
#endif
        for (i = 1; i <= ngroup; i++)
            if (numingroup[i] < omppropnum)
            {
                ...
                for (j = 0; j < numingroup[i]; j++)
                {
                    ...
                    for (k = 0; k < 3; k++) { pdata[i].gcm[k] += (*Pval).GetPosition(k) * massval; }
                }
                ...
                {
                    Pval = &Part[j + noffset[i]];
                    for (k = 0; k < 3; k++) { Pval->SetPosition(k, (*Pval).GetPosition(k) - pdata[i].gcm[k]); }
                }
                // sort by radius
                gsl_heapsort(&Part[noffset[i]], numingroup[i], sizeof(Particle), RadCompare);
                pdata[i].gsize      = Part[noffset[i] + numingroup[i] - 1].Radius();
    ...
    for (i = 1; i <= ngroup; i++)
        if (numingroup[i] >= omppropnum)
        {
            ...
            EncMass = cmx = cmy = cmz = 0.;
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(j, Pval, massval)
            {
#pragma omp for reduction(+ : EncMass, cmx, cmy, cmz)
#endif
                for (j = 0; j < numingroup[i]; j++)
            ...
            while (opt.iIterateCM)
            {
                ...
                Ninside         = 0;
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(j, Pval, x, y, z, massval)
                {
#pragma omp for reduction(+ : EncMass, Ninside, cmx, cmy, cmz)
#endif
                    for (j = 0; j < numingroup[i]; j++)
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, Pval) private(                                  \
    massval, x, y, z, vx, vy, vz, J, rc, rcold, EncMass, numinvir, num200c, num200m, rhoval, rhoval2) \
...
#pragma omp for schedule(dynamic) nowait
            ...
            for (i = 1; i <= ngroup; i++)
            {
                // here masses are technically exclusive but this routine is generally called before objects are
                // separated into halo/substructures
                CalculateSphericalOverdensity(opt, pdata[i], numingroup[i], &Part[noffset[i]], m200val, m200mval,
                                              mBN98val, virval, m500val, SOlgrhovals);
        ...
        {
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, Pval) private(massval, EncMass, Ninside, rc)
            {
#pragma omp for schedule(dynamic) nowait
#endif
                for (i = 1; i <= ngroup; i++)
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, x, y, z, Pval)
        {
#pragma omp for schedule(dynamic) nowait
#endif
            for (i = 1; i <= ngroup; i++)
...
#pragma omp for schedule(dynamic) nowait
...
#endif
        //
        if (opt.iverbose >= 2) { cout << ThisTask << " building trees for SO search " << endl; }
        ...
        {
            for (i = 1; i <= ngroup; i++)
                if (maxsearchdist < maxrdist[i]) maxsearchdist = maxrdist[i];
            cout << ThisTask << " max search distance is " << maxsearchdist << " in period fraction "
                 << maxsearchdist / opt.p << endl;
        }
#ifdef USEMPI
        ...
        if (NProcs > 1)
        {
            ...
            // run search on exported particles and determine which local particles need to be exported back (or
            // imported)
            nimport = MPIBuildParticleNNImportList(nbodies, tree, Part);
            if (nimport > 0)
                treeimport =
                    new KDTree(PartDataGet, nimport, opt.HaloMinSize, tree->TPHYS, tree->KEPAN, 100, 0, 0, 0, period);
        }
#endif
        time2 = MyGetTime();
        ...
        fac = -log(4.0 * M_PI / 3.0);
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, taggedparts, radii, masses, indices, posparts, velparts, \
                                             typeparts, n, dx, EncMass, J, rc, rhoval, rhoval2, tid, SOpids, iSOfound)
        {
#pragma omp for schedule(dynamic) nowait
#endif
            for (i = 1; i <= ngroup; i++)
            {
...
#if defined(GASON) || defined(STARON) || defined(BHON)
                if (opt.iextragasoutput || opt.iextrastaroutput || opt.iSphericalOverdensityPartList)
                ...
                if (NProcs > 1)
                {
                    // if halo has overlap then search the imported particles as well, add them to the radii and mass
                    // vectors
                            ...
                            for (j = 0; j < taggedparts.size(); j++)
                            {
                                ...
                                for (k = 0; k < 3; k++)
                                {
                                    ...
                                    if (opt.iextrahalooutput)
                                    {
                                        ...
                                        velparts[j + offset][k] =
                                            PartDataGet[taggedparts[j]].GetVelocity(k) - pdata[i].gcmvel[k];
                                    }
                ...
                Int_t llindex = CalculateSphericalOverdensity(opt, pdata[i], radii, masses, indices, m200val, m200mval,
                                                              mBN98val, virval, m500val, SOlgrhovals);
...
void GetFOFMass(Options &opt, const Int_t nbodies, Particle *Part, Int_t ngroup, Int_t *&pfof, Int_t *&numingroup,
                PropData *&pdata, Int_t *&noffset)
{
    Particle *Pval;
    Int_t     i, j, k;
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, Pval, massval)
    {
#pragma omp for schedule(dynamic) nowait
#endif
        for (i = 1; i <= ngroup; i++)
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, taggedparts, radii, masses, indices, posref, posparts, velparts, \
                                             typeparts, n, dx, EncMass, J, rc, rhoval, rhoval2, tid, SOpids, iSOfound)
    {
#pragma omp for schedule(dynamic) nowait
#endif
        for (i = 1; i <= ngroup; i++)
        {
            ...
            if (opt.iPropertyReferencePosition == PROPREFCM)
                ...
                posref = pdata[i].gposmbp;
            else if (opt.iPropertyReferencePosition == PROPREFMINPOT)
                posref = pdata[i].gposminpot;
            iSOfound = 0;
...
#if defined(GASON) || defined(STARON) || defined(BHON) || defined(HIGHRES)
            if (opt.iextragasoutput || opt.iextrastaroutput || opt.iextrainterloperoutput ||
                opt.iSphericalOverdensityPartList)
                typeparts.resize(taggedparts.size());
#endif
            if (opt.iSphericalOverdensityPartList) SOpids.resize(taggedparts.size());
            for (j = 0; j < taggedparts.size(); j++)
            {
                ...
                for (k = 0; k < 3; k++)
                {
                    ...
                    if (opt.iextrahalooutput)
                    {
                        posparts[j][k] = dx;
                        velparts[j][k] =
                            Part[taggedparts[j]].GetVelocity(k) - pdata[i].gcmvel[k]; //??? i think this should be okay
                    }
                    radii[j] += dx * dx;
                }
                radii[j] = sqrt(radii[j]);
            ...
            if (opt.iextrahalooutput)
            {
                for (j = 0; j < radii.size(); j++)
                {
                    ...
                    for (auto iso = 0; iso < opt.SOnum; iso++)
                        if (rc < pdata[i].SO_radius[iso]) { pdata[i].SO_angularmomentum[iso] += J; }
#ifdef GASON
                    if (opt.iextragasoutput)
                    {
                        if (typeparts[indices[j]] == GASTYPE)
                    ...
                    {
                        if (typeparts[indices[j]] == DARK2TYPE || typeparts[indices[j]] == DARK3TYPE ||
                            (typeparts[indices[j]] == DARKTYPE && massval > opt.zoomlowmassdm))
                        {
                            if (rc <= pdata[i].gR200c) { pdata[i].M_200crit_interloper += massval; }
                            if (rc <= pdata[i].gR200m) { pdata[i].M_200mean_interloper += massval; }
                            if (rc <= pdata[i].gRBN98) { pdata[i].M_BN98_interloper += massval; }
                            for (auto iso = 0; iso < opt.SOnum; iso++)
                                if (rc < pdata[i].SO_radius[iso]) { pdata[i].SO_mass_interloper[iso] += massval; }
                        }
                    }
#endif
                }
...
void GetGlobalSpatialMorphology(const Int_t nbodies, Particle *p, Double_t &q, Double_t &s, Double_t Error,
                                Matrix &eigenvec, int imflag, int itype, int iiterate)
{
    ...
    if (iiterate)
    {
        do
        {
            ...
            eigenvecp = Matrix(0.);
            if (imflag == 1)
                CalcMTensorWithMass(M, q, s, nbodies, p, itype);
            else
                CalcMTensor(M, q, s, nbodies, p, itype);
            e         = M.Eigenvalues();
            oldq      = q;
            olds      = s;
            q         = sqrt(e[1] / e[0]);
                 ...
                 int itype)
{
    ...
    if (n >= ompunbindnum)
    {
#pragma omp                              parallel default(shared) private(i, r2, weight)
        {
...
void CalcPosSigmaTensor(const Int_t n, Particle *p, Double_t &a, Double_t &b, Double_t &c, Matrix &eigenvec, Matrix &I,
                        int itype)
{
    Double_t   r2, det, Ixx, Iyy, Izz, Ixy, Ixz, Iyz, weight;
    Coordinate e;
    ...
    if (n >= ompunbindnum)
    {
#pragma omp parallel default(shared) private(i, weight)
        {
#pragma omp for schedule(dynamic) reduction(+ : Ixx, Iyy, Izz, Ixy, Ixz, Iyz, mtot)
    ...
    else
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                for (int k = 0; k < 3; k++) { I(j, k) += (p[i].GetPosition(j) * p[i].GetPosition(k)) * weight; }
            }
            mtot += weight;
        }
#ifdef USEOPENMP
...
void CalcVelSigmaTensor(const Int_t n, Particle *p, Double_t &a, Double_t &b, Double_t &c, Matrix &eigenvec, Matrix &I,
                        int itype)
{
    Double_t   r2, det, Ixx, Iyy, Izz, Ixy, Ixz, Iyz, weight;
    Coordinate e;
...
#pragma omp for schedule(dynamic) reduction(+ : Ixx, Iyy, Izz, Ixy, Ixz, Iyz, mtot)
    ...
    else
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                for (int k = 0; k < 3; k++) { I(j, k) += (p[i].GetVelocity(j) * p[i].GetVelocity(k)) * weight; }
            }
            mtot += weight;
        }
#ifdef USEOPENMP
...
#pragma omp for schedule(dynamic) reduction(+:Ixx,Iyy,Izz,Ixy,Ixz,Iyz,Ivxvx,Ivyvy,Ivzvz,Ivxvy,Ivxvz,Ivyvz,Ixvx,Iyvx,Izvx,Ixvy,Iyvy,Izvy,Ixvz,Iyvz,Izvz,mtot)
    ...
    else
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            for (int j = 0; j < 6; j++)
            {
                for (int k = 0; k < 6; k++) { I(j, k) += (p[i].GetPhase(j) * p[i].GetPhase(k)) * weight; }
            }
            mtot += weight;
        }
#ifdef USEOPENMP
...
void CalcMTensor(Matrix &M, const Double_t q, const Double_t s, const Int_t n, Particle *p, int itype)
{
    ...
    if (n >= ompunbindnum)
    {
#pragma omp                              parallel default(shared) private(i, a2, weight)
        {
    ...
    else
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            if (a2 != 0)
            {
                ...
                a2 = 1.0 / a2 * weight;
                for (j = 0; j < 3; j++)
                    for (k = 0; k < 3; k++) M(j, k) += p[i].GetPosition(j) * p[i].GetPosition(k) * a2;
            }
        }
#ifdef USEOPENMP
    }
...
void ReorderInclusiveMasses(const Int_t &numgroups, const Int_t &newnumgroups, Int_t *&numingroup, PropData *&pdata)
{
    ...
    for (Int_t i = 1; i <= numgroups; i++)
        if (numingroup[i] > 0) pq->Push(i, numingroup[i]);
    for (Int_t i = 1; i <= newnumgroups; i++)
    {
        Int_t groupid = pq->TopQueue(), size = pq->TopPriority();
        pq->Pop();
...
//@{
/*!
    Calculate the potential energy and kinetic energy relative to the velocity frame stored in gcmvel. Note that
   typically this is the velocity of particles within the inner region used to determine the centre-of-mass. BUT of
   course, this frame is not without its flaws, as in a chaotic mergering system, one might not be able to disentangle
   structures and the centre-of-mass need not be located at the "centre" or dense point of any of the merging
   structures. Once the energy is calculated, the total energy is stored in potential, that way it is easy to sort
   particles according to their binding energy.

    The overall structure of the code is a bit lengthy simple to break up calculations appropriately for OMP style
   parallization. For small groups it is more efficient to parallize across groups, whereas for large groups containing
   many particles, we loop over the particles to sum quantities.

    \todo might alter binding energy to use the velocity around the particle at the deepest point in the potential.
 */
void GetBindingEnergy(Options &opt, const Int_t nbodies, Particle *Part, Int_t ngroup, Int_t *&pfof, Int_t *&numingroup,
                      PropData *&pdata, Int_t *&noffset)
{
#ifndef USEMPI
    int ThisTask = 0, NProcs = 1;
    ...
    // Note could use more complicated b-spline but at least here since not evolving dynamics, plummer potential not an
    // issue.
    ...
    // used to temporarily store pids. Needed for large groups as the tree code used to calculate potential overwrites
    // the id of particles so that once finished it puts the particles back into the input order. Therefore store id
    // values in PID  value (which can be over written) also if wish to use the deepest potential as a reference, then
    // used to store original order
    Int_t *storepid;
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, r2, v2, poti, Ti, pot, Eval, npot, storepid, menc, potmin, \
                                             ipotmin)
        {
#pragma omp for schedule(dynamic) nowait
#endif
                ...
                if (numingroup[i] < ompunbindnum)
                {
                    for (j = 0; j < numingroup[i]; j++)
                    {
                        for (k = j + 1; k < numingroup[i]; k++)
                        {
                            ...
                            for (int n = 0; n < 3; n++)
                                r2 +=
                                    pow(Part[j + noffset[i]].GetPosition(n) - Part[k + noffset[i]].GetPosition(n), 2.0);
                            r2 += eps2;
                            r2 = 1.0 / sqrt(r2);
#ifdef NOMASS
                            pot = -opt.G * r2 * mw2;
...
#endif
                            pdata[i].Pot += pot;
                            poti = Part[j + noffset[i]].GetPotential() + pot;
                            Part[j + noffset[i]].SetPotential(poti);
                            poti = Part[k + noffset[i]].GetPotential() + pot;
                            Part[k + noffset[i]].SetPotential(poti);
                        }
                    }
                }
#ifdef USEOPENMP
    ...
    else
    {
        for (i = 1; i <= ngroup; i++)
            if (numingroup[i] < ompunbindnum)
                for (j = 0; j < numingroup[i]; j++)
                    Part[j + noffset[i]].SetPotential(Part[j + noffset[i]].GetGravityPotential());
    }
#endif
...
#pragma omp for schedule(dynamic) nowait
            ...
            for (i = 1; i <= ngroup; i++)
                if (numingroup[i] < ompunbindnum)
                {
                    ...
                    ipotmin = 0;
                    for (j = 0; j < numingroup[i]; j++)
                        if (Part[j + noffset[i]].GetPotential() < potmin)
                        {
                            if (opt.ParticleTypeForRefenceFrame != -1 &&
                                Part[noffset[i] + j].GetType() != opt.ParticleTypeForRefenceFrame)
                                continue;
                    ...
                    {
                        for (k = 0; k < 3; k++)
                            Part[j + noffset[i]].SetPosition(k, Part[j + noffset[i]].GetPosition(k) - cmpotmin[k]);
                    }
                    gsl_heapsort(&Part[noffset[i]], numingroup[i], sizeof(Particle), RadCompare);
                    ...
                    {
                        for (k = 0; k < 3; k++)
                            pdata[i].gcmvel[k] += Part[j + noffset[i]].GetVelocity(k) * Part[j + noffset[i]].GetMass();
    ...
    {
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, potmin, ipotmin)
        {
#pragma omp for schedule(dynamic) nowait
#endif
            for (i = 1; i <= ngroup; i++)
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, r2, v2, poti, Ti, pot, Eval, npot, storepid)
    {
#pragma omp for schedule(dynamic) nowait
#endif
        for (i = 1; i <= ngroup; i++)
            if (numingroup[i] < ompunbindnum)
            {
                for (j = 0; j < numingroup[i]; j++)
                {
                    v2 = 0.;
                    for (int n = 0; n < 3; n++)
                        v2 += pow(Part[j + noffset[i]].GetVelocity(n) - pdata[i].gcmvel[n], 2.0);
#ifdef NOMASS
...
#ifdef GASON
                    if (Part[j + noffset[i]].GetDensity() < 0 && Part[j + noffset[i]].GetType() == GASTYPE)
                        pdata[i].Efrac_gas += 1.0;
#endif
#ifdef STARON
                    if (Part[j + noffset[i]].GetDensity() < 0 && Part[j + noffset[i]].GetType() == STARTYPE)
                        pdata[i].Efrac_star += 1.0;
#endif
                }
...
#endif
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(j, v2, Ti, Ei)
            {
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, Emostbound, imostbound)
    {
#pragma omp for schedule(dynamic) nowait
#endif
        for (i = 1; i <= ngroup; i++)
...
/*!
    ...
    This code first sorts particles according to their (local mpi) group id and calculates center of mass and binding
   energy.
*/
Int_t **SortAccordingtoBindingEnergy(Options &opt, const Int_t nbodies, Particle *Part, Int_t ngroup, Int_t *&pfof,
                                     Int_t *numingroup, PropData *pdata, Int_t ioffset)
{
#ifndef USEMPI
    int ThisTask = 0, NProcs = 1;
#endif
    ...
    {
        if (opt.iPropertyReferencePosition == PROPREFCM)
            cout << ThisTask << " Calculate properties using CM as reference " << endl;
        else if (opt.iPropertyReferencePosition == PROPREFMBP)
            cout << ThisTask << " Calculate properties using most bound particle as reference " << endl;
        else if (opt.iPropertyReferencePosition == PROPREFMINPOT)
            cout << ThisTask << " Calculate properties using minimum potential particle as reference " << endl;
        if (opt.iSortByBindingEnergy)
            cout << ThisTask << " Sort particles by binding energy" << endl;
        else
            cout << ThisTask << " Sort particles by potential energy" << endl;
    }
    ...
    for (i = 0; i < nbodies; i++)
    {
        ...
        if (pfof[Part[i].GetID()] > ioffset)
            Part[i].SetPID(pfof[Part[i].GetID()]);
        else
            Part[i].SetPID(nbodies + 1); // here move all particles not in groups to the back of the particle array
    }
...
#pragma omp for nowait
        ...
        for (i = 1; i <= ngroup; i++)
        {
            if (opt.iSortByBindingEnergy) { qsort(&Part[noffset[i]], numingroup[i], sizeof(Particle), DenCompare); }
            else
            {
            ...
            for (j = 0; j < numingroup[i]; j++)
                if (Part[noffset[i] + j].GetDensity() > 0)
                {
                    pdata[i].iunbound = j;
                    break;
                }
    ...
    for (i = 1; i <= ngroup; i++)
    {
        ...
        if (opt.iseparatefiles)
            ...
            for (j = 0; j < numingroup[i]; j++) pglist[i][j] = j + noffset[i];
        if (numingroup[i] > 0)
            pglist[i][numingroup[i]] = pdata[i].iunbound;
        else
            pglist[i][0] = 0;
    }
    delete[] noffset;
...
/*
   Calculate Halo properties only, assumes that information in particle PIDs is meaningless, useful when don't care
   about particle tracking and just want halo catalogs (like when analysing results from runs like PICOLA (or say 2LPT
   runs))
*/
void CalculateHaloProperties(Options &opt, const Int_t nbodies, Particle *Part, Int_t ngroup, Int_t *&pfof,
                             Int_t *numingroup, PropData *pdata)
{
#ifndef USEMPI
    int ThisTask = 0, NProcs = 1;
#endif
...
Int_t *GetSubstrutcureNum(Int_t ngroups)
{
    ...
    for (int i = nhierarchy - 1; i >= 1; i--)
    {
        ...
        for (int j = 0; j < papsldata[i]->nsinlevel; j++)
...
{
    Double_t bnx = -(opt.Omega_k * pow(a, -2.0) + opt.Omega_Lambda) /
                   (opt.Omega_k * pow(a, -2.0) + opt.Omega_m * pow(a, -3.0) + opt.Omega_Lambda);
...
{
    return opt.h * opt.H *
           sqrt(opt.Omega_k * pow(a, -2.0) + opt.Omega_m * pow(a, -3.0) + opt.Omega_r * pow(a, -4.0) +
                opt.Omega_Lambda + opt.Omega_de * pow(a, -3.0 * (1 + opt.w_de)));
}
...
double GetInvaH(double a, void *params)
{
    ...
    double H = sqrt(Omega_k * pow(a, -2.0) + Omega_m * pow(a, -3.0) + Omega_r * pow(a, -3.0) + Omega_Lambda +
                    Omega_de * pow(a, -3.0 * (1 + w_de)));
    return 1.0 / (a * H);
}
...
//@{
inline Double_t GetApertureRadiusInterpolation(const Double_t &oldrc, const Double_t &rc, const Double_t &EncMass,
                                               const Double_t &mass, const Double_t refmass)
{
...
void CalculateApertureQuantities(Options &opt, Int_t &ning, Particle *Part, PropData &pdata)
{
    ...
    unsigned int Ninside = 0, NinsideGas = 0, NinsideGasSF = 0, NinsideGasNSF = 0, NinsideStar = 0, NinsideBH = 0,
                 NinsideInterloper = 0;
    Double_t EncMass = 0, EncMassGas = 0, EncMassGasSF = 0, EncMassGasNSF = 0, EncMassStar = 0, EncMassBH = 0,
             EncMassInterloper = 0;
    Double_t EncVelDisp = 0, EncVelDispGas = 0, EncVelDispGasSF = 0, EncVelDispGasNSF = 0, EncVelDispStar = 0,
             EncVelDispBH = 0, EncVelDispInterloper = 0;
    Double_t EncVRDisp = 0, EncVRDispGas = 0, EncVRDispGasSF = 0, EncVRDispGasNSF = 0, EncVRDispStar = 0,
             EncVRDispBH = 0, EncVRDispInterloper = 0;
    Double_t   EncSFR    = 0;
    ...
    if (opt.aperturenum > 0)
    {
...
#endif
            veldisp = 0;
            for (auto k = 0; k < 3; k++) veldisp += pow(Pval->GetVelocity(k) - pdata.gcmvel[k], 2.0);
            veldisp *= mass;
            vrdisp = 0;
            for (auto k = 0; k < 3; k++)
                vrdisp += pow((Pval->GetVelocity(k) - pdata.gcmvel[k]) * Pval->GetPosition(k), 2.0);
            vrdisp *= mass / (rc * rc);
        ...
        for (auto j = 0; j < ning; j++)
        {
            ...
            for (auto k = 0; k < opt.aperturenum; k++)
            {
                ...
                {
                    pdata.aperture_rhalfmass[k] = GetApertureRadiusInterpolation(
                        oldrc, rc, EncMass, mass, 0.5 * pdata.aperture_mass[k]); // oldrc;
                    iaptindex++;
                }
#ifdef GASON
                if (EncMassGas >= 0.5 * pdata.aperture_mass_gas[k] && pdata.aperture_rhalfmass_gas[k] == -1)
                {
                    pdata.aperture_rhalfmass_gas[k] = GetApertureRadiusInterpolation(
                        oldrc_gas, rc, EncMassGas, mass, 0.5 * pdata.aperture_mass_gas[k]); // oldrc_gas;
                    iaptindex++;
                }
#ifdef STARON
                if (EncMassGasSF >= 0.5 * pdata.aperture_mass_gas_sf[k] && pdata.aperture_rhalfmass_gas_sf[k] == -1)
                {
                    pdata.aperture_rhalfmass_gas_sf[k] = GetApertureRadiusInterpolation(
                        oldrc_gas_sf, rc, EncMassGasSF, mass, 0.5 * pdata.aperture_mass_gas_sf[k]); // oldrc_gas_sf;
                    iaptindex++;
                }
                if (EncMassGasNSF >= 0.5 * pdata.aperture_mass_gas_nsf[k] && pdata.aperture_rhalfmass_gas_nsf[k] == -1)
                {
                    pdata.aperture_rhalfmass_gas_nsf[k] = GetApertureRadiusInterpolation(
                        oldrc_gas_nsf, rc, EncMassGasNSF, mass, 0.5 * pdata.aperture_mass_gas_nsf[k]); // oldrc_gas_nsf;
                    iaptindex++;
                }
#endif
#endif
                ...
                {
                    pdata.aperture_rhalfmass_star[k] = GetApertureRadiusInterpolation(
                        oldrc_star, rc, EncMassStar, mass, 0.5 * pdata.aperture_mass_star[k]); // oldrc_star;
                    iaptindex++;
                }
#endif
            }
    ...
    if (opt.apertureprojnum > 0)
    {
        for (auto j = 0; j < ning; j++)
        {
            ...
            for (auto k = 0; k < 3; k++)
            {
                x2[k] = Pval->GetPosition(k);
                x2[k] = x2[k] * x2[k];
            }
            proj[j].rproj[0] = sqrt(x2[0] + x2[1]);
            proj[j].rproj[1] = sqrt(x2[0] + x2[2]);
            proj[j].rproj[2] = sqrt(x2[1] + x2[2]);
        }
        ...
        for (auto k = 0; k < 3; k++)
        {
            if (k == 0)
            {
                sort(proj.begin(), proj.end(),
                     [](projectedmass &a, projectedmass &b) { return a.rproj[0] < b.rproj[0]; });
            }
            if (k == 1)
            {
                sort(proj.begin(), proj.end(),
                     [](projectedmass &a, projectedmass &b) { return a.rproj[1] < b.rproj[1]; });
            }
            if (k == 2)
            {
                sort(proj.begin(), proj.end(),
                     [](projectedmass &a, projectedmass &b) { return a.rproj[2] < b.rproj[2]; });
            }
            iaptindex = 0;
            EncMass = EncMassGas = EncMassGasSF = EncMassGasNSF = EncMassStar = EncMassBH = EncMassInterloper = 0;
            EncSFR                                                                                            = 0;
            ...
            for (auto j = 0; j < ning; j++)
            {
                ...
                for (auto i = 0; i < opt.apertureprojnum; i++)
                {
                    ...
                    {
                        pdata.aperture_rhalfmass_proj[i][k] = GetApertureRadiusInterpolation(
                            oldrc, rc, EncMass, mass, 0.5 * pdata.aperture_mass_proj[i][k]); // oldrc;
                        iaptindex++;
                    }
#ifdef GASON
                    if (EncMassGas >= 0.5 * pdata.aperture_mass_proj_gas[k][i] &&
                        pdata.aperture_rhalfmass_proj_gas[i][k] == -1)
                    {
                        pdata.aperture_rhalfmass_proj_gas[i][k] = GetApertureRadiusInterpolation(
                            oldrc_gas, rc, EncMassGas, mass, 0.5 * pdata.aperture_mass_proj_gas[i][k]); // oldrc_gas;
                        iaptindex++;
                    }
#ifdef STARON
                    if (EncMassGasSF >= 0.5 * pdata.aperture_mass_proj_gas_sf[i][k] &&
                        pdata.aperture_rhalfmass_proj_gas_sf[i][k] == -1)
                    {
                        pdata.aperture_rhalfmass_proj_gas_sf[i][k] = GetApertureRadiusInterpolation(
                            oldrc_gas_sf, rc, EncMassGasSF, mass,
                            0.5 * pdata.aperture_mass_proj_gas_sf[i][k]); // oldrc_gas_sf;
                        iaptindex++;
                    }
                    if (EncMassGasNSF >= 0.5 * pdata.aperture_mass_proj_gas_nsf[i][k] &&
                        pdata.aperture_rhalfmass_proj_gas_nsf[i][k] == -1)
                    {
                        pdata.aperture_rhalfmass_proj_gas_nsf[i][k] = GetApertureRadiusInterpolation(
                            oldrc_gas_nsf, rc, EncMassGasNSF, mass,
                            0.5 * pdata.aperture_mass_proj_gas_nsf[i][k]); // oldrc_gas_nsf;
                        iaptindex++;
                    }
#endif
#endif
#ifdef STARON
                    if (EncMassStar >= 0.5 * pdata.aperture_mass_proj_star[i][k] &&
                        pdata.aperture_rhalfmass_proj_star[i][k] == -1)
                    {
                        pdata.aperture_rhalfmass_proj_star[i][k] =
                            GetApertureRadiusInterpolation(oldrc_star, rc, EncMassStar, mass,
                                                           0.5 * pdata.aperture_mass_proj_star[i][k]); // oldrc_star;
                        iaptindex++;
                    }
#endif
                }
...
//@{
// loop over radii to get overdensity working outwards from some small fraction of the mass or at least 1 particles +
// small fraction of min halo size
Int_t CalculateSphericalOverdensity(Options &opt, PropData &pdata, vector<Double_t> &radii, vector<Double_t> &masses,
                                    vector<Int_t> &indices, Double_t &m200val, Double_t &m200mval, Double_t &mBN98val,
                                    Double_t &virval, Double_t &m500val, vector<Double_t> &SOlgrhovals)
{
    int minnum = max((int)(opt.SphericalOverdensityMinHaloFac * radii.size() + 1),
                     (int)(opt.HaloMinSize * opt.SphericalOverdensityMinHaloFac + 1));
    ...
    EncMass = 0;
    for (auto j = 0; j < minnum; j++) EncMass += masses[indices[j]];
    ...
    for (auto j = minnum; j < radii.size(); j++)
    {
        ...
        if (pdata.gRvir == 0)
            if (rhoval < virval)
            {
                // linearly interpolate, unless previous density also below threshold (which would happen at the start,
                // then just set value)
                delta       = (virval - rhoval);
        ...
        for (auto iso = 0; iso < opt.SOnum; iso++)
        {
            if (pdata.SO_radius[iso] == 0)
                if (rhoval < SOlgrhovals[iso])
                {
        ...
        if (pdata.gR200m != 0 && pdata.gR200c != 0 && pdata.gRvir != 0 && pdata.gR500c != 0 && pdata.gRBN98 != 0 &&
            iSOfound == opt.SOnum)
        {
            llindex = j;
            break;
    ...
    for (auto iso = 0; iso < opt.SOnum; iso++)
        if (pdata.SO_mass[iso] < MinMass) { pdata.SO_mass[iso] = pdata.SO_radius[iso] = 0.0; }
    return llindex;
}

Int_t CalculateSphericalOverdensity(Options &opt, PropData &pdata, Int_t &numingroup, Particle *Part, Double_t &m200val,
                                    Double_t &m200mval, Double_t &mBN98val, Double_t &virval, Double_t &m500val,
                                    vector<Double_t> &SOlgrhovals)
{
    int minnum = max((int)(opt.SphericalOverdensityMinHaloFac * numingroup + 1),
                     (int)(opt.HaloMinSize * opt.SphericalOverdensityMinHaloFac + 1));
...
void CalculateSphericalOverdensitySubhalo(Options &opt, PropData &pdata, Int_t &numingroup, Particle *Part,
                                          Double_t &m200val, Double_t &m200mval, Double_t &mBN98val, Double_t &virval,
                                          Double_t &m500val, vector<Double_t> &SOlgrhovals)
{
    ...
    for (auto j = numingroup - 1; j >= 0; j--)
    {
        ...
        rc     = Pval->Radius();
        rhoval = log(EncMass) - 3.0 * log(rc) + fac;
        if (pdata.gRvir == 0 && EncMass >= 0.01 * pdata.gmass)
            if (rhoval > virval)
            {
                pdata.gMvir = EncMass;
                pdata.gRvir = rc;
            }
        if (pdata.gR200c == 0 && EncMass >= 0.01 * pdata.gmass)
            if (rhoval > m200val)
            {
                pdata.gM200c = EncMass;
                pdata.gR200c = rc;
            }
        if (pdata.gR200m == 0 && EncMass >= 0.01 * pdata.gmass)
            if (rhoval > m200mval)
            {
                pdata.gM200m = EncMass;
                pdata.gR200m = rc;
            }
        if (pdata.gR500c == 0 && EncMass >= 0.01 * pdata.gmass)
            if (rhoval > m500val)
            {
                pdata.gM500c = EncMass;
                pdata.gR500c = rc;
            }
        if (pdata.gRBN98 == 0 && EncMass >= 0.01 * pdata.gmass)
            if (rhoval > mBN98val)
            {
...
void CalculateSphericalOverdensityExclusive(Options &opt, PropData &pdata, Int_t &numingroup, Particle *Part,
                                            Double_t &m200val, Double_t &m200mval, Double_t &mBN98val, Double_t &virval,
                                            Double_t &m500val, vector<Double_t> &SOlgrhovals)
{
    Double_t  EncMass, rc, rhoval, fac;
    ...
    for (auto j = numingroup - 1; j >= 0; j--)
    {
        ...
        rc     = Pval->Radius();
        rhoval = log(EncMass) - 3.0 * log(rc) + fac;
        if (pdata.gRvir_excl == 0 && EncMass >= 0.01 * pdata.gmass)
            if (rhoval > virval)
            {
                pdata.gMvir_excl = EncMass;
                pdata.gRvir_excl = rc;
            }
        if (pdata.gR200c_excl == 0 && EncMass >= 0.01 * pdata.gmass)
            if (rhoval > m200val)
            {
                pdata.gM200c_excl = EncMass;
                pdata.gR200c_excl = rc;
            }
        if (pdata.gR200m_excl == 0 && EncMass >= 0.01 * pdata.gmass)
            if (rhoval > m200mval)
            {
                pdata.gM200m_excl = EncMass;
                pdata.gR200m_excl = rc;
            }
        if (pdata.gRBN98_excl == 0 && EncMass >= 0.01 * pdata.gmass)
            if (rhoval > mBN98val)
            {
                pdata.gMBN98_excl = EncMass;
                pdata.gRBN98_excl = rc;
            }
...
void SetSphericalOverdensityMasstoTotalMass(Options &opt, PropData &pdata)
{
    if (pdata.gRvir == 0)
    {
        pdata.gMvir = pdata.gmass;
        pdata.gRvir = pdata.gsize;
    }
    if (pdata.gR200c == 0)
    {
        pdata.gM200c = pdata.gmass;
        pdata.gR200c = pdata.gsize;
    }
    if (pdata.gR200m == 0)
    {
        pdata.gM200m = pdata.gmass;
        pdata.gR200m = pdata.gsize;
    }
    if (pdata.gR500c == 0)
    {
        pdata.gM500c = pdata.gmass;
        pdata.gR500c = pdata.gsize;
    }
    if (pdata.gRBN98 == 0)
    {
        pdata.gMBN98 = pdata.gmass;
        pdata.gRBN98 = pdata.gsize;
    }
    for (auto iso = 0; iso < opt.SOnum; iso++)
        if (pdata.SO_radius[iso] == 0)
        {
...
{
    if (pdata.gRvir_excl == 0)
    {
        pdata.gMvir_excl = pdata.gmass;
        pdata.gRvir_excl = pdata.gsize;
    }
    if (pdata.gR200c_excl == 0)
    {
        pdata.gM200c_excl = pdata.gmass;
        pdata.gR200c_excl = pdata.gsize;
    }
    if (pdata.gR200m_excl == 0)
    {
        pdata.gM200m_excl = pdata.gmass;
        pdata.gR200m_excl = pdata.gsize;
    }
    if (pdata.gRBN98_excl == 0)
    {
        pdata.gMBN98_excl = pdata.gmass;
        pdata.gRBN98_excl = pdata.gsize;
    }
}
