...
inline Double_t SkewGauss(Double_t x, void *param)
{
    ...
    if (x <= params[1])
        return params[0] * exp(-0.5 * (x - params[1]) * (x - params[1]) / (params[2] * params[3]));
    else
...
inline Double_t DiffSkewGaussAmp(Double_t x, void *param)
{
    ...
    if (x <= params[1])
        return exp(-0.5 * dx2 * ivar * is2);
    else
        return exp(-0.5 * dx2 * ivar);
}
inline Double_t DiffSkewGaussMean(Double_t x, void *param)
{
    ...
    Double_t  is2    = 1.0 / params[3];
    if (x < params[1])
        return params[0] * exp(-0.5 * dx2 * ivar * is2) * (x - params[1]) * ivar * is2;
    else if (x == params[1])
        return 0.;
    else
        return params[0] * exp(-0.5 * dx2 * ivar) * (x - params[1]) * ivar;
}
inline Double_t DiffSkewGaussVar(Double_t x, void *param)
{
    ...
    Double_t  is2    = 1.0 / params[3];
    if (x <= params[1])
        return params[0] * exp(-0.5 * dx2 * ivar * is2) * dx2 * ivar * ivar * is2 * 0.5;
    else
        return params[0] * exp(-0.5 * dx2 * ivar) * dx2 * ivar * ivar * 0.5;
}
inline Double_t DiffSkewGaussSkew(Double_t x, void *param)
{
    ...
    Double_t  is2    = 1.0 / params[3];
    if (x <= params[1])
        return params[0] * exp(-0.5 * dx2 * ivar * is2) * dx2 * ivar * is2 * is2 * 0.5;
    else
        return 0.0;
}
//@}
...
{
    Double_t *params = (Double_t *)param;
    return params[0] * exp(-0.5 * (x - params[1]) * (x - params[1]) / (params[2])) * (x - params[1]) * (x - params[1]) /
