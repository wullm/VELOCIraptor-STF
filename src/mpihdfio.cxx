...
/*!
    ...
    only read tasks should call this routine. It is tricky to get appropriate load balancing and correct number of
   particles per processor.\n

    I could use recursive binary splitting like kd-tree along most spread axis till have appropriate number of volumes
   corresponding to number of processors. Or build a Peno-Hilbert space filling curve.
...
void MPIDomainExtentHDF(Options &opt)
{
    ...
    {
        if (opt.num_files > 1)
            sprintf(buf, "%s.0.hdf5", opt.fname);
        else
        ...
        */
        ...
        {
            mpi_xlim[i][0] = 0;
            mpi_xlim[i][1] = hdf_header_info.BoxSize;
        }
    }
...
void MPINumInDomainHDF(Options &opt)
{
    ...
    if (ireadtask[ThisTask] >= 0)
    {
        ...
        for (i = 0; i < opt.num_files; i++)
        {
            ...
            if (opt.num_files > 1)
                sprintf(buf, "%s.%d.hdf5", opt.fname, i);
            else
                sprintf(buf, "%s.hdf5", opt.fname);
            // Open the specified file and the specified dataset in the file.
            ...
            {
                vlongbuff =
                    read_attribute_v<long long>(Fhdf[i], hdf_header_info[i].names[hdf_header_info[i].INuminFile]);
