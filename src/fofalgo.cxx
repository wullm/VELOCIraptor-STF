...
int FOFStreamwithprobLX(Particle &a, Particle &b, Double_t *params)
{
    ...
    for (int j = 0; j < 3; j++)
    {
        ds1 += (a.GetPosition(j) - b.GetPosition(j)) * (a.GetPosition(j) - b.GetPosition(j)) /
               (params[6] * 0.25 * (1.0 + a.GetVelocity(j) * a.GetVelocity(j) / v1) *
                (1.0 + a.GetVelocity(j) * a.GetVelocity(j) / v1));
        ds2 += (a.GetPosition(j) - b.GetPosition(j)) * (a.GetPosition(j) - b.GetPosition(j)) /
               (params[6] * 0.25 * (1.0 + b.GetVelocity(j) * b.GetVelocity(j) / v2) *
                (1.0 + b.GetVelocity(j) * b.GetVelocity(j) / v2));
    }
    v1    = sqrt(v1);
    v2    = sqrt(v2);
    vnorm = 1.0 / v1 / v2;
...
int FOFStreamwithprobNNLX(Particle &a, Particle &b, Double_t *params)
{
    ...
    {
        ds1 += (a.GetPosition(j) - b.GetPosition(j)) * (a.GetPosition(j) - b.GetPosition(j)) /
               (dist2 * 0.25 * (1.0 + a.GetVelocity(j) * a.GetVelocity(j) / v1) *
                (1.0 + a.GetVelocity(j) * a.GetVelocity(j) / v1));
        ds2 += (a.GetPosition(j) - b.GetPosition(j)) * (a.GetPosition(j) - b.GetPosition(j)) /
               (dist2 * 0.25 * (1.0 + b.GetVelocity(j) * b.GetVelocity(j) / v2) *
                (1.0 + b.GetVelocity(j) * b.GetVelocity(j) / v2));
    }
    v1    = sqrt(v1);
    v2    = sqrt(v2);
...
int FOFStreamwithprobscaleell(Particle &a, Particle &b, Double_t *params)
{
    ...
    if (a.GetMass() <= b.GetMass())
        ellscale = params[6] * pow(a.GetMass() / params[10], (Double_t)(2. / 3.0));
    else
        ellscale = params[6] * pow(b.GetMass() / params[10], (Double_t)(2. / 3.0));
    for (int j = 0; j < 3; j++)
    {
...
int FOFStreamwithprobscaleellNN(Particle &a, Particle &b, Double_t *params)
{
    ...
    // use smaller ellscale where linking length scales with mass relative to smallest mass params[10]
    if (a.GetMass() <= b.GetMass())
        ellscale = params[6] * pow(2.1, 2.0 * log(a.GetMass() / params[10]) / log(2.0) / 3.0);
    else
        ellscale = params[6] * pow(2.1, 2.0 * log(a.GetMass() / params[10]) / log(2.0) / 3.0);
    for (int j = 0; j < 3; j++)
    {
...
int FOFPositivetypes(Particle &a, Particle &b, Double_t *params) { return (a.GetType() >= 0 && b.GetType() >= 0); }

int FOFchecksub(Particle &a, Double_t *params)
{
    if (a.GetPotential() >= params[9])
        return 0;
    else
        return -1;
}
