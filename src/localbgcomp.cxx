 ...
 *  \brief this file contains routines to compare the local velocity density function of a particle relative to that
 predicted by the backgound and then calculates the normalized deviation or outlier value ellprob \todo once ratio is
 calculated, must figure out best way to do global mpi fit. Probably best to combine the binned distribution and fit
 that
 */

#include "stf.h"

/*! This calculates the logarithmic ratio of the measured velocity density and the expected velocity density assuming a
   bg muiltivariate gaussian distribution \todo must adjust interpolation scheme so that if NN has cells in a
   neighbouring MPI domain, the information is stored locally. This may require a rewrite of the grid cell structure or
   the near neighbour list so that if grid cell has NN in another processor, actually physically store the information
   cm, cmvel, veldisp locally to that grid cell. Another option is to determine all cells that are NN of a cell in
   another mpi's domain, build a grid export list that contains the relevant information and an index that is easily
   accessible when searching the list of NN cells locally.
*/
...
#pragma omp for schedule(dynamic) nowait
    ...
    // if using MPI since number of cells is far fewer than number of particles, simple gather collect all the data so
    // that each processor has access to it
#ifdef USEMPI
    if (opt.iSingleHalo)
    {
        Ngridlocal = ngrid;
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, w, wsum, sv, vsv, fbg, vp, maxdist, vmweighted, isvweighted, tid, \
                                             tempdenv)
    {
#pragma omp for schedule(dynamic) nowait
#endif
        ...
        tid = 0;
            ...
            for (int j = 0; j < 3; j++)
                for (int k = 0; k < 3; k++) isvweighted(j, k) = 0.0;
            ...
            for (int j = 0; j <= MAXNGRID; j++)
            {
                w = (maxdist - dist[tid][j]) / (maxdist * dist[tid][j]);
                w = w * w;
                // w=1.0/dist[tid][j];
                wsum += w;
            ...
            vsv = 0.;
            for (int m = 0; m < 3; m++)
                for (int n = 0; n < 3; n++) vsv += vp[m] * vp[n] * isvweighted(m, n);
            fbg = log(sv) - 0.5 * vsv;
...
#pragma omp for
    ...
    // now bin data and find initial estimates for most probable value and the FWHM on either side of the most probable
    // value deltar=(rmax-rmin)/(Double_t)nbins;
    deltar = (4.0 * fabs(rmin)) / (Double_t)nbins;
    rmin -= deltar * 0.025;
    deltar *= 1.05;
    // for (i=0;i<nbins;i++) rbin[i]=0;
    for (int j = 0; j < nthreads; j++)
        for (i = 0; i < nbins; i++) omp_rbin[j][i] = 0;
    mtot = 0;
#ifdef USEOPENMP
    if (nbodies > ompperiodnum)
    {
...
#pragma omp for private(i, tid, w, ir) reduction(+ : mtot)
...
#endif
    for (int j = 1; j < nthreads; j++)
        for (i = 0; i < nbins; i++) omp_rbin[0][i] += omp_rbin[j][i];
    rbin = &omp_rbin[0][0];
    ...
    for (i = iprob; i >= 0; i--)
    {
        if (rbin[i] <= exp(-0.5 * sl * sl) * rbin[iprob])
        {
            ...
            sdlow =
                (meanr -
                 (((exp(-0.5 * sl * sl) * rbin[iprob] - rbin[jprob]) / (rbin[jprob + 1] - rbin[jprob]) + jprob + 0.5) *
                      deltar +
                  rmin)) /
                sl;
            break;
        }
        if (i == 0) { sdlow = (iprob - jprob) * deltar / sl; }
    }
    for (i = iprob; i < nbins; i++)
    {
        ...
        {
            jprob  = i;
            sdhigh = ((((exp(-0.5 * sl * sl) * rbin[iprob] - rbin[jprob - 1]) / (rbin[jprob] - rbin[jprob - 1]) +
                        jprob + 0.5) *
                           deltar +
                       rmin) -
                      meanr) /
    ...
    // if object is small or bg search (ie sublevel==-1, then to keep statistics high, use preliminary determination of
    ...
    do
    {
        ...
        int npeak = 0;
        for (i = 0; i < nbodies; i++)
            if (Part[i].GetPotential() >= rmin && Part[i].GetPotential() < rmax) npeak++;
        // delete[] rbin;
        // delete[] xbin;
    ...
    for (i = 0; i < nbins; i++)
    {
        if (rbin[i] != 0)
            ...
            W(i, i) =
                1.0 /
                minprob; // simple Poisson weighting with value in bin as mean and Poisson distributed about this mean
    }
    meanr = (iprob + 0.5) * deltar + rmin;
    sl    = 0.9;
    for (i = iprob; i >= 0; i--)
    ...
    {
        chi2 =
            FitNonLinLS(fitfunc, difffuncs, nparams, params, covar, nbins, xbin, rbin, &W, 1e-2, 0.95, fixp[i], 1, 20);
        int ifitfail = 0;
        ...
        {
            meanr  = params[1];
            sdlow  = sqrt(params[2] * params[3]);
            sdhigh = sqrt(params[2]);
            nfix   = 0;
            for (int j = 0; j < nparams; j++) nfix += (fixp[i][j] == 1);
            oldchi2 = chi2;
            if (opt.iverbose > 2)
                printf("chi2/dof=%e/%d, A=%e mu=%e var=%e s=%e\n", chi2, nbins - (nparams - nfix) - 1, params[0],
                       params[1], sqrt(params[2]), sqrt(params[3]));
        }
        else if (oldchi2 < chi2)
            break;
        else
        {
            ...
            params[0] = maxprob;
            params[1] = meanr;
            params[2] = sdhigh * sdhigh;
            params[3] = (sdlow * sdlow) / (sdhigh * sdhigh);
        }
    }
...
/*! Calculates the normalized deviations from the mean of the dominated population.
    \todo note that before had FOFSTPROB set density to probability, but here set to ell, the normalized logaritmic
   "distance" from predicted maxwellian velocity density) but could add routine that transforms these values to
   probablity if necessary.
...
Int_t GetOutliersValues(Options &opt, const Int_t nbodies, Particle *Part, int sublevel)
{
    ...
    // printf("Using GLOBAL values to characterize the distribution and determine the normalized values used to
    // determine outlier likelihood\n");
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, tempell) firstprivate(temp1, temp2, temp3)
    {
