 ...
 *  \brief this file contains routines used with MPI compilation.
    ...
    MPI routines generally pertain to domain decomposition or to specific MPI tasks that determine what needs to be
 broadcast between various threads.
...
/*!
    ...
    only ThisTask==0 should call this routine. It is tricky to get appropriate load balancing and correct number of
   particles per processor.\n

    I could use recursive binary splitting like kd-tree along most spread axis till have appropriate number of volumes
   corresponding to number of processors.

    NOTE: assume that cannot store data so position information is read Nsplit times to determine boundaries of
   subvolumes could also randomly subsample system and produce tree from that should store for each processor the node
   structure generated by the domain decomposition what I could do is read file twice, one to get extent and other to
   calculate entropy then decompose along some primary axis, then choose orthogonal axis, keep iterating till have
   appropriate number of subvolumes then store the boundaries of the subvolume. this means I don't store data but get at
   least reasonable domain decomposition

    NOTE: pkdgrav uses orthoganl recursive bisection along with kd-tree, gadget-2 uses peno-hilbert curve to map
   particles and oct-trees the question with either method is guaranteeing load balancing. for ORB achieved by splitting
   (sub)volume along a dimension (say one with largest spread or max entropy) such that either side of the cut has
   approximately the same number of particles (ie: median splitting). But for both cases, load balancing requires
   particle information so I must load the system then move particles about to ensure load balancing.
...
/// determine the initial domains, ie: bisection distance mpi_dxsplit, which is used to determien what processor a
/// particle is assigned to here the domains are constructured in data units
void MPIInitialDomainDecomposition()
{
    ...
    if (ThisTask == 0)
    {
            ...
            if (mpi_nxsplit[iy] > 1)
                for (j = 0; j < mpi_nxsplit[iy]; j++)
                {
                    ...
                    if (mpi_nxsplit[iz] > 1)
                        ...
                        {
                            bndval[2] =
                                (mpi_xlim[iz][1] - mpi_xlim[iz][0]) * (Double_t)(k + 1) / (Double_t)mpi_nxsplit[iz];
        ...
        for (int j = 0; j < NProcs; j++)
        {
            ...
            cout.precision(10);
            for (int k = 0; k < 3; k++)
                cout << k << " " << mpi_domain[j].bnd[k][0] << " " << mpi_domain[j].bnd[k][1] << " | ";
            cout << endl;
        }
    }
    // broadcast data
...
void MPINumInDomain(Options &opt)
{
    ...
    if (opt.inputtype == IOTIPSY)
        MPINumInDomainTipsy(opt);
    else if (opt.inputtype == IOGADGET)
        MPINumInDomainGadget(opt);
    else if (opt.inputtype == IORAMSES)
        MPINumInDomainRAMSES(opt);
#ifdef USEHDF
    else if (opt.inputtype == IOHDF)
        MPINumInDomainHDF(opt);
#endif
...
void MPIDomainDecomposition(Options &opt)
{
    ...
    if (opt.inputtype == IOTIPSY)
        MPIDomainDecompositionTipsy(opt);
    else if (opt.inputtype == IOGADGET)
...
void MPIAdjustDomain(Options opt)
{
    ...
    for (int j = 0; j < NProcs; j++)
        for (int k = 0; k < 3; k++)
        {
            mpi_domain[j].bnd[k][0] *= lscale;
            mpi_domain[j].bnd[k][1] *= lscale;
        }
}
...
int MPIGetParticlesProcessor(Double_t x, Double_t y, Double_t z)
{
    ...
    for (int j = 0; j < NProcs; j++)
    {
        if ((mpi_domain[j].bnd[0][0] <= x) && (mpi_domain[j].bnd[0][1] >= x) && (mpi_domain[j].bnd[1][0] <= y) &&
            (mpi_domain[j].bnd[1][1] >= y) && (mpi_domain[j].bnd[2][0] <= z) && (mpi_domain[j].bnd[2][1] >= z))
            return j;
    }
    cerr << ThisTask << " has particle outside the mpi domains of every process (" << x << "," << y << "," << z << ")"
         << endl;
                                       ...
                                       vector<Particle> *&Preadbuf)
{
    ...
    else
    {
        ...
        else if (ireadtask[ibuf] >= 0)
        {
            if (ibuf != ThisTask)
            {
                if (Nreadbuf[ireadtask[ibuf]] == Preadbuf[ireadtask[ibuf]].size())
                    Preadbuf[ireadtask[ibuf]].resize(Preadbuf[ireadtask[ibuf]].size() + BufSize);
...
int MPIInDomain(Double_t xsearch[3][2], Double_t bnd[3][2])
{
    ...
    else
    {
        ...
        else
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                if (!((bnd[j][1] < xsearch[j][0] + mpi_period) || (bnd[j][0] > xsearch[j][1] + mpi_period)))
                {
                    xsearchp[j][0] += mpi_period;
                    xsearchp[j][1] += mpi_period;
                }
                else if (!((bnd[j][1] < xsearch[j][0] - mpi_period) || (bnd[j][0] > xsearch[j][1] - mpi_period)))
                {
                    xsearchp[j][0] -= mpi_period;
                    xsearchp[j][1] -= mpi_period;
                }
...
int MPISearchForOverlap(Coordinate &x, Double_t &rdist)
{
    ...
    for (auto k = 0; k < 3; k++)
    {
        xsearch[k][0] = x[k] - rdist;
        xsearch[k][1] = x[k] + rdist;
    }
    return MPISearchForOverlap(xsearch);
}
...
int MPISearchForOverlap(Double_t xsearch[3][2])
{
    ...
    {
        for (k = 0; k < 3; k++)
            if (xsearch[k][0] < 0 || xsearch[k][1] > mpi_period) ireflec[numreflecs++] = k;
        if (numreflecs == 1)
            numreflecchoice = 1;
            ...
            numreflecchoice = 7;
        for (j = 0; j < numreflecchoice; j++)
            for (k = 0; k < 3; k++)
            {
                xsearchp[j][k][0] = xsearch[k][0];
                xsearchp[j][k][1] = xsearch[k][1];
            }
        if (numreflecs == 1)
        {
        ...
        for (j = 0; j < NProcs; j++)
            for (k = 0; k < numreflecchoice; k++)
            {
                if (j != ThisTask)
                {
                    if (!((mpi_domain[j].bnd[0][1] < xsearchp[k][0][0]) ||
                          (mpi_domain[j].bnd[0][0] > xsearchp[k][0][1]) ||
                          (mpi_domain[j].bnd[1][1] < xsearchp[k][1][0]) ||
                          (mpi_domain[j].bnd[1][0] > xsearchp[k][1][1]) ||
                          (mpi_domain[j].bnd[2][1] < xsearchp[k][2][0]) ||
                          (mpi_domain[j].bnd[2][0] > xsearchp[k][2][1])))
                        numoverlap++;
                }
            }
    }
...
int MPISearchForOverlapUsingMesh(Options &opt, Double_t xsearch[3][2])
{
    ...
    for (auto j : celllist)
    {
        ...
        /// Only check if particles have overlap with neighbouring cells that are on another MPI domain and have not
...
//@{
/// Distribute the mpi processes that read the input files so as to spread the read threads evenly throughout the
/// MPI_COMM_WORLD
void MPIDistributeReadTasks(Options &opt, int *&ireadtask, int *&readtaskID)
{
    ...
    for (int i = 0; i < opt.nsnapread; i++)
    {
        ireadtask[i * spacing] = i;
        readtaskID[i]          = i * spacing;
    }
}
...
//@{
/// for all threads not reading snapshots, simply receive particles as necessary from all threads involved with reading
/// the data
                                        ...
                                        Particle *&Pbaryons)
{
    ...
    MPI_Status status;
    // for all threads not reading snapshots, simply receive particles as necessary from all threads involved with
    // reading the data first determine which threads are going to send information to this thread.
    for (i = 0; i < opt.nsnapread; i++)
        if (irecv[i])
        {
            mpi_irecvflag[i] = 0;
            MPI_Irecv(&Nlocalthreadbuf[i], 1, MPI_Int_t, readtaskID[i], ThisTask + NProcs, MPI_COMM_WORLD,
                      &mpi_request[i]);
        }
    Nlocaltotalbuf = 0;
            ...
            if (irecv[i])
            {
                if (mpi_irecvflag[i] == 0)
                {
                    ...
                    if (mpi_irecvflag[i])
                    {
                        ...
                        {
                            MPI_Recv(&Part[Nlocal], sizeof(Particle) * Nlocalthreadbuf[i], MPI_BYTE, readtaskID[i],
                                     ThisTask, MPI_COMM_WORLD, &status);
                            Nlocal += Nlocalthreadbuf[i];
                            Nlocaltotalbuf += Nlocalthreadbuf[i];
                            mpi_irecvflag[i] = 0;
                            MPI_Irecv(&Nlocalthreadbuf[i], 1, MPI_Int_t, readtaskID[i], ThisTask + NProcs,
                                        ...
                                        Int_t *&mpi_nsend_baryon)
{
    if (ireadtask[ThisTask] >= 0)
    {
        ...
        {
            // if there are an even number of read tasks, then communicate such that 0 communcates with N-1, 1<->N-2,
            // etc and moves on to next communication 0<->N-2, 1<->N-3, etc with the communication in chunks first base
            // on read thread position
            ...
            if (sendTask != recvTask &&
                (mpi_nsend[ThisTask * NProcs + recvTask] > 0 || mpi_nsend[recvTask * NProcs + ThisTask] > 0))
            {
                ...
                do
                {
                    ...
                    // blocking point-to-point send and receive. Here must determine the appropriate offset point in the
                    // local export buffer for sending data and also the local appropriate offset in the local the
                    // receive buffer for information sent from the local receiving buffer
                    MPI_Sendrecv(&Pbuf[nreadoffset[ireadtask[recvTask]] + sendoffset],
                                 sizeof(Particle) * cursendchunksize, MPI_BYTE, recvTask, TAG_IO_A + isendrecv,
                                 &Part[Nlocal], sizeof(Particle) * currecvchunksize, MPI_BYTE, recvTask,
                                 TAG_IO_A + isendrecv, MPI_COMM_WORLD, &status);
            ...
            if (opt.iBaryonSearch && opt.partsearchtype != PSTALL)
            {
                ...
                do
                {
                    ...
                    // receive buffer for information sent from the local receiving buffer
                    MPI_Sendrecv(
                        &Pbuf[nreadoffset[ireadtask[recvTask]] + mpi_nsend[ThisTask * NProcs + recvTask] + sendoffset],
                        sizeof(Particle) * cursendchunksize, MPI_BYTE, recvTask, TAG_IO_B + isendrecv,
                        &Pbaryons[Nlocalbaryon[0]], sizeof(Particle) * currecvchunksize, MPI_BYTE, recvTask,
                        TAG_IO_B + isendrecv, MPI_COMM_WORLD, &status);
...
void MPISendParticlesBetweenReadThreads(Options &opt, vector<Particle> *&Preadbuf, Particle *Part, int *&ireadtask,
                                        int *&readtaskID, Particle *&Pbaryons, MPI_Comm &mpi_comm_read,
                                        Int_t *&mpi_nsend_readthread, Int_t *&mpi_nsend_readthread_baryon)
{
    if (ireadtask[ThisTask] >= 0)
    {
        ...
        for (ibuf = 0; ibuf < opt.nsnapread; ibuf++)
        {
            ...
            if (ibuf > 0 && recvTask == 0) icycle = 1;
            // if sendtask!=recvtask, and information needs to be sent, send information
            if (sendTask != recvTask && (mpi_nsend_readthread[sendTask * opt.nsnapread + recvTask] > 0 ||
                                         mpi_nsend_readthread[recvTask * opt.nsnapread + sendTask] > 0))
            {
                ...
                do
                {
                    ...
                    // local export buffer for sending data and also the local appropriate offset in the local the
                    // receive buffer for information sent from the local receiving buffer
                    MPI_Sendrecv(&Preadbuf[recvTask][sendoffset], sizeof(Particle) * cursendchunksize, MPI_BYTE,
                                 recvTask, TAG_IO_A + isendrecv, &(Part[Nlocal]), sizeof(Particle) * currecvchunksize,
                                 MPI_BYTE, recvTask, TAG_IO_A + isendrecv, mpi_comm_read, &status);
                    Nlocal += currecvchunksize;
                    sendoffset += cursendchunksize;
                    recvoffset += currecvchunksize;
                    isendrecv++;
            ...
            if (opt.iBaryonSearch && opt.partsearchtype != PSTALL)
            {
                ...
                do
                {
                    ...
                    // receive buffer for information sent from the local receiving buffer
                    MPI_Sendrecv(
                        &Preadbuf[recvTask][mpi_nsend_readthread[sendTask * opt.nsnapread + recvTask] + sendoffset],
                        sizeof(Particle) * cursendchunksize, MPI_BYTE, recvTask, TAG_IO_B + isendrecv,
                        &Pbaryons[Nlocalbaryon[0]], sizeof(Particle) * currecvchunksize, MPI_BYTE, recvTask,
                        TAG_IO_B + isendrecv, mpi_comm_read, &status);
                    Nlocalbaryon[0] += currecvchunksize;
                    sendoffset += cursendchunksize;
                    recvoffset += currecvchunksize;
                    isendrecv++;
...
void MPIGetExportNum(const Int_t nbodies, Particle *Part, Double_t rdist)
{
    ...
    ///\em Or outer loop is over threads, inner loop over nbodies and just have a idlist of size Nlocal that tags
    ///particles which must be exported. Then its a much quicker follow up loop (no if statement) that stores the data
    for (j = 0; j < NProcs; j++) nsend_local[j] = 0;
    for (i = 0; i < nbodies; i++)
    {
        for (int k = 0; k < 3; k++)
        {
            xsearch[k][0] = Part[i].GetPosition(k) - rdist;
            xsearch[k][1] = Part[i].GetPosition(k) + rdist;
        }
        for (j = 0; j < NProcs; j++)
        {
            if (j != ThisTask)
...
/*! Determine which particles have a spatial linking length such that linking overlaps the domain of another processor
...
void MPIBuildParticleExportList(const Int_t nbodies, Particle *Part, Int_t *&pfof, Int_tree_t *&Len, Double_t rdist)
{
    ...
    // and then gather the number of particles to be sent from mpi thread m to mpi thread n in the
    // mpi_nsend[NProcs*NProcs] array via [n+m*NProcs]
    MPI_Allgather(nsend_local, NProcs, MPI_Int_t, mpi_nsend, NProcs, MPI_Int_t, MPI_COMM_WORLD);
    NImport = 0;
    for (j = 0; j < NProcs; j++) NImport += mpi_nsend[ThisTask + j * NProcs];
    // now send the data.
    ...
    for (j = 0; j < NProcs; j++)
    {
        if (j != ThisTask)
        {
            sendTask = ThisTask;
            recvTask = j;
            if (mpi_nsend[ThisTask + recvTask * NProcs] >= maxNumPart || nsend_local[recvTask] >= maxNumPart)
                bufferFlag++;
        }
    }
    ...
    if (bufferFlag)
    {
        ...
        for (j = 0; j < NProcs; j++)
        {
            ...
            if (nsend_local[j] > 0) numBuffersToSend[j] = (nsend_local[j] / numPartInBuffer) + 1;
        }
        ...
        for (int i = 0; i < NProcs; i++)
            if (numBuffersToRecv[i] > maxnbufferslocal) maxnbufferslocal = numBuffersToRecv[i];
        for (int i = 0; i < NProcs; i++)
            if (numBuffersToSend[i] > maxnbufferslocal) maxnbufferslocal = numBuffersToSend[i];
        ...
        for (int i = 1; i < NProcs; i++)
        {
            ...
            for (int jj = 0; jj < numBuffersToSend[dst] - 1; jj++)
            {
                ...
                MPI_Isend(&FoFDataIn[noffset[dst] + buffOffset], sizeof(struct fofdata_in) * size, MPI_BYTE, dst,
                          (int)(TAG_FOF_A * maxnbuffers + jj + 1), MPI_COMM_WORLD, &rqst);
                MPI_Isend(&PartDataIn[noffset[dst] + buffOffset], sizeof(Particle) * size, MPI_BYTE, dst,
                          (int)(TAG_FOF_B * maxnbuffers * 3 + jj + 1), MPI_COMM_WORLD, &rqst);
                buffOffset += size;
            }
            ...
            if (size > 0 && numBuffersToSend[dst] > 0)
            {
                ...
                MPI_Isend(&FoFDataIn[noffset[dst] + buffOffset], sizeof(struct fofdata_in) * size, MPI_BYTE, dst,
                          (int)(TAG_FOF_A * maxnbuffers + numBuffersToSend[dst]), MPI_COMM_WORLD, &rqst);
                MPI_Isend(&PartDataIn[noffset[dst] + buffOffset], sizeof(Particle) * size, MPI_BYTE, dst,
                          (int)(TAG_FOF_B * maxnbuffers * 3 + numBuffersToSend[dst]), MPI_COMM_WORLD, &rqst);
            }
            // Receive Buffers
            buffOffset = 0;
            for (int jj = 0; jj < numBuffersToRecv[src]; jj++)
            {
                ...
                MPI_Recv(&FoFDataGet[nbuffer[src] + buffOffset], sizeof(struct fofdata_in) * numInBuffer, MPI_BYTE, src,
                         (int)(TAG_FOF_A * maxnbuffers + jj + 1), MPI_COMM_WORLD, &status);
                MPI_Recv(&PartDataGet[nbuffer[src] + buffOffset], sizeof(Particle) * numInBuffer, MPI_BYTE, src,
                         (int)(TAG_FOF_B * maxnbuffers * 3 + jj + 1), MPI_COMM_WORLD, &status);
        ...
        if (nexport > 0 || nimport > 0)
        {
            for (j = 0; j < NProcs; j++) // for(j=1;j<NProcs;j++)
            {
                if (j != ThisTask)
                {
                    ...
                    nbuffer[recvTask] = 0;
                    for (int k = 0; k < recvTask; k++)
                        nbuffer[recvTask] += mpi_nsend[ThisTask + k * NProcs]; // offset on local receiving buffer
                    if (mpi_nsend[ThisTask * NProcs + recvTask] > 0 || mpi_nsend[recvTask * NProcs + ThisTask] > 0)
                    {
                        // blocking point-to-point send and receive. Here must determine the appropriate offset point in
                        // the local export buffer for sending data and also the local appropriate offset in the local
                        // the receive buffer for information sent from the local receiving buffer first send FOF data
                        // and then particle data
                        MPI_Sendrecv(&FoFDataIn[noffset[recvTask]], nsend_local[recvTask] * sizeof(struct fofdata_in),
                                     MPI_BYTE, recvTask, TAG_FOF_A, &FoFDataGet[nbuffer[recvTask]],
                                     mpi_nsend[ThisTask + recvTask * NProcs] * sizeof(struct fofdata_in), MPI_BYTE,
                                     recvTask, TAG_FOF_A, MPI_COMM_WORLD, &status);
                        MPI_Sendrecv(&PartDataIn[noffset[recvTask]], nsend_local[recvTask] * sizeof(Particle), MPI_BYTE,
                                     recvTask, TAG_FOF_B, &PartDataGet[nbuffer[recvTask]],
                                     mpi_nsend[ThisTask + recvTask * NProcs] * sizeof(Particle), MPI_BYTE, recvTask,
                                     TAG_FOF_B, MPI_COMM_WORLD, &status);
                    }
                }
            }
        }
...
/*! like \ref MPIGetExportNum but number based on NN search, useful for reducing memory costs at the expense of cpu
 * cycles
 */
void MPIGetNNExportNum(const Int_t nbodies, Particle *Part, Double_t *rdist)
{
    ...
    for (i = 0; i < nbodies; i++)
    {
        ...
        for (int k = 0; k < 3; k++)
        {
            xsearch[k][0] = Part[i].GetPosition(k) - rdist[i];
            xsearch[k][1] = Part[i].GetPosition(k) + rdist[i];
        }
        for (j = 0; j < NProcs; j++)
        {
            if (j != ThisTask)
...
/*
void MPIGetNNExportNumUsingMesh(Options &opt, const Int_t numleafnodes, vector<leaf_node_info> &leafnodes, KDTree
*&tree){ Int_t i, j,nthreads,nexport=0,nimport=0; Int_t nsend_local[NProcs],noffset[NProcs],nbuffer[NProcs]; Double_t
xsearch[3][2]; Int_t sendTask,recvTask; MPI_Status status; int indomain; int *sent_mpi_domain = new int[NProcs]; int
inode;
...
particles
    ...
    for (i=0;i<nbodies;i++)
    {
        ...
        for(int k=0;k<3;k++)
{xsearch[k][0]=Part[i].GetPosition(k)-leafnodes[inode].searchdist;xsearch[k][1]=Part[i].GetPosition(k)+leafnodes[inode].searchdist;}
        vector<int> celllist=MPIGetCellListInSearchUsingMesh(opt,xsearch);
        for (auto j:celllist) {
            const int cellnodeID = opt.cellnodeids[j];
            /// Only check if particles have overlap with neighbouring cells that are on another MPI domain and have not
already been sent to if (sent_mpi_domain[cellnodeID] == 1) continue; vector<int>
celllist=MPIGetCellListInSearchUsingMesh(opt,xsearch); for (auto j:celllist) { const int cellnodeID =
opt.cellnodeids[j];
                /// Only check if particles have overlap with neighbouring cells that are on another MPI domain and have
not already been sent to if (sent_mpi_domain[cellnodeID] == 1) continue; nexport++; nsend_local[cellnodeID]++;
                sent_mpi_domain[cellnodeID]++;
            }
        }
    }
    //and then gather the number of particles to be sent from mpi thread m to mpi thread n in the
mpi_nsend[NProcs*NProcs] array via [n+m*NProcs] MPI_Allgather(nsend_local, NProcs, MPI_Int_t, mpi_nsend, NProcs,
MPI_Int_t, MPI_COMM_WORLD); NImport=0; for (j=0;j<NProcs;j++)NImport+=mpi_nsend[ThisTask+j*NProcs]; NExport=nexport;
}
*/
#endif

/*! like \ref MPIBuildParticleExportList but each particle has a different distance stored in rdist used to find nearest
 * neighbours
 */
void MPIBuildParticleNNExportList(const Int_t nbodies, Particle *Part, Double_t *rdist)
{
    ...
    if (nexport > 0)
    {
        std::sort(NNDataIn, NNDataIn + nexport,
                  [](const nndata_in &a, const nndata_in &b) { return a.ToTask < b.ToTask; });
    }
    ...
    MPI_Allgather(nsend_local, NProcs, MPI_Int_t, mpi_nsend, NProcs, MPI_Int_t, MPI_COMM_WORLD);
    // now send the data.
    ///\todo In determination of particle export, eventually need to place a check for the communication buffer so that
    ///if exported number is larger than the size of the buffer, iterate over the number exported
            ...
            if (j != ThisTask)
            {
                ...
                if (mpi_nsend[ThisTask * NProcs + recvTask] > 0 || mpi_nsend[recvTask * NProcs + ThisTask] > 0)
                {
                    ...
                    for (auto ichunk = 0; ichunk < numsendrecv; ichunk++)
                    {
                        // blocking point-to-point send and receive. Here must determine the appropriate offset point in
                        // the local export buffer for sending data and also the local appropriate offset in the local
                        // the receive buffer for information sent from the local receiving buffer
                        MPI_Sendrecv(&NNDataIn[noffset[recvTask] + sendoffset],
                                     cursendchunksize * sizeof(struct nndata_in), MPI_BYTE, recvTask, TAG_NN_A + ichunk,
                                     &NNDataGet[nbuffer[recvTask] + recvoffset],
                                     currecvchunksize * sizeof(struct nndata_in), MPI_BYTE, recvTask, TAG_NN_A + ichunk,
                                     MPI_COMM_WORLD, &status);
                        sendoffset += cursendchunksize;
                        recvoffset += currecvchunksize;
                        if (cursendchunksize > mpi_nsend[recvTask + ThisTask * NProcs] - sendoffset)
                            cursendchunksize = mpi_nsend[recvTask + ThisTask * NProcs] - sendoffset;
                        if (currecvchunksize > mpi_nsend[ThisTask + recvTask * NProcs] - sendoffset)
                            currecvchunksize = mpi_nsend[ThisTask + recvTask * NProcs] - recvoffset;
                    }
                }
            }
        }
        ...
        for (j = 0; j < NProcs; j++) // for(j=1;j<NProcs;j++)
        {
            if (j != ThisTask)
            {
                ...
                if (mpi_nsend[ThisTask * NProcs + recvTask] > 0 || mpi_nsend[recvTask * NProcs + ThisTask] > 0)
                {
                    ...
                    // local export buffer for sending data and also the local appropriate offset in the local the
                    // receive buffer for information sent from the local receiving buffer
                    MPI_Sendrecv(&NNDataIn[noffset[recvTask]], nsend_local[recvTask] * sizeof(struct nndata_in),
                                 MPI_BYTE, recvTask, TAG_NN_A, &NNDataGet[nbuffer[recvTask]],
                                 mpi_nsend[ThisTask + recvTask * NProcs] * sizeof(struct nndata_in), MPI_BYTE, recvTask,
                                 TAG_NN_A, MPI_COMM_WORLD, &status);
                }
            }
        }
    }
...
/*! Mirror to \ref MPIGetNNExportNum, use exported particles, run ball search to find number of all local particles that
   need to be imported back to exported particle's thread so that a proper NN search can be made.
*/
...
#pragma omp parallel
    ...
    for (j = 0; j < NProcs; j++)
    {
        ...
        // search local list and tag all local particles that need to be exported back (or imported) to the exported
        // particles thread
    ...
    for (j = 0; j < NProcs; j++)
        for (int k = 0; k < NProcs; k++) oldnsend[k + j * NProcs] = mpi_nsend[k + j * NProcs];
    MPI_Allgather(nsend_local, NProcs, MPI_Int_t, mpi_nsend, NProcs, MPI_Int_t, MPI_COMM_WORLD);
    NImport = 0;
    for (j = 0; j < NProcs; j++) NImport += mpi_nsend[ThisTask + j * NProcs];
    NExport = nexport;
    for (j = 0; j < NProcs; j++)
        for (int k = 0; k < NProcs; k++) mpi_nsend[k + j * NProcs] = oldnsend[k + j * NProcs];
}

/*! Mirror to \ref MPIBuildParticleNNExportList, use exported particles, run ball search to find all local particles
   that need to be imported back to exported particle's thread so that a proper NN search can be made.
*/
...
#pragma omp parallel
        ...
        if (j != ThisTask)
        {
            ...
            if (mpi_nsend[ThisTask * NProcs + recvTask] > 0 || mpi_nsend[recvTask * NProcs + ThisTask] > 0)
            {
                ...
                for (auto ichunk = 0; ichunk < numsendrecv; ichunk++)
                {
                    ...
                    // local export buffer for sending data and also the local appropriate offset in the local the
                    // receive buffer for information sent from the local receiving buffer
                    MPI_Sendrecv(&PartDataIn[noffset[recvTask] + sendoffset], cursendchunksize * sizeof(Particle),
                                 MPI_BYTE, recvTask, TAG_NN_B + ichunk, &PartDataGet[nbuffer[recvTask] + recvoffset],
                                 currecvchunksize * sizeof(Particle), MPI_BYTE, recvTask, TAG_NN_B + ichunk,
                                 MPI_COMM_WORLD, &status);
                    sendoffset += cursendchunksize;
                    recvoffset += currecvchunksize;
                    if (cursendchunksize > mpi_nsend[recvTask + ThisTask * NProcs] - sendoffset)
    ...
    ncount = 0;
    for (int k = 0; k < NProcs; k++) ncount += mpi_nsend[ThisTask + k * NProcs];
    return ncount;
}
...
vector<bool> MPIGetHaloSearchExportNum(const Int_t ngroup, PropData *&pdata, vector<Double_t> &rdist)
{
    ...
    for (i = 1; i <= ngroup; i++)
    {
        ...
        for (int k = 0; k < 3; k++)
        {
            xsearch[k][0] = pdata[i].gcm[k] - rdist[i];
            xsearch[k][1] = pdata[i].gcm[k] + rdist[i];
        }
        for (j = 0; j < NProcs; j++)
        {
            if (j != ThisTask)
                                  ...
                                  vector<bool> &halooverlap)
{
        ...
        for (j = 0; j < NProcs; j++)
        {
            if (j != ThisTask)
            {
                ...
                if (MPIInDomain(xsearch, mpi_domain[j].bnd))
                {
                    ...
                    NNDataIn[nexport].R2       = rdist[i] * rdist[i];
                    // NNDataIn[nexport].V2=vdist2[i];
                    for (int k = 0; k < 3; k++) { NNDataIn[nexport].Pos[k] = pdata[i].gcm[k]; }
                    nexport++;
                    nsend_local[j]++;
                }
            }
    ...
    for (j = 1, noffset[0] = 0; j < NProcs; j++) noffset[j] = noffset[j - 1] + nsend_local[j - 1];
    // and then gather the number of items to be sent from mpi thread m to mpi thread n in the mpi_nsend[NProcs*NProcs]
    // array via [n+m*NProcs]
    MPI_Allgather(nsend_local, NProcs, MPI_Int_t, mpi_nsend, NProcs, MPI_Int_t, MPI_COMM_WORLD);
    // now send the data.
        ...
        if (j != ThisTask)
        {
            ...
            if (mpi_nsend[ThisTask * NProcs + recvTask] > 0 || mpi_nsend[recvTask * NProcs + ThisTask] > 0)
            {
                ...
                for (auto ichunk = 0; ichunk < numsendrecv; ichunk++)
                {
                    ...
                    // local export buffer for sending data and also the local appropriate offset in the local the
                    // receive buffer for information sent from the local receiving buffer
                    MPI_Sendrecv(&NNDataIn[noffset[recvTask] + sendoffset], cursendchunksize * sizeof(struct nndata_in),
                                 MPI_BYTE, recvTask, TAG_NN_A + ichunk, &NNDataGet[nbuffer[recvTask] + recvoffset],
                                 currecvchunksize * sizeof(struct nndata_in), MPI_BYTE, recvTask, TAG_NN_A + ichunk,
                                 MPI_COMM_WORLD, &status);
                    sendoffset += cursendchunksize;
                    recvoffset += currecvchunksize;
                                           ...
                                           vector<bool> &halooverlap)
{
    ...
    // then store the offset in the export data for the jth Task in order to send data.
    noffset[0] = 0;
    for (auto j = 1; j < NProcs; j++) noffset[j] = noffset[j - 1] + nsend_local[j - 1];
    // and then gather the number of items to be sent from mpi thread m to mpi thread n in the mpi_nsend[NProcs*NProcs]
    // array via [n+m*NProcs]
    MPI_Allgather(nsend_local, NProcs, MPI_Int_t, mpi_nsend, NProcs, MPI_Int_t, MPI_COMM_WORLD);
    // now send the data.
...
/*! Mirror to \ref MPIGetHaloSearchExportNum, use exported positions, run ball search to find number of all local
   particles that need to be imported back to exported positions's thread so that a proper search can be made.
*/
...
/*! Mirror to \ref MPIBuildHaloSearchExportList, use exported particles, run ball search to find all local particles
   that need to be imported back to exported particle's thread so that a proper NN search can be made.
*/
...
/*! Similar to \ref MPIBuildParticleExportList, however this is for associated baryon search where particles have been
   moved from original mpi domains and their group id accessed through the id array and their stored id and length in
   numingroup
*/
                                            ...
                                            Int_t *numingroup, Double_t rdist)
{
    ...
    for (j = 0; j < NProcs; j++) NImport += mpi_nsend[ThisTask + j * NProcs];
    // now send the data.
    ///\todo In determination of particle export for FOF routines, eventually need to place a check for the
    ///communication buffer so that if exported number is larger than the size of the buffer, iterate over the number
    /// exported
    for (j = 0; j < NProcs; j++) nimport += mpi_nsend[ThisTask + j * NProcs];
    if (nexport > 0 || nimport > 0)
    {
                ...
                if (mpi_nsend[ThisTask * NProcs + recvTask] > 0 || mpi_nsend[recvTask * NProcs + ThisTask] > 0)
                {
                    ...
                    for (auto ichunk = 0; ichunk < numsendrecv; ichunk++)
                    {
                        ...
                        MPI_Sendrecv(&FoFDataIn[noffset[recvTask] + sendoffset],
                                     cursendchunksize * sizeof(struct fofdata_in), MPI_BYTE, recvTask,
                                     TAG_FOF_A + ichunk, &FoFDataGet[nbuffer[recvTask] + recvoffset],
                                     currecvchunksize * sizeof(struct fofdata_in), MPI_BYTE, recvTask,
                                     TAG_FOF_A + ichunk, MPI_COMM_WORLD, &status);
                        MPI_Sendrecv(&PartDataIn[noffset[recvTask] + sendoffset], cursendchunksize * sizeof(Particle),
                                     MPI_BYTE, recvTask, TAG_FOF_B + ichunk,
                                     &PartDataGet[nbuffer[recvTask] + recvoffset], currecvchunksize * sizeof(Particle),
...
///\todo alter to that this now ranks threads so that group ids are larger if thread has more particles. This ensures
///that mpi threads send particles to thread with the fewest particles when linking across mpi domains during a FOF
/// search
void MPIAdjustLocalGroupIDs(const Int_t nbodies, Int_t *pfof)
{
    ...
    for (int j = 0; j < NProcs; j++)
    {
        rankorder[NProcs - 1 - j] = pq->TopQueue();
        pq->Pop();
    }
    Int_t offset = 0;
    for (int j = 0; j < NProcs; j++)
    {
        if (rankorder[j] == ThisTask) break;
        offset += mpi_nlocal[rankorder[j]];
    }
    // Int_t offset=nbodies*ThisTask;
    for (Int_t i = 0; i < nbodies; i++)
        if (pfof[i] > 0) pfof[i] += offset;
    mpi_maxgid = 0;
    ...
    {
        if (rankorder[j] == ThisTask) break;
        mpi_gidoffset += mpi_ngroups[rankorder[j]];
    }
}

// My idea is this for doing the stiching. First generate export list of particle data and another seperate data
// structure for the FOF data Next, when examining local search using export particles (since iGroup=0 is unlinked) if a
// export particle current iGroup is larger Then adjust the local particle and all members of its group so long as its
// group is NOT group zero. Calculate the number of new links and determine the total number of new links across all mpi
// threads. If that number is not zero, then groups have been found that are across processor domains. One has to iterate
// the check across the domains till no more new links have been found. That is must update the export particles Group
// ids then begin the check anew.

// Couple of key things to think about are, one I really shouldn't have to run the check again to find the particles
// that meet the conditions across threads since that has NOT changed. must figure out a way to store relevant particles.
// Otherwise, continuously checking, seems a waste of cpu cycles. second I must pass along head, tail, next, and length
// information, maybe by using a plist structure so that it is easy to alter the particles locally to new group id Also
// must determine optimal way of setting which processor the group should end up on. Best way might be to use the length
// of the group locally since that would minimize the broadcasts.
...
void MPIUpdateExportList(const Int_t nbodies, Particle *Part, Int_t *&pfof, Int_tree_t *&Len)
{
    ...
    for (j = 0; j < NProcs; j++)
    {
        ...
        nsend_local[j] = mpi_nsend[j + ThisTask * NProcs];
    }
    for (j = 1, noffset[0] = 0; j < NProcs; j++) noffset[j] = noffset[j - 1] + nsend_local[j - 1];
    for (i = 0; i < nexport; i++)
    {
        ...
        if (j != ThisTask)
        {
            ...
            if (mpi_nsend[ThisTask * NProcs + recvTask] > 0 || mpi_nsend[recvTask * NProcs + ThisTask] > 0)
            {
                ...
                for (auto ichunk = 0; ichunk < numsendrecv; ichunk++)
                {
                    ...
                    // receive buffer for information sent from the local receiving buffer
                    MPI_Sendrecv(&FoFDataIn[noffset[recvTask] + sendoffset],
                                 cursendchunksize * sizeof(struct fofdata_in), MPI_BYTE, recvTask, TAG_FOF_A + ichunk,
                                 &FoFDataGet[nbuffer[recvTask] + recvoffset],
                                 currecvchunksize * sizeof(struct fofdata_in), MPI_BYTE, recvTask, TAG_FOF_A + ichunk,
                                 MPI_COMM_WORLD, &status);
                    sendoffset += cursendchunksize;
                    recvoffset += currecvchunksize;
                    if (cursendchunksize > mpi_nsend[recvTask + ThisTask * NProcs] - sendoffset)
...
/*! This routine searches the local particle list using the positions of the exported particles to see if any local
   particles met the linking criterion and any other FOF criteria of said exported particle. If that is the case, then
   the group id of the local particle and all other particles that belong to the same group are adjusted if the group id
   of the exported particle is smaller. This routine returns the number of links found between the local particles and
   all other exported particles from all other mpi domains. \todo need to update lengths if strucden flag used to limit
   particles for which real velocity density calculated
*/
...
Int_t MPIGroupExchange(const Int_t nbodies, Particle *Part, Int_t *&pfof)
{
    ...
    for (j = 0; j < NProcs; j++) nsend_local[j] = 0;
    // first determine how big a local array is needed to store linked particles and broadcast information to create new
    // nsend array
    nlocal = 0;
    for (i = 0; i < nbodies; i++)
    {
        if (mpi_foftask[i] != ThisTask) nsend_local[mpi_foftask[i]]++;
    }
    MPI_Allgather(nsend_local, NProcs, MPI_Int_t, mpi_nsend, NProcs, MPI_Int_t, MPI_COMM_WORLD);
    nexport = nimport = 0;
    for (j = 0; j < NProcs; j++)
    ...
    NImport = nimport;
    if (nexport > 0)
        FoFGroupDataExport = new fofid_in[nexport];
    else
        FoFGroupDataExport = new fofid_in[1];
    ...
    for (j = 0; j < NProcs; j++)
    {
        noffset_import[j] = 0;
        if (j != ThisTask)
            for (int k = 0; k < j; k++) noffset_import[j] += mpi_nsend[ThisTask + k * NProcs];
    }
    ...
    ///\todo In determination of particle export for FOF routines, eventually need to place a check for the
    ///communication buffer so that if exported number is larger than the size of the buffer, iterate over the number
    /// exported \todo ideally need to implement MPI Type defs and serialization using boost
            ...
            if (mpi_nsend[ThisTask * NProcs + recvTask] > 0 || mpi_nsend[recvTask * NProcs + ThisTask] > 0)
            {
                ...
                for (auto ichunk = 0; ichunk < numsendrecv; ichunk++)
                {
                    MPI_Sendrecv(&FoFGroupDataExport[noffset_export[recvTask] + sendoffset],
                                 cursendchunksize * sizeof(struct fofid_in), MPI_BYTE, recvTask, TAG_FOF_C + ichunk,
                                 &FoFGroupDataLocal[noffset_import[recvTask] + recvoffset],
                                 currecvchunksize * sizeof(struct fofid_in), MPI_BYTE, recvTask, TAG_FOF_C + ichunk,
                                 MPI_COMM_WORLD, &status);
                    sendoffset += cursendchunksize;
                    recvoffset += currecvchunksize;
                    if (cursendchunksize > mpi_nsend[recvTask + ThisTask * NProcs] - sendoffset)
...
Int_t MPIBaryonGroupExchange(const Int_t nbodies, Particle *Part, Int_t *&pfof)
{
    ...
    for (i = 0; i < nbodies; i++) Part[i].SetID(i);
    Int_t *storeval = new Int_t[nbodies];
    // if trying to reduce memory allocation,  if nlocal < than the memory allocated adjust local list so that all
    // particles to be exported are near the end. and allocate the appropriate memory for pfof and mpi_idlist. otherwise,
    // need to actually copy the particle data into FoFGroupDataLocal and proceed as normal, storing info, send info,
    // delete particle array, allocate a new array large enough to store info and copy over info
    ///\todo eventually I should replace arrays with vectors so that the size can change, removing the need to free and
    ///allocate
...
Int_t MPIBaryonCompileGroups(const Int_t nbodies, Particle *Part, Int_t *&pfof, Int_t minsize, int iorder)
{
    ...
    else
    {
        ...
        // now sort again which will put particles group then id order, and determine size of groups and their current
        // group id;
        qsort(FoFGroupDataLocal, nbodies, sizeof(struct fofid_in), fof_id_cmp);
        numingroup = new Int_t[ngroups + 1];
        plist      = new Int_t *[ngroups + 1];
    ...
    for (i = 1; i <= ngroups; i++)
        if (numingroup[i] > 0) delete[] plist[i];
    delete[] plist;
    delete[] numingroup;
...
/// Determine which exported dm particle is closest in phase-space to a local baryon particle and assign that particle
/// to the group of that dark matter particle if is closest particle
                       ...
                       Double_t *localdist, Int_t nsearch, Double_t *param, Double_t *period)
{
    ...
    if (NImport > 0)
    {
        // now dark matter particles associated with a group existing on another mpi domain are local and can be
        // searched.
        KDTree *mpitree =
            new KDTree(PartDataGet, NImport, nsearch / 2, mpitree->TPHYS, mpitree->KEPAN, 100, 0, 0, 0, period);
        if (nsearch > NImport) nsearch = NImport;
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, j, k, tid, p1, pindex, x1, D2, dval, rval, nnID, dist2)
        {
                    ...
                    for (j = 0; j < nsearch; j++)
                    {
                        ...
                        if (numingroup[pfofbaryons[i]] < FoFDataGet[pindex].iLen)
                        {
                            if (fofcmp(p1, PartDataGet[nnID[j]], param))
                            {
                                for (k = 0; k < 3; k++)
                                {
                                    D2 += (p1.GetPosition(k) - PartDataGet[nnID[j]].GetPosition(k)) *
                                              (p1.GetPosition(k) - PartDataGet[nnID[j]].GetPosition(k)) / param[6] +
                                          (p1.GetVelocity(k) - PartDataGet[nnID[j]].GetVelocity(k)) *
                                              (p1.GetVelocity(k) - PartDataGet[nnID[j]].GetVelocity(k)) / param[7];
                                }
#ifdef GASON
                                D2 += p1.GetU() / param[7];
#endif
                                ...
                                {
                                    dval           = D2;
                                    pfofbaryons[i] = FoFDataGet[pindex].iGroup;
                                    rval           = dist2[j];
                                    mpi_foftask[i] = FoFDataGet[pindex].iGroupTask;
                                }
                            }
                        }
                    }
...
Int_t MPIBaryonExchange(const Int_t nbaryons, Particle *Pbaryons, Int_t *pfofbaryons)
{
    ...
    NImport = nimport;
    FoFGroupDataExport =
        new fofid_in[nexport +
                     1]; //+1 just buffer to ensure that if nothing broadcast, easy to allocate and deallocate memory

    // if trying to reduce memory allocation, then need to check amount stored locally and how much that needs to be
    // adjusted by if nlocal < than the memory allocated adjust local list so that all particles to be exported are near
    // the end. and allocate the appropriate memory for pfofbaryons and mpi_idlist. otherwise, need to actually copy the
    // particle data into FoFGroupDataLocal and proceed as normal, storing info, send info, delete particle array,
    // allocate a new array large enough to store info and copy over info
    ///\todo eventually I should replace arrays with vectors so that the size can change, removing the need to free and
    ///allocate
...
/// This alters the group ids by an offset determined by the number of groups on all previous mpi threads so that the
/// group has a unique group id. Prior to this, group ids are determined locally.
inline void MPIAdjustGroupIDs(const Int_t nbodies, Int_t *pfof)
{
    Int_t noffset = 0;
    for (int j = 0; j < ThisTask; j++) noffset += mpi_ngroups[j];
    for (Int_t i = 0; i < nbodies; i++)
        if (pfof[i] > 0) pfof[i] += noffset;
}
...
void MPICollectFOF(const Int_t nbodies, Int_t *&pfof)
{
    ...
    if (ThisTask == 0)
    {
        ...
        for (int j = 1; j < NProcs; j++)
            if (maxnlocal < mpi_nsend[j * NProcs]) maxnlocal = mpi_nsend[j * NProcs];
        delete[] pfof;
    ...
    for (int j = 1; j < NProcs; j++)
    {
        ...
        if (ThisTask == 0)
        {
            ...
            MPI_Recv(mpi_indexlist, mpi_nsend[sendTask * NProcs], MPI_Int_t, sendTask, TAG_FOF_E, MPI_COMM_WORLD,
                     &status);
...
void MPIBuildGridData(const Int_t ngrid, GridCell *grid, Coordinate *gvel, Matrix *gveldisp)
{
    ...
    for (j = 0; j < NProcs; j++)
    {
        if (j != ThisTask)
        {
            ...
            MPI_Sendrecv(grid, Ngridlocal * sizeof(struct GridCell), MPI_BYTE, recvTask, TAG_GRID_A,
                         &mpi_grid[noffset[recvTask]],
                         mpi_nsend[ThisTask + recvTask * NProcs] * sizeof(struct GridCell), MPI_BYTE, recvTask,
                         TAG_GRID_A, MPI_COMM_WORLD, &status);
            MPI_Sendrecv(gvel, Ngridlocal * sizeof(struct Coordinate), MPI_BYTE, recvTask, TAG_GRID_B,
                         &mpi_gvel[noffset[recvTask]],
                         mpi_nsend[ThisTask + recvTask * NProcs] * sizeof(struct Coordinate), MPI_BYTE, recvTask,
                         TAG_GRID_B, MPI_COMM_WORLD, &status);
            MPI_Sendrecv(gveldisp, Ngridlocal * sizeof(struct Matrix), MPI_BYTE, recvTask, TAG_GRID_C,
                         &mpi_gveldisp[noffset[recvTask]],
                         mpi_nsend[ThisTask + recvTask * NProcs] * sizeof(struct Matrix), MPI_BYTE, recvTask,
                         TAG_GRID_C, MPI_COMM_WORLD, &status);
        }
    }
}
//@}
...
//@{
/// comprasion function used to sort particles for export so that all particles being exported to the same processor are
/// in a contiguous block and well ordered \todo, I should remove this and determine the ordering before hand.
int fof_export_cmp(const void *a, const void *b)
{
    if (((struct fofdata_in *)a)->Task < (((struct fofdata_in *)b)->Task)) return -1;
    if (((struct fofdata_in *)a)->Task > (((struct fofdata_in *)b)->Task)) return +1;
    return 0;
}

/// comprasion function used to sort particles for export so that all particles being exported to the same processor are
/// in a contiguous block and well ordered
int nn_export_cmp(const void *a, const void *b)
{
    if (((struct nndata_in *)a)->ToTask < (((struct nndata_in *)b)->ToTask)) return -1;
    if (((struct nndata_in *)a)->ToTask > (((struct nndata_in *)b)->ToTask)) return +1;
    return 0;
}

/// comprasion function used to sort grouped particles so that easy to determine total number of groups locally, size of
/// groups, etc.
int fof_id_cmp(const void *a, const void *b)
{
    if (((struct fofid_in *)a)->iGroup > (((struct fofid_in *)b)->iGroup)) return -1;

    if (((struct fofid_in *)a)->iGroup < (((struct fofid_in *)b)->iGroup)) return +1;

    if (((struct fofid_in *)a)->p.GetType() < (((struct fofid_in *)b)->p.GetType())) return -1;

    if (((struct fofid_in *)a)->p.GetType() > (((struct fofid_in *)b)->p.GetType())) return +1;

    if (((struct fofid_in *)a)->p.GetID() < (((struct fofid_in *)b)->p.GetID())) return -1;
    ...
    for (auto ix = ixstart; ix <= ixend; ix++)
    {
        for (auto iy = iystart; iy <= iyend; iy++)
        {
            for (auto iz = izstart; iz <= izend; iz++)
            {
                index = 0;
                if (iz < 0)
                    index += opt.numcellsperdim + iz;
                else if (iz >= opt.numcellsperdim)
                    index += iz - opt.numcellsperdim;
                else
                    index += iz;
                if (iy < 0)
                    index += (opt.numcellsperdim + iy) * opt.numcellsperdim;
                else if (iy >= opt.numcellsperdim)
                    index += (iy - opt.numcellsperdim) * opt.numcellsperdim;
                else
                    index += iy * opt.numcellsperdim;
                if (ix < 0)
                    index += (opt.numcellsperdim + ix) * opt.numcellsperdim * opt.numcellsperdim;
                else if (ix >= opt.numcellsperdim)
                    index += (ix - opt.numcellsperdim) * opt.numcellsperdim * opt.numcellsperdim;
                else
                    index += ix * opt.numcellsperdim * opt.numcellsperdim;
            ...
            if (j != ThisTask)
            {
                ...
                if (mpi_nsend[ThisTask * NProcs + recvTask] > 0 || mpi_nsend[recvTask * NProcs + ThisTask] > 0)
                {
                    ...
                    for (auto ichunk = 0; ichunk < numsendrecv; ichunk++)
                    {
                        ...
                        // the local export buffer for sending data and also the local appropriate offset in the local
                        // the receive buffer for information sent from the local receiving buffer
                        MPI_Sendrecv(&PartBufSend[noffset[recvTask] + sendoffset], cursendchunksize * sizeof(Particle),
                                     MPI_BYTE, recvTask, TAG_SWIFT_A + ichunk,
                                     &PartBufRecv[nbuffer[recvTask] + recvoffset], currecvchunksize * sizeof(Particle),
