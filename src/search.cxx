...
#pragma omp parallel
    ...
    param[0] = tree->TPHYS;
    param[1] =
        (opt.ellxscale * opt.ellxscale) * (opt.ellphys * opt.ellphys) * (opt.ellhalophysfac * opt.ellhalophysfac);
    param[6] = param[1];
    ...
    if (numompregions >= 4 && nthreads > 1 && opt.iopenmpfof == 1)
    {
        ...
        if (opt.iverbose)
            cout << ThisTask << ": finished building " << numompregions << " domains and trees " << MyGetTime() - time3
                 << endl;
    }
    else
    {
        ...
        if (opt.iverbose)
            cout << ThisTask << ": finished building single tree with single OpenMP " << MyGetTime() - time3 << endl;
    }
    ...
    cout << ThisTask << " Parameters used are : ellphys=" << sqrt(param[6]) << " Lunits (ell^2=" << param[6]
         << " and likely " << sqrt(param[6]) / opt.ellxscale << " in interparticle spacing" << endl;
    ...
    if (numompregions >= 4 && nthreads > 1 && opt.iopenmpfof == 1)
    {
        ...
        numgroups = OpenMPLocalSearch(opt, nbodies, Part, pfof, storeorgIndex, Head, Next, tree3dfofomp, param, rdist,
                                      ompminsize, fofcmp, numompregions, ompdomain);
        if (opt.iverbose)
            cout << ThisTask << ": finished omp local search of " << numompregions << " containing total of "
                 << numgroups << " groups " << MyGetTime() - time3 << endl;
        if (numgroups > 0)
        {

            // then for each omp region determine the particles to "import" from other
            // omp regions
            ompimport = OpenMPImportParticles(opt, nbodies, Part, pfof, storeorgIndex, numompregions, ompdomain, rdist,
                                              omp_nrecv_total, omp_nrecv_offset, omp_import_total);
            if (omp_import_total > 0)
            {
                OpenMPLinkAcross(opt, nbodies, Part, pfof, storeorgIndex, Head, Next, param, fofcheck, numompregions,
                                 ompdomain, tree3dfofomp, omp_nrecv_total, omp_nrecv_offset, ompimport);
...
#pragma omp for schedule(dynamic) nowait
        ...
        if (numgroups > 0) { numgroups = OpenMPResortParticleandGroups(nbodies, Part, pfof, minsize); }
    ...
    else
    {
        ...
        if (opt.partsearchtype == PSTALL && opt.iBaryonSearch > 1)
            pfof = tree->FOFCriterionSetBasisForLinks(fofcmp, param, numgroups, minsize, iorder, 0, FOFchecktype, Head,
                                                      Next);
        else
    ...
    if (opt.partsearchtype == PSTALL && opt.iBaryonSearch > 1)
        pfof =
            tree->FOFCriterionSetBasisForLinks(fofcmp, param, numgroups, minsize, iorder, 0, FOFchecktype, Head, Next);
    else
        pfof = tree->FOF(sqrt(param[1]), numgroups, minsize, iorder, Head, Next);
#endif
    ...
    if (numgroups > 0 && (opt.iSubSearch == 1 && opt.foftype != FOF6DCORE))
    {
        ...
        if (!(opt.iBaryonSearch >= 1 && opt.partsearchtype == PSTALL))
            for (i = 0; i < nbodies; i++) Part[i].SetType(numingroup[pfof[Part[i].GetID()]] >= MINSUBSIZE);
        ...
        for (i = 0; i < nbodies; i++)
            if (Part[i].GetType() > 0) numinstrucs++;
    ...
    else
    {
        ...
        if (opt.iverbose >= 2)
        {
            ...
            cout << ThisTask << " has found locally " << numgroups << " with lower min size of " << minsize
                 << ", with  " << sum << " particles in all groups" << endl;
        }
        if (numgroups)
        {
            numingroup = BuildNumInGroup(nbodies, numgroups, pfof);
        ...
        cout << ThisTask << ": Finished local search, nexport/nimport = " << NExport << " " << NImport << " in "
        ...
        do
        {
            ...
            {
                links_across =
                    MPILinkAcross(nbodies, tree, Part.data(), pfof, Len, Head, Next, param[1], fofcheck, param);
            }
            else
            {
            ...
            if (opt.iverbose >= 2)
            {
                cout << ThisTask << " has found " << links_across << " links to particles on other mpi domains "
                     << endl;
            }
        ...
        if (ThisTask == 0)
            cout << ThisTask << ": finished linking across MPI domains in " << MyGetTime() - time2 << endl;
    ...
    if (opt.iverbose >= 2)
    {
        ...
        cout << ThisTask << " has found after full search " << numgroups << " with lower min size of " << minsize
             << ", with  " << sum << " particles in all groups" << endl;
    }
    ...
    if (totalgroups > 0 && (opt.iSubSearch == 1 && opt.foftype != FOF6DCORE))
    {
        ...
        if (opt.iverbose)
            cout << ThisTask << " has " << numinstrucs << " particles for which density must be calculated" << endl;
    ...
    if (opt.fofbgtype <= FOF6D && totalgroups > 0)
    {
        ...
        if (opt.fofbgtype == FOF6D && opt.iKeepFOF == 0)
        {
            ...
            cout << "Parameters used are : ellphys=" << sqrt(param[6]) << " Lunits, ellvel=" << sqrt(param[7])
                 << " Vunits." << endl;
        }
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, vscale2, mtotregion, vx, vy, vz, vmean)
            {
#pragma omp for schedule(dynamic, 1) nowait
                ...
                for (i = 1; i <= numgroups; i++)
                {
                    ...
                    {
                        for (int k = 0; k < 3; k++)
                            vscale2 += pow(Part[j + noffset[i]].GetVelocity(k) - vmean[k], 2.0) *
...
#pragma omp for schedule(dynamic, 1) nowait
            ...
            tid = 0;
                ...
                */
                ...
                for (Int_t j = 0; j < numingroup[i]; j++) { Part[noffset[i] + j].ScalePhase(xscaling, vscaling); }
                xscaling     = 1.0 / xscaling;
                vscaling     = 1.0 / vscaling;
                treeomp[tid] = new KDTree(&(Part.data()[noffset[i]]), numingroup[i], opt.Bsize, treeomp[tid]->TPHS,
                                          tree->KEPAN, 100);
                pfofomp[i]   = treeomp[tid]->FOF(1.0, ngomp[i], minsize, 1, &Head[noffset[i]], &Next[noffset[i]],
        ...
        if (opt.iKeepFOF)
        {
            if (opt.iverbose >= 2 && ThisTask == 0)
                cout << "Storing the 3D fof envelopes of the 6d fof structures found" << endl;
            ...
            for (i = 1; i <= numgroups; i++)
            {
                numin3dfof = 0;
                for (int j = 0; j < numingroup[i]; j++)
                    numin3dfof += (pfof6dfof[ids[Part[noffset[i] + j].GetID() + noffset[i]]] == 0);
                if (numin3dfof == 0)
                {
                    opt.num3dfof--;
                    i3dfofleft[i] = 0;
            ...
            for (i = 0; i < Nlocal; i++)
                if (pfof6dfof[i] != 0) pfof[i] = pfof6dfof[i] + opt.num3dfof;
    ...
    else
    {
        ...
        if (ng > 0)
        {
            ...
            for (i = opt.num3dfof + 1; i <= numgroups; i++)
                if (maxval < numingroup[i]) maxval = numingroup[i];
            ...
            ReorderGroupIDsAndArraybyValue(numgroups, numgroups, numingroup, pfof, pglist, value6d3d,
                                           id_3dfof_of_6dfof);
 ...
 */
Int_t *SearchSubset(Options &opt, const Int_t nbodies, const Int_t nsubset, Particle *Partsubset, Int_t &numgroups,
                    Int_t sublevel, Int_t *pnumcores)
{
    KDTree *    tree;
    Int_t *     pfof, i, ii;
    FOFcompfunc fofcmp;
    ...
    if (opt.foftype == FOFSTPROB)
    {
        if (opt.iverbose >= 2)
        {
            cout << "FOFSTPROB which uses: ellphys, vratio, thetaopen, and "
                    "ellthreshold.\n";
            cout << "Parameters used are : ellphys=" << sqrt(param[6]) << " Lunits, vratio=" << param[7]
...
#pragma omp for
    ...
    if (opt.iiterflag && numgroups > 0 && opt.foftype != FOF6DCORE)
    {
        ...
        Int_t numgrouplinks, *numgrouplinksIndex, **newIndex, *oldnumingroup, **newintergroupIndex,
            **intergroupgidIndex;
...
#pragma omp parallel
        ...
        if (opt.iverbose >= 2)
        {
            ...
            cout << ThisTask << " "
                 << "Parameters used are : ellphys=" << sqrt(param[6]) << " Lunits, vratio=" << param[7]
                 << ", cos(thetaopen)=" << param[8] << ", ";
        ...
        DetermineNewLinks(nsubset, Partsubset, pfof, numgroups, newlinks, newlinksIndex, numgrouplinksIndex, nnID[tid],
                          &newIndex);
        ...
        // unlinked)
        LinkUntagged(Partsubset, numgroups, pfof, numingroup, pglist, newlinks, numgrouplinksIndex, newIndex, Head,
                     Next, GroupTail, nnID[tid]);
        for (Int_t j = 1; j <= numgroups; j++) delete[] newIndex[j];

        for (i = 0; i < nsubset; i++)
            if (Partsubset[i].GetPotential() < param[9] && nnID[0][Partsubset[i].GetID()] == 0)
                nnID[0][Partsubset[i].GetID()] = -1;

        fofcmp   = &FOFStreamwithprob;
        param[1] = (opt.ellxscale * opt.ellxscale) * (opt.ellphys * opt.ellphys); // *opt.ellxfac*opt.ellxfac;//increase
        ...
        for (i = 1; i <= numgroups; i++)
        {
            ...
            do
            {
                if (Partsubset[ss].GetPotential() >= param[9]) { newlinksIndex[newlinks++] = ss; }
            } while ((ss = Next[ss]) >= 0);
        }
        ...
        {
            SearchForNewLinks(nsubset, tree, Partsubset, pfof, fofcmp, param, newlinks, newlinksIndex, nnID, nthreads);
            DetermineNewLinks(nsubset, Partsubset, pfof, numgroups, newlinks, newlinksIndex, numgrouplinksIndex,
                              nnID[tid], &newIndex);
            LinkUntagged(Partsubset, numgroups, pfof, numingroup, pglist, newlinks, numgrouplinksIndex, newIndex, Head,
                         Next, GroupTail, nnID[tid]);
            for (Int_t j = 1; j <= numgroups; j++) delete[] newIndex[j];
        ...
        for (i = 1; i <= numgroups; i++)
            if (igflag[i] == 0)
            {
                ...
                do
                {
                    if (Partsubset[ss].GetDensity() >= param[9]) { newlinksIndex[newlinks++] = ss; }
                } while ((ss = Next[ss]) >= 0);
            }
        for (ii = 0; ii < newlinks; ii++)
            nnID[0][Partsubset[newlinksIndex[ii]].GetID()] = pfof[Partsubset[newlinksIndex[ii]].GetID()];
        do
        {
            ...
            // search in parallel
            SearchForNewLinks(nsubset, tree, Partsubset, pfof, fofcmp, param, newlinks, newlinksIndex, nnID, nthreads);
            DetermineGroupLinks(nsubset, Partsubset, pfof, numgroups, newlinks, newlinksIndex, numgrouplinksIndex,
                                nnID[tid], &newIndex);
            DetermineGroupMergerConnections(Partsubset, numgroups, pfof, ilflag, numgrouplinksIndex,
                                            intergrouplinksIndex, nnID[tid], &newIndex, &newintergroupIndex,
                                            &intergroupgidIndex);
            newlinks = 0;
            mergers += MergeGroups(opt, Partsubset, numgroups, pfof, numingroup, oldnumingroup, pglist,
                                   numgrouplinksIndex, &intergroupgidIndex, &newintergroupIndex, intergrouplinksIndex,
                                   Head, Next, GroupTail, igflag, nnID[tid], newlinks, newlinksIndex);
        } while (mergers);

        param[1] = (opt.ellxscale * opt.ellxscale) * (opt.ellphys * opt.ellphys) * opt.ellxfac *
...
#pragma omp parallel
        ...
        for (i = 0; i < nsubset; i++)
            if (numingroup[pfof[i]] < opt.MinSize) pfof[i] = 0;
        for (i = 1; i <= numgroups; i++) numingroup[i] = 0;
        for (i = 0; i < nsubset; i++) numingroup[pfof[i]]++;
        ...
        for (i = 0; i < nsubset; i++)
            if (pfof[i] > 0) pglist[pfof[i]][numingroup[pfof[i]]++] = i;
        if (ng) ReorderGroupIDs(numgroups, ng, numingroup, pfof, pglist);
        for (i = 1; i <= numgroups; i++)
            if (numingroup[i] > 0) delete[] pglist[i];
        delete[] pglist;
        delete[] numingroup;
        numgroups = ng;
        if (opt.iverbose >= 2)
    ...
    if (nsubset >= MINSUBSIZE && opt.iLargerCellSearch && opt.foftype != FOF6DCORE)
    {
        ...
        if (numgroups > 0)
        {
            ...
            if (opt.iverbose >= 2)
                cout << ThisTask << " "
                     << "bg search using " << ngrid << " grid cells, with each node containing ~"
        ...
        if (numgroups > 0)
        {
            ...
            if (opt.iverbose >= 2)
            {
                ...
                cout << ThisTask << " "
                     << "Parameters used are : ellphys=" << sqrt(param[6]) << " Lunits, ellvel=" << sqrt(param[7])
                     << " Vunits." << endl;
                cout << ThisTask << " " << param[9] << " is outlier threshold and contains more than " << minsize
                     << endl;
            }
            ...
            if (numgroupsbg >= bgoffset + 1)
            {
                ...
                for (i = 0; i < nsubset; i++)
                    if (pfof[i] == bgoffset && pfofbg[i] > 0) pfof[i] = oldng + (pfofbg[i] - bgoffset);
...
#pragma omp parallel
                ...
                SearchForNewLinks(nsubset, tree, Partsubset, pfof, fofcmp, param, newlinks, newlinksIndex, nnID,
                                  nthreads);
                tid = 0;
                DetermineNewLinks(nsubset, Partsubset, pfof, numgroups, newlinks, newlinksIndex, numgrouplinksIndex,
                                  nnID[tid], &newIndex);
                LinkUntagged(Partsubset, numgroups, pfof, numingroup, pglist, newlinks, numgrouplinksIndex, newIndex,
                             Head, Next, GroupTail, nnID[tid]);
                for (Int_t j = 1; j <= numgroups; j++) delete[] newIndex[j];
                newlinks = 0;
                for (i = oldng + 1; i <= numgroups; i++)
                    if (numingroup[i] > 0)
                ...
                DetermineGroupLinks(nsubset, Partsubset, pfof, numgroups, oldlinks, oldlinksIndex, numgrouplinksIndex,
                                    nnID[tid], &newIndex);
                DetermineGroupMergerConnections(Partsubset, numgroups, pfof, ilflag, numgrouplinksIndex,
                                                intergrouplinksIndex, nnID[tid], &newIndex, &newintergroupIndex,
                                                &intergroupgidIndex);
                mergers +=
                    MergeGroups(opt, Partsubset, numgroups, pfof, numingroup, numingroup, pglist, numgrouplinksIndex,
                                &intergroupgidIndex, &newintergroupIndex, intergrouplinksIndex, Head, Next, GroupTail,
...
#pragma omp parallel
        ...
        if (numgroups > 0)
            if (opt.iverbose >= 2)
                cout << numgroups << " substructures found after large grid search" << endl;
            else
            {
                if (opt.iverbose >= 2)
                    cout << ThisTask << ": "
    ...
    if ((opt.iHaloCoreSearch > 0 &&
         ((!opt.iSingleHalo && sublevel <= maxhalocoresublevel) || (opt.iSingleHalo && sublevel == 0))) ||
        opt.foftype == FOF6DCORE)
    {
        ...
        if (opt.halocorenumloops > 1)
        {
            ...
            Double_t dispval =
                opt.halocorevfaciter * opt.halocorevfaciter * opt.halocorexfaciter * opt.halocorexfaciter;
            ...
            do
            {
                ...
                if (numgroupsbg > 1)
                {
                    numactiveloops++;
                    for (i = 0; i < nsubset; i++)
                        if (pfofbg[Partsubset[i].GetID()] == 1 && pfofbgnew[Partsubset[i].GetID()] <= 1 &&
                            pfof[Partsubset[i].GetID()] == 0)
                            Partsubset[i].SetType(numactiveloops);
        ...
        if (numgroupsbg >= bgoffset + 1)
        {
            ...
            {
                HaloCoreGrowth(opt, nsubset, Partsubset, pfof, pfofbg, numgroupsbg, param, dispfac, numactiveloops,
                               corelevel, nthreads);
                if (pnumcores != NULL) *pnumcores = numgroupsbg;
                if (numgroupsbg >= bgoffset + 1)
                {
                    for (i = 0; i < nsubset; i++)
                        if (pfofbg[i] > bgoffset) pfof[i] = numgroups + (pfofbg[i] - bgoffset);
                ...
                if (opt.iverbose >= 2)
                    cout << ThisTask << ": After 6dfof core search and assignment there are " << numgroups << " groups"
                         << endl;
            }
        }
        else
    ...
    if (numgroups > 0 && opt.coresubmergemindist > 0 && nsubset >= MINSUBSIZE)
        MergeSubstructuresPhase(opt, nsubset, Partsubset, pfof, numgroups, numsubs, numgroupsbg);
...
void HaloCoreGrowth(Options &opt, const Int_t nsubset, Particle *&Partsubset, Int_t *&pfof, Int_t *&pfofbg,
                    Int_t &numgroupsbg, Double_t param[], vector<Double_t> &dispfac, int numactiveloops,
                    vector<int> &corelevel, int nthreads)
{
    ...
    if (opt.iverbose >= 2)
    {
        ...
        for (i = 1; i <= numgroupsbg; i++)
            cout << i << " " << mcore[i] << " " << mcore[i] / mcore[1] << " " << ncore[i] << " " << dispfac[i] << endl;
    }
    ...
    if (nincore < nsubset)
    {
        ...
        if (opt.iPhaseCoreGrowth)
        {
            ...
            for (i = 1; i <= numgroupsbg; i++)
            {
                ...
                {
                    for (int k = 0; k < 6; k++)
                        Pcore[noffset[i] + j].SetPhase(k, Pcore[noffset[i] + j].GetPhase(k) - cmphase[i](k, 0));
                }
...
#pragma omp for reduction(+ : nreduce)
                ...
                if (opt.iPhaseCoreGrowth >= 2)
                {
                    ...
                    for (i = 1; i <= numgroupsbg; i++)
                        if (corelevel[i] >= iloop)
                        {
                            ...
                            {
                                for (int k = 0; k < 6; k++)
                                    Pcore[noffset[i] + j].SetPhase(k, Pcore[noffset[i] + j].GetPhase(k) -
                                                                          cmphase[i](k, 0));
                            }
                            CalcPhaseSigmaTensor(ncore[i], &Pcore[noffset[i]], invdisp[i]);
                            invdisp[i] = invdisp[i].Inverse();
        ...
        else
        {
            ...
            {
#pragma omp parallel default(shared) private(i, tid, Pval, x1, D2, dval, mval, pid, pidcore)
                {
#pragma omp for
                    ...
                    for (i = 0; i < nsubset; i++)
                    {
                        ...
                        if (pfofbg[pid] == 0 && pfof[pid] == 0)
                        {
                            ...
                            for (int k = 0; k < 3; k++)
                            {
                                dval += (Pval->GetPosition(k) - Pcore[pidcore].GetPosition(k)) *
                                            (Pval->GetPosition(k) - Pcore[pidcore].GetPosition(k)) / param[6] +
                                        (Pval->GetVelocity(k) - Pcore[pidcore].GetVelocity(k)) *
                                            (Pval->GetVelocity(k) - Pcore[pidcore].GetVelocity(k)) / param[7];
                            }
        ...
        if (opt.partsearchtype != PSTSTAR && opt.foftype != FOF6DCORE)
            mincoresize = max((Int_t)(nsubset * opt.halocorenfac),
                              (Int_t)opt.MinSize); // max((Int_t)(nsubset*MAXCELLFRACTION/2.0),(Int_t)opt.MinSize);
        else
...
void MergeSubstructuresCoresPhase(Options &opt, const Int_t nsubset, Particle *&Partsubset, Int_t *&pfof,
                                  Int_t &numsubs, Int_t &numcores)
{
    ...
    for (auto i = 0; i < nsubset; i++)
    {
        ...
        if (pfofval <= numsubs)
        {
            ...
            for (auto k = 0; k < 6; k++)
                subs[pfofval].SetPhase(k,
                                       subs[pfofval].GetPhase(k) + Partsubset[i].GetPhase(k) * Partsubset[i].GetMass());
        }
        else
        {
            pfofval -= numsubs + 1;
            ...
            for (auto k = 0; k < 6; k++)
                cores[pfofval].SetPhase(k, cores[pfofval].GetPhase(k) +
                                               Partsubset[i].GetPhase(k) * Partsubset[i].GetMass());
        }
    }
    noffset[0] = 0;
    for (auto i = 1; i <= numsubs + numcores; i++) noffset[i] = numingroup[i - 1] + noffset[i - 1];
    pfofval = 1;
    for (auto &x : subs)
    {
        x.SetPID(pfofval);
    ...
    sort(indexing.begin(), indexing.end(), [](indexfof &a, indexfof &b) { return a.fofval < b.fofval; });
    // get the dispersions
    for (auto i = 0; i < nsubset; i++)
    {
        pfofval = pfof[Partsubset[i].GetID()];
        ...
        if (pfofval <= numsubs)
        {
            pfofval -= 1;
            disp = 0;
            for (auto k = 0; k < 3; k++) disp += pow(Partsubset[i].GetPosition(k) - subs[pfofval].GetPosition(k), 2.0);
            sigXsubs[pfofval] += disp * Partsubset[i].GetMass();
            disp = 0;
            for (auto k = 0; k < 3; k++) disp += pow(Partsubset[i].GetVelocity(k) - subs[pfofval].GetVelocity(k), 2.0);
        ...
        else
        {
            pfofval -= numsubs + 1;
            disp = 0;
            for (auto k = 0; k < 3; k++) disp += pow(Partsubset[i].GetPosition(k) - cores[pfofval].GetPosition(k), 2.0);
            sigXcores[pfofval] += disp * Partsubset[i].GetMass();
            disp = 0;
            for (auto k = 0; k < 3; k++) disp += pow(Partsubset[i].GetVelocity(k) - cores[pfofval].GetVelocity(k), 2.0);
    ...
    for (auto i = 0; i < numcores; i++)
    {
        ...
        {
            disp = 0;
            for (auto k = 0; k < 3; k++) disp += pow(subs[taggedsubs[j]].GetPosition(k) - cores[i].GetPosition(k), 2.0);
            dist2 = disp / sigXcores[i];
            disp  = 0;
            for (auto k = 0; k < 3; k++) disp += pow(subs[taggedsubs[j]].GetVelocity(k) - cores[i].GetVelocity(k), 2.0);
...
void MergeSubstructuresPhase(Options &opt, const Int_t nsubset, Particle *&Partsubset, Int_t *&pfof, Int_t &numgroups,
                             Int_t &numsubs, Int_t &numcores)
{
    // get the phase centres of objects and see if they overlap
    ...
    for (auto i = 0; i < nsubset; i++)
    {
        ...
        subs[pfofval].SetMass(subs[pfofval].GetMass() + Partsubset[i].GetMass());
        for (auto k = 0; k < 6; k++)
            subs[pfofval].SetPhase(k, subs[pfofval].GetPhase(k) + Partsubset[i].GetPhase(k) * Partsubset[i].GetMass());
    }
    noffset[0] = 0;
    for (auto i = 1; i <= numgroups; i++) noffset[i] = numingroup[i - 1] + noffset[i - 1];
    pfofval = 1;
    for (auto &x : subs)
    {
        x.SetPID(pfofval);
    ...
    for (auto i = 0; i < numgroups; i++)
    {
        ...
        for (auto j = 0; j < taggedsubs.size(); j++)
        {
            ...
            // double disp2;
            disp = 0;
            for (auto k = 0; k < 3; k++) disp += pow(subs[taggedsubs[j]].GetPosition(k) - subs[i].GetPosition(k), 2.0);
            // disp2=disp;
            dist2sub1 = disp / sigXsubs[index1];
            dist2sub2 = disp / sigXsubs[index2];
            disp      = 0;
            for (auto k = 0; k < 3; k++) disp += pow(subs[taggedsubs[j]].GetVelocity(k) - subs[i].GetVelocity(k), 2.0);
...
*/
void SearchSubSub(Options &opt, const Int_t nsubset, vector<Particle> &Partsubset, Int_t *&pfof, Int_t &ngroup,
                  Int_t &nhalos, PropData *pdata)
{
    ...
    if (ngroup > 0)
    {
        ...
        if (opt.iBaryonSearch >= 1 && opt.partsearchtype == PSTALL)
            numingroup = BuildNumInGroupTyped(nsubset, ngroup, pfof, Partsubset.data(), DARKTYPE);
        else
        ...
        {
            if (opt.iBaryonSearch >= 1 && opt.partsearchtype == PSTALL)
                pglist = BuildPGListTyped(nsubset, ngroup, numingroup, pfof, Partsubset.data(), DARKTYPE);
            else
...
#pragma omp for private(j)
                    ...
                    if (subnumingroup[i] >= MINSUBSIZE && opt.foftype != FOF6DCORE)
                    {
                        ...
                        ngrid = tree->GetNumLeafNodes();
                        if (opt.iverbose)
                            cout << ThisTask << " Substructure " << i << " at sublevel " << sublevel << " with "
                                 << subnumingroup[i] << " particles split into are " << ngrid
                                 << " grid cells, with each node containing ~" << subnumingroup[i] / ngrid
                                 << " particles" << endl;
                        grid = new GridCell[ngrid];
                        ...
                        for (int j = 0; j < ngrid; j++) opt.HaloLocalSigmaV += pow(gveldisp[j].Det(), 1. / 3.);
                    ...
                    subpfof = SearchSubset(opt, subnumingroup[i], subnumingroup[i], subPart, subngroup[i], sublevel,
                                           &numcores[i]);
                    ...
                    if (subngroup[i])
                    {
                        ...
                        if (opt.uinfo.unbindflag && subngroup[i] > 0)
                        {
                            ...
                            iunbindflag = CheckUnboundGroups(opt, subnumingroup[i], subPart, subngroup[i], subpfof,
                            ...
                            if (iunbindflag)
                            {
                                ...
                                if (subngroup[i] > 0)
                                {
                                    subsubnumingroup[i] = BuildNumInGroup(subnumingroup[i], subngroup[i], subpfof);
                                    subsubpglist[i] =
                                        BuildPGList(subnumingroup[i], subngroup[i], subsubnumingroup[i], subpfof);
                                }
                        ...
                        for (j = 0; j < subnumingroup[i]; j++)
                            if (subpfof[j] > 0) pfof[subpglist[i][j]] = ngroup + ngroupidoffset + subpfof[j];
                    ...
                    for (Int_t i = 1; i <= oldnsubsearch; i++)
                    {
                        ...
                        if (pfof[subpglist[i][ii]] > ngroup + ngroupidoffset - ns)
                        {
                            ...
                            for (Int_t jj = 0; jj < nsubset; jj++)
                                if (pfof[jj] > subpfofold[i]) pfof[jj]--;
                            pcsld->nsinlevel--;
                            ngroupidoffsetold--;
                        }
                        else
                        ...
                        for (Int_t j = 1; j <= subngroup[i]; j++)
                        {
                            ...
                            if (j >= (subngroup[i] - numcores[i]) + 1)
                                ...
                                pcsld->nextlevel->stypeinlevel[nscount] = HALOSTYPE + 10 * sublevel;
                            nscount++;
                        }
                    }
                ...
                for (Int_t i = 1; i <= oldnsubsearch; i++)
                    for (Int_t j = 1; j <= subngroup[i]; j++)
                        if (subsubnumingroup[i][j] > MINSUBSIZE) nsubsearch++;
                if (nsubsearch > 0)
                {
                    subnumingroup = new Int_t[nsubsearch + 1];
                    subpglist     = new Int_t *[nsubsearch + 1];
        ...
        if (!opt.iSingleHalo && opt.iBoundHalos > 1 && !opt.iBaryonSearch)
        {
            ...
            if (CheckUnboundGroups(opt, nsubset, Partsubset.data(), nhalos, pfof, numingroup, pglist, 0))
            {
                    ...
                    for (int j = 1; j <= papsldata[i]->nsinlevel; j++)
                    {
                        ...
                        if (parentgid[gidval] != GROUPNOPARENT)
                        {
                            if (numingroup[parentgid[gidval]] > 0)
                                papsldata[i]->gidparenthead[j] = &pfof[pglist[parentgid[gidval]][0]];
                            else
                        ...
                        if (uparentgid[gidval] != GROUPNOPARENT)
                        {
                            if (numingroup[uparentgid[gidval]] > 0)
                                papsldata[i]->giduberparenthead[j] = &pfof[pglist[uparentgid[gidval]][0]];
                            else
                ...
                for (Int_t i = 0; i < nsubset; i++)
                    if (pfof[i] > ng) pfof[i] -= nhaloidoffset;
            }
            for (Int_t i = 1; i <= ngroup; i++) delete[] pglist[i];
            delete[] pglist;
            delete[] numingroup;
...
*/
int CheckSignificance(Options &opt, const Int_t nsubset, Particle *Partsubset, Int_t &numgroups, Int_t *numingroup,
                      Int_t *pfof, Int_t **pglist)
{
    Int_t     i;
    ...
    */
    ...
    ellaveexp = sqrt(2.0 / M_PI) * exp(-ellvallim * ellvallim) * exp(0.5 * ellvallim * ellvallim) /
    ...
    if (opt.iverbose)
    {
        cout << "Checking that groups have a significance level of " << opt.siglevel << " and contain more than "
             << opt.MinSize << " members" << endl;
    }
 ...
 */
Int_t *SearchBaryons(Options &opt, Int_t &nbaryons, Particle *&Pbaryons, const Int_t ndark, vector<Particle> &Part,
                     Int_t *&pfofdark, Int_t &ngroupdark, Int_t &nhalos, int ihaloflag, int iinclusive, PropData *pdata)
{
    KDTree *        tree;
    Double_t *      period;
...
#pragma omp parallel
    ...
    if (npartingroups > 0)
    {
        ...
        if (opt.HaloVelDispScale == 0)
        {
            ...
            {
                for (int j = 0; j < 3; j++)
                    opt.HaloVelDispScale += pow(Part[i].GetVelocity(j) - vmean[j], 2.0) * Part[i].GetMass();
            }
            opt.HaloVelDispScale /= mtotregion;
            param[2] = opt.HaloVelDispScale;
        }
        else
            param[2] = opt.HaloVelDispScale * 16.0; // here use factor of 4 in local dispersion //could
...
#ifdef USEOPENMP
#pragma omp parallel default(shared) private(i, tid, p1, pindex, x1, D2, dval, rval, icheck, nnID, dist2, baryonfofold)
        {
            ...
            tid = 0;
                ...
                if (dist2[0] < param[6])
                {
                    for (int j = 0; j < nsearch; j++)
                    {
                        ...
                        if (opt.partsearchtype == PSTALL)
                            icheck = ((pfofdark[pindex] > nhalos) || (pfofdark[pindex] == baryonfofold));
                        else
                            icheck = (numingroup[pfofbaryons[i]] < numingroup[pfofdark[pindex]]);
                        if (icheck)
                        {
                            if (fofcmp(p1, Part[nnID[j]], param))
                            {
                                for (int k = 0; k < 3; k++)
                                {
                                    D2 += (p1.GetPosition(k) - Part[nnID[j]].GetPosition(k)) *
                                              (p1.GetPosition(k) - Part[nnID[j]].GetPosition(k)) / param[6] +
                                          (p1.GetVelocity(k) - Part[nnID[j]].GetVelocity(k)) *
                                              (p1.GetVelocity(k) - Part[nnID[j]].GetVelocity(k)) / param[7];
                                }
...
// if NOT mpi
    ...
    if (opt.uinfo.unbindflag && ngroupdark > 0)
    {
        ...
        if (opt.iBoundHalos == 0)
        {
            ...
            for (i = 1; i <= nhalos; i++) { ningall[i] = -ningall[i]; }
        }
        if (CheckUnboundGroups(opt, nparts, Part.data(), ngroupdark, pfofall, ningall, pglistall, 0))
        {
                ...
                else
                {
                    ...
                    if (parentgid[i] != GROUPNOPARENT)
                    {
                        if (ningall[parentgid[i]] > 0)
                            papsldata[nlevel]->gidparenthead[ii] = &pfofall[pglistall[parentgid[i]][0]];
                        else
                            papsldata[nlevel]->gidparenthead[ii] = NULL;
                    }
                    if (uparentgid[i] != GROUPNOPARENT)
                    {
                        if (ningall[uparentgid[i]] > 0)
                            papsldata[nlevel]->giduberparenthead[ii] = &pfofall[pglistall[uparentgid[i]][0]];
                        else
                            papsldata[nlevel]->giduberparenthead[ii] = NULL;
                    }
                }
            ...
            for (i = nhierarchy - 1; i >= 0; i--)
            {
                ...
                for (Int_t k = 1; k <= papsldata[i]->nsinlevel; k++)
                    if (papsldata[i]->Phead[k] != NULL) ninlevel++;
                for (Int_t k = 1, j = 0; k <= ninlevel; k++)
                {
                    if (papsldata[i]->Phead[k] == NULL)
                    {
                        ...
                        j = iend;
                        while (papsldata[i]->Phead[j] == NULL) j--;
                        iend--;
                        // copy the first non-NULL pointer to current NULL pointers,
                        // ie: for a non-existing structure which has been unbound, remove
                        // it from the structure list by copying the pointers of the next
            ...
            if (opt.iverbose)
                cout << ThisTask << " Reorder after finding baryons and unbinding, previously had " << ng
                     << " groups and now have " << ngroupdark << endl;
    ...
    for (int i = nhierarchy - 1; i >= 1; i--)
    {
        ...
        for (int j = 1; j <= papsldata[i]->nsinlevel; j++)
        {
            ...
            if (papsldata[i]->giduberparenthead[j] != NULL &&
                ...
                uparentgid[*(papsldata[i]->gidhead[j])] = *(papsldata[i]->giduberparenthead[j]);
...
void CopyHierarchy(Options &opt, PropData *pdata, Int_t ngroups, Int_t *nsub, Int_t *parentgid, Int_t *uparentgid,
                   Int_t *stype)
{
...
#pragma omp for reduction(+ : sigmav)
        ...
        for (i = 1; i <= numgroups; i++)
        {
            ...
            {
                for (int k = 0; k < 3; k++)
                    cmval[k] += Part[pglist[i][j]].GetVelocity(k) * Part[pglist[i][j]].GetMass();
                mt += Part[pglist[i][j]].GetMass();
            }
            mt = 1.0 / mt;
            for (int k = 0; k < 3; k++) cmval[k] *= mt;
            for (int k = 0; k < 3; k++)
                for (int l = 0; l < 3; l++) dispmatrix(k, l) = 0.;
            for (Int_t j = 0; j < numingroup[i]; j++)
            {
                for (int k = 0; k < 3; k++)
                    for (int l = k; l < 3; l++)
            ...
            for (int k = 0; k < 3; k++)
                for (int l = k; l < 3; l++) dispmatrix(k, l) *= mt;
            for (int k = 1; k < 3; k++)
                for (int l = 0; l < k; l++) dispmatrix(k, l) = dispmatrix(l, k);
                                ...
                                Int_t *newlinksIndex, Int_t *numgrouplinksIndex, Int_t *nnID, Int_t ***newIndex)
{
    ...
    for (i = 0; i < newlinks; i++)
    {
        ...
        { (*newIndex)[nnID[ppid]][numgrouplinksIndex[nnID[ppid]]++] = newlinksIndex[i]; }
    }
}
...
inline void DetermineGroupMergerConnections(Particle *Partsubset, Int_t numgroups, Int_t *pfof, int *ilflag,
                                            ...
                                            Int_t ***newIndex, Int_t ***newintergroupIndex, Int_t ***intergroupgidIndex)
{
        ...
        if (intergrouplinks > 0)
        {
            ...
            for (ii = 0; ii < numgrouplinksIndex[i]; ii++)
            {
                ...
                for (Int_t j = 0; j < intergrouplinks; j++)
                    if (pfof[ppid] == (*intergroupgidIndex)[i][j]) { (*newintergroupIndex)[i][j]++; }
            }
        }
...
inline void SearchForNewLinks(Int_t nsubset, KDTree *tree, Particle *Partsubset, Int_t *pfof, FOFcompfunc &fofcmp,
                              Double_t *param, Int_t newlinks, Int_t *newlinksIndex, Int_t **nnID, Double_t **dist2,
                              int nthreads)
{
    Int_t i, ii;
    int   tid;
    ...
    if (newlinks > ompsearchnum)
    {
        for (ii = 0; ii < nsubset; ii++)
            for (int j = 1; j < nthreads; j++) nnID[0][ii + j * nsubset] = nnID[0][ii];
...
#pragma omp for schedule(dynamic, 1) nowait
            ...
            {
                tid = omp_get_thread_num();
                tree->SearchCriterion(newlinksIndex[ii], fofcmp, param, pfof[Partsubset[newlinksIndex[ii]].GetID()],
                                      &nnID[0][tid * nsubset], &dist2[0][tid * nsubset]);
            }
        }
#pragma omp parallel default(shared) private(ii, tid)
        {
...
inline void SearchForNewLinks(Int_t nsubset, KDTree *tree, Particle *Partsubset, Int_t *pfof, FOFcompfunc &fofcmp,
                              Double_t *param, Int_t newlinks, Int_t *newlinksIndex, Int_t **nnID, int nthreads)
{
    Int_t i, ii;
    int   tid;
#ifdef USEOPENMP
...
inline void LinkUntagged(Particle *Partsubset, Int_t numgroups, Int_t *pfof, Int_t *numingroup, Int_t **pglist,
                         Int_t newlinks, Int_t *numgrouplinksIndex, Int_t **newIndex, Int_tree_t *Head,
                         Int_tree_t *Next, Int_tree_t *GroupTail, Int_t *nnID)
{
...
/// search).
inline Int_t MergeGroups(Options &opt, Particle *Partsubset, Int_t numgroups, Int_t *pfof, Int_t *numingroup,
                         Int_t *oldnumingroup, Int_t **pglist, Int_t *numgrouplinksIndex, Int_t ***intergroupgidIndex,
                         Int_t ***newintergroupIndex, Int_t *intergrouplinksIndex, Int_tree_t *Head, Int_tree_t *Next,
                         Int_tree_t *GroupTail, int *igflag, Int_t *nnID, Int_t &newlinks, Int_t *newlinksIndex)
{
...
/// other)
inline Int_t MergeHaloGroups(Options &opt, Particle *Partsubset, Int_t numgroups, Int_t *pfof, Int_t *numingroup,
                             Int_t *oldnumingroup, Int_t **pglist, Int_t *numgrouplinksIndex,
                             Int_t ***intergroupgidIndex, Int_t ***newintergroupIndex, Int_t *intergrouplinksIndex,
                             Int_t *Head, Int_t *Next, Int_t *GroupTail, int *igflag, Int_t *nnID, Int_t &newlinks,
                             Int_t *newlinksIndex)
{
    Int_t i, gid, gidjoin, pindex, lindex, sindex, tail, ss, intergrouplinks, mergers = 0;
    for (i = 1; i <= numgroups; i++)
        if (igflag[i] == 0 && numgrouplinksIndex[i] > 0)
        {
            ...
            for (Int_t j = 0; j < numgrouplinksIndex[i]; j++)
            {
                ...
                {
                    // merge if enough links relative to original size of group
                    int merge = (((*newintergroupIndex)[i][j] > opt.fmergebg * oldnumingroup[gidjoin]) ||
