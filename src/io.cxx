...
/// If we need to do that level of checking, lookup return values of stat which will give you more details on why stat
/// failed.
...
Int_t ReadHeader(Options &opt)
{
    ...
    if (opt.inputtype == IOTIPSY)
    {
        ...
        if (!Ftip)
        {
            cerr << "ERROR: Unable to open " << opt.fname << endl;
            exit(8);
        }
        else
            cout << "Reading tipsy format from " << opt.fname << endl;
        ...
        if (opt.partsearchtype == PSTALL)
            ...
            return tipsyheader.ndark;
        else if (opt.partsearchtype == PSTGAS)
            return tipsyheader.nsph;
        else if (opt.partsearchtype == PSTSTAR)
            return tipsyheader.nstar;
    }
    else if (opt.inputtype == IOGADGET)
    {
        if (opt.partsearchtype == PSTALL)
            ...
            return get_nbodies(opt.fname, -2);
        else if (opt.partsearchtype == PSTGAS)
            return get_nbodies(opt.fname, GGASTYPE);
        else if (opt.partsearchtype == PSTSTAR)
            return get_nbodies(opt.fname, GSTARTYPE);
        else if (opt.partsearchtype == PSTBH)
            return get_nbodies(opt.fname, GBHTYPE);
    }
    else if (opt.inputtype == IORAMSES)
        return RAMSES_get_nbodies(opt.fname, opt.partsearchtype, opt);
#ifdef USEHDF
    else if (opt.inputtype == IOHDF)
        return HDF_get_nbodies(opt.fname, opt.partsearchtype, opt);
#endif
#ifdef USEXDR
    else if (opt.inputtype == IONCHILADA)
        return Nchilada_get_nbodies(opt.fname, opt.partsearchtype, opt);
#endif
    return 0;
}
...
void ReadData(Options &opt, vector<Particle> &Part, const Int_t nbodies, Particle *&Pbaryons, Int_t nbaryons)
{
    ...
    if (ThisTask == 0)
    {
        ...
        cout << opt.mpiparticlebufsize * NProcs * sizeof(Particle) / 1024.0 / 1024.0 / 1024.0
             << " of memory to store particle data" << endl;
        cout << "Sending information to non-read threads in chunks of " << opt.mpiparticlebufsize << " particles "
             << endl;
    ...
    if (opt.inputtype == IOTIPSY)
        ReadTipsy(opt, Part, nbodies, Pbaryons, nbaryons);
    else if (opt.inputtype == IOGADGET)
        ReadGadget(opt, Part, nbodies, Pbaryons, nbaryons);
    else if (opt.inputtype == IORAMSES)
        ReadRamses(opt, Part, nbodies, Pbaryons, nbaryons);
#ifdef USEHDF
    else if (opt.inputtype == IOHDF)
        ReadHDF(opt, Part, nbodies, Pbaryons, nbaryons);
#endif
#ifdef USEXDR
    else if (opt.inputtype == IONCHILADA)
        ReadNchilada(opt, Part, nbodies, Pbaryons, nbaryons);
#endif
...
void AdjustStarQuantities(Options &opt, vector<Particle> &Part, const Int_t nbodies)
{
    ...
    if (opt.istellaragescalefactor != 0 || opt.stellarageinputconversion != 1.0)
    {
        ...
        for (auto &p : Part)
        {
            ...
            if (opt.istellaragescalefactor == 1) { tage = CalcCosmicTime(opt, p.GetTage(), opt.a); }
...
void ReadLocalVelocityDensity(Options &opt, const Int_t nbodies, vector<Particle> &Part)
{
    ...
    if (opt.ibinaryout == OUTBINARY)
    {
        ...
        {
            Fin.read((char *)&tempd, sizeof(Double_t));
            Part[i].SetDensity(tempd);
        }
    }
    else
    {
...
void WriteLocalVelocityDensity(Options &opt, const Int_t nbodies, vector<Particle> &Part)
{
...
#ifdef USEMPI
    if (opt.smname == NULL)
        sprintf(fname, "%s.smdata.%d", opt.outname, ThisTask);
    else
        sprintf(fname, "%s.%d", opt.smname, ThisTask);
#else
    if (opt.smname == NULL)
        sprintf(fname, "%s.smdata", opt.outname);
    else
        sprintf(fname, "%s", opt.smname);
#endif
    if (opt.ibinaryout == OUTBINARY)
    {
        ...
        for (Int_t i = 0; i < nbodies; i++)
        {
            tempd = Part[i].GetDensity();
            Fout.write((char *)&tempd, sizeof(Double_t));
        }
    }
    if (opt.ibinaryout == OUTBINARY)
    {
...
/*! Writes a tipsy formatted fof.grp array file that contains the number of particles first then for each particle the
   group id of that particle group zero is untagged particles. \n
*/
                       ...
                       Int_t nadditional)
{
    ...
    {
        // declare the attributes in a header group, assiging the group handle, setting the name, no time step
        // indicator, and a flag saying yes to all statistics
        ...
        adios_err = adios_define_attribute(adios_grp_handle, datagroupnames.group[itemp].c_str(), "",
                                           datagroupnames.adiosgroupdatatype[itemp], to_string(ThisTask).c_str(), "");
        itemp++;
        adios_err = adios_define_attribute(adios_grp_handle, datagroupnames.group[itemp].c_str(), "",
                                           datagroupnames.adiosgroupdatatype[itemp], to_string(NProcs).c_str(), "");
        itemp++;
        adios_err = adios_define_attribute(adios_grp_handle, datagroupnames.group[itemp].c_str(), "",
                                           datagroupnames.adiosgroupdatatype[itemp], to_string(ng).c_str(), "");
        itemp++;
        adios_err = adios_define_attribute(adios_grp_handle, datagroupnames.group[itemp].c_str(), "",
                                           datagroupnames.adiosgroupdatatype[itemp], to_string(ngtot).c_str(), "");
        itemp++;
    ...
    else if (opt.ibinaryout == OUTADIOS)
    {
        ...
        // if want to define dimensions can either create a variable that stores the dimensions or store the value as a
        // string. store local dim
        ...
        // then define the group actually storing the data. Might be useful to define an offset variable as well for
        // quick access when reading offset would be the last field in the code below
        adios_err       = adios_define_var(adios_grp_handle, datagroupnames.group[itemp].c_str(), "",
                                     datagroupnames.adiosgroupdatatype[itemp], "ng", "ngtot", "ngmpioffset");
        ...
        for (Int_t i = 1; i <= ngroups; i++) Fout << numingroup[i] << endl;
    ...
    for (Int_t i = 2; i <= ngroups; i++)
        offset[i] = offset[i - 1] + numingroup[i - 1] - pglist[i - 1][numingroup[i - 1]];
    if (opt.ibinaryout == OUTBINARY) Fout.write((char *)&offset[1], sizeof(Int_t) * ngroups);
#ifdef USEHDF
    else if (opt.ibinaryout == OUTHDF)
    {
    ...
    if (opt.ibinaryout == OUTASCII || opt.ibinaryout == OUTBINARY) Fout.close();
        ...
        adios_err = adios_close(adios_file_handle);
#endif
    ...
    else if (opt.ibinaryout == OUTADIOS)
    {
        ...
        adios_err =
            adios_open(&adios_file_handle3, "VELOCIraptor_catalog_particles.unbound", fname3, "w", MPI_COMM_WORLD);
    }
#endif
    else
    {
    ...
    for (Int_t i = 1; i <= ngroups; i++)
    {
        nids += pglist[i][numingroup[i]];
        nuids += numingroup[i] - pglist[i][numingroup[i]];
    }
#ifdef USEMPI
    ...
    else if (opt.ibinaryout == OUTADIOS)
    {
        ...
        adios_err = adios_define_attribute(adios_grp_handle, datagroupnames.group[itemp].c_str(), "",
                                           datagroupnames.adiosgroupdatatype[itemp], to_string(ThisTask).c_str(), "");
        adios_err = adios_define_attribute(adios_grp_handle3, datagroupnames.group[itemp].c_str(), "",
                                           datagroupnames.adiosgroupdatatype[itemp], to_string(ThisTask).c_str(), "");
        itemp++;
        adios_err = adios_define_attribute(adios_grp_handle, datagroupnames.group[itemp].c_str(), "",
                                           datagroupnames.adiosgroupdatatype[itemp], to_string(NProcs).c_str(), "");
        ...
        itemp++;
        adios_err = adios_define_attribute(adios_grp_handle, datagroupnames.group[itemp].c_str(), "",
                                           datagroupnames.adiosgroupdatatype[itemp], to_string(nids).c_str(), "");
        adios_err = adios_define_attribute(adios_grp_handle3, datagroupnames.group[itemp].c_str(), "",
                                           datagroupnames.adiosgroupdatatype[itemp], to_string(nuids).c_str(), "");
        itemp++;
        adios_err = adios_define_attribute(adios_grp_handle, datagroupnames.group[itemp].c_str(), "",
                                           datagroupnames.adiosgroupdatatype[itemp], to_string(nidstot).c_str(), "");
        adios_err = adios_define_attribute(adios_grp_handle3, datagroupnames.group[itemp].c_str(), "",
                                           datagroupnames.adiosgroupdatatype[itemp], to_string(nuidstot).c_str(), "");
        itemp++;
        ///\todo don't actually know if I should use adios attribute or var to store simple single values
    }
#endif
    ...
    nids  = 0;
    for (Int_t i = 1; i <= ngroups; i++)
        for (Int_t j = 0; j < pglist[i][numingroup[i]]; j++) idval[nids++] = Part[pglist[i][j]].GetPID();
    ...
    else if (opt.ibinaryout == OUTADIOS)
    {
        ...
        adios_err       = adios_define_var(adios_grp_handle, datagroupnames.group[itemp].c_str(), "",
                                     datagroupnames.adiosgroupdatatype[itemp], "nids", "nidstot", "nidsmpioffset");
        ...
        for (Int_t i = 0; i < nids; i++) Fout << idval[i] << endl;
    delete[] idval;
    if (opt.ibinaryout == OUTASCII || opt.ibinaryout == OUTBINARY) Fout.close();
#ifdef USEHDF
    ...
    nuids = 0;
    for (Int_t i = 1; i <= ngroups; i++)
        for (Int_t j = pglist[i][numingroup[i]]; j < numingroup[i]; j++) idval[nuids++] = Part[pglist[i][j]].GetPID();
    ...
    else if (opt.ibinaryout == OUTADIOS)
    {
        ...
        adios_err       = adios_define_var(adios_grp_handle3, datagroupnames.group[itemp].c_str(), "",
                                     datagroupnames.adiosgroupdatatype[itemp], "nuids", "nuidstot", "nuidsmpioffset");
        ...
        for (Int_t i = 0; i < nuids; i++) Fout3 << idval[i] << endl;
    delete[] idval;
    ...
    if (opt.ibinaryout == OUTHDF) Fhdf3.close();
        ...
        adios_err = adios_close(adios_file_handle3);
#endif
...
void WriteGroupPartType(Options &opt, const Int_t ngroups, Int_t *numingroup, Int_t **pglist, vector<Particle> &Part)
{
    ...
    nids    = 0;
    for (Int_t i = 1; i <= ngroups; i++)
        for (Int_t j = 0; j < pglist[i][numingroup[i]]; j++) typeval[nids++] = Part[pglist[i][j]].GetType();
    ...
    else if (opt.ibinaryout == OUTHDF)
    {
        ...
        for (Int_t i = 0; i < nids; i++) Fout << typeval[i] << endl;
    delete[] typeval;
    ...
    else if (opt.ibinaryout == OUTHDF)
    {
        ...
        for (Int_t i = 0; i < nuids; i++) Fout2 << typeval[i] << endl;
    delete[] typeval;
...
void WriteSOCatalog(Options &opt, const Int_t ngroups, vector<Int_t> *SOpids, vector<int> *SOtypes)
{
    ...
    else if (opt.ibinaryout == OUTADIOS)
    {
        ...
        adios_select_method(adios_grp_handle, "MPI", "", "");
        // define some attributes
        adios_err = adios_define_attribute(adios_grp_handle, datagroupnames.SO[itemp].c_str(), "",
                                           datagroupnames.adiosSOdatatype[itemp], to_string(ThisTask).c_str(), "");
        itemp++;
        adios_err = adios_define_attribute(adios_grp_handle, datagroupnames.SO[itemp].c_str(), "",
                                           datagroupnames.adiosSOdatatype[itemp], to_string(NProcs).c_str(), "");
        itemp++;
        adios_err = adios_define_attribute(adios_grp_handle, datagroupnames.SO[itemp].c_str(), "",
                                           datagroupnames.adiosSOdatatype[itemp], to_string(ng).c_str(), "");
        itemp++;
        adios_err = adios_define_attribute(adios_grp_handle, datagroupnames.SO[itemp].c_str(), "",
                                           datagroupnames.adiosSOdatatype[itemp], to_string(ngtot).c_str(), "");
        itemp++;
        adios_err = adios_define_attribute(adios_grp_handle, datagroupnames.SO[itemp].c_str(), "",
                                           datagroupnames.adiosSOdatatype[itemp], to_string(nSOids).c_str(), "");
        itemp++;
        adios_err = adios_define_attribute(adios_grp_handle, datagroupnames.SO[itemp].c_str(), "",
                                           datagroupnames.adiosSOdatatype[itemp], to_string(nSOidstot).c_str(), "");
        itemp++;
        ///\todo don't actually know if I should use adios attribute or var to store simple single values
    }
#endif
    ...
    else if (opt.ibinaryout == OUTADIOS)
    {
        ...
        // then define the group actually storing the data. Might be useful to define an offset variable as well for
        // quick access when reading offset would be the last field in the code below
        adios_err       = adios_define_var(adios_grp_handle, datagroupnames.SO[itemp].c_str(), "",
                                     datagroupnames.adiosSOdatatype[itemp], "ng", "ngtot", "ngmpioffset");
        adios_err       = adios_write(adios_file_handle, "ng", &ng);
        adios_err       = adios_write(adios_file_handle, "ngtot", &ngtot);
        Int_t mpioffset = 0;
        for (Int_t itask = 0; itask < ThisTask; itask++) mpioffset += mpi_ngroups[itask];
    ...
    if (nSOids > 0)
    {
        ...
        for (Int_t i = 1; i <= ngroups; i++)
        {
            for (Int_t j = 0; j < SOpids[i].size(); j++) idval[nSOids++] = SOpids[i][j];
            SOpids[i].resize(0);
        }
        ...
        for (Int_t i = 1; i <= ngroups; i++)
        {
            for (Int_t j = 0; j < SOtypes[i].size(); j++) typeval[nSOids++] = SOtypes[i][j];
            SOtypes[i].resize(0);
        }
#endif
    }
    ...
    else if (opt.ibinaryout == OUTADIOS)
    {
        ...
        adios_err       = adios_define_var(adios_grp_handle, datagroupnames.SO[itemp].c_str(), "",
                                     datagroupnames.adiosSOdatatype[itemp], "nSOids", "nSOidstot", "nSOidsmpioffset");
...
///\name Final outputs such as properties and output that can be used to construct merger trees and substructure
///hierarchy
//@{
...
void WriteProperties(Options &opt, const Int_t ngroups, PropData *pdata)
{
    ...
    else
    {
        ...
        for (Int_t i = 0; i < head.headerdatainfo.size(); i++) Fout << head.headerdatainfo[i] << "(" << i + 1 << ") ";
        Fout << endl;
    ...
    for (Int_t i = 1; i <= ngroups; i++)
    {
        if (opt.ibinaryout == OUTBINARY) { pdata[i].WriteBinary(Fout, opt); }
#ifdef USEHDF
        else if (opt.ibinaryout == OUTHDF)
        {
    ...
    if (opt.ibinaryout == OUTHDF)
    {
        ...
        // first is halo ids, then id of most bound particle, host halo id, number of direct subhaloes, number of
        // particles
        ...
        if (opt.iaperturecalc && opt.aperturenum > 0)
        {
            ...
            for (auto j = 0; j < opt.aperturenum; j++)
            {
                for (Int_t i = 0; i < ngroups; i++)
                    ((unsigned int *)data)[i] = pdata[i + 1].aperture_npart_interloper[j];
                Fhdf.write_dataset(head.headerdatainfo[itemp], ng, data, head.hdfpredtypeinfo[itemp]);
                itemp++;
            }
#endif
        ...
        if (opt.iaperturecalc && opt.apertureprojnum > 0)
        {
            for (auto k = 0; k < 3; k++)
            {
                ...
                {
                    for (Int_t i = 0; i < ngroups; i++)
                        ((Double_t *)data)[i] = pdata[i + 1].aperture_mass_proj_gas[j][k];
                    Fhdf.write_dataset(head.headerdatainfo[itemp], ng, data, head.hdfpredtypeinfo[itemp]);
                    itemp++;
                }
#ifdef STARON
                ...
                {
                    for (Int_t i = 0; i < ngroups; i++)
                        ((Double_t *)data)[i] = pdata[i + 1].aperture_mass_proj_gas_sf[j][k];
                    Fhdf.write_dataset(head.headerdatainfo[itemp], ng, data, head.hdfpredtypeinfo[itemp]);
                    itemp++;
                }
                for (auto j = 0; j < opt.apertureprojnum; j++)
                {
                    for (Int_t i = 0; i < ngroups; i++)
                        ((Double_t *)data)[i] = pdata[i + 1].aperture_mass_proj_gas_nsf[j][k];
                    Fhdf.write_dataset(head.headerdatainfo[itemp], ng, data, head.hdfpredtypeinfo[itemp]);
                    itemp++;
                }
#endif
                ...
                {
                    for (Int_t i = 0; i < ngroups; i++)
                        ((Double_t *)data)[i] = pdata[i + 1].aperture_mass_proj_star[j][k];
                    Fhdf.write_dataset(head.headerdatainfo[itemp], ng, data, head.hdfpredtypeinfo[itemp]);
                    itemp++;
                }
#endif
                ...
                {
                    for (Int_t i = 0; i < ngroups; i++)
                        ((Double_t *)data)[i] = pdata[i + 1].aperture_rhalfmass_proj[j][k];
                    Fhdf.write_dataset(head.headerdatainfo[itemp], ng, data, head.hdfpredtypeinfo[itemp]);
                    itemp++;
                }
#ifdef GASON
                ...
                {
                    for (Int_t i = 0; i < ngroups; i++)
                        ((Double_t *)data)[i] = pdata[i + 1].aperture_rhalfmass_proj_gas[j][k];
                    Fhdf.write_dataset(head.headerdatainfo[itemp], ng, data, head.hdfpredtypeinfo[itemp]);
                    itemp++;
                }
#ifdef STARON
                ...
                {
                    for (Int_t i = 0; i < ngroups; i++)
                        ((Double_t *)data)[i] = pdata[i + 1].aperture_rhalfmass_proj_gas_sf[j][k];
                    Fhdf.write_dataset(head.headerdatainfo[itemp], ng, data, head.hdfpredtypeinfo[itemp]);
                    itemp++;
                }
                for (auto j = 0; j < opt.apertureprojnum; j++)
                {
                    for (Int_t i = 0; i < ngroups; i++)
                        ((Double_t *)data)[i] = pdata[i + 1].aperture_rhalfmass_proj_gas_nsf[j][k];
                    Fhdf.write_dataset(head.headerdatainfo[itemp], ng, data, head.hdfpredtypeinfo[itemp]);
                    itemp++;
                }
#endif
                ...
                {
                    for (Int_t i = 0; i < ngroups; i++)
                        ((Double_t *)data)[i] = pdata[i + 1].aperture_rhalfmass_proj_star[j][k];
                    Fhdf.write_dataset(head.headerdatainfo[itemp], ng, data, head.hdfpredtypeinfo[itemp]);
                    itemp++;
                }
#endif
                ...
                {
                    for (Int_t i = 0; i < ngroups; i++)
                        ((Double_t *)data)[i] = pdata[i + 1].aperture_SFR_proj_gas[j][k];
                    Fhdf.write_dataset(head.headerdatainfo[itemp], ng, data, head.hdfpredtypeinfo[itemp]);
                    itemp++;
                }
#endif
        ...
        if (opt.SOnum > 0 && opt.iextrahalooutput)
        {
            ...
            if (opt.iextragasoutput)
            {
                ...
                {
                    for (Int_t i = 0; i < ngroups; i++)
                        ((Double_t *)data)[i] = pdata[i + 1].SO_angularmomentum_gas[j][0];
                    Fhdf.write_dataset(head.headerdatainfo[itemp], ng, data, head.hdfpredtypeinfo[itemp]);
                    itemp++;
                    for (Int_t i = 0; i < ngroups; i++)
                        ((Double_t *)data)[i] = pdata[i + 1].SO_angularmomentum_gas[j][1];
                    Fhdf.write_dataset(head.headerdatainfo[itemp], ng, data, head.hdfpredtypeinfo[itemp]);
                    itemp++;
                    for (Int_t i = 0; i < ngroups; i++)
                        ((Double_t *)data)[i] = pdata[i + 1].SO_angularmomentum_gas[j][2];
                    Fhdf.write_dataset(head.headerdatainfo[itemp], ng, data, head.hdfpredtypeinfo[itemp]);
                    itemp++;
                }
#ifdef STARON
            ...
            if (opt.iextrastaroutput)
            {
                ...
                {
                    for (Int_t i = 0; i < ngroups; i++)
                        ((Double_t *)data)[i] = pdata[i + 1].SO_angularmomentum_star[j][0];
                    Fhdf.write_dataset(head.headerdatainfo[itemp], ng, data, head.hdfpredtypeinfo[itemp]);
                    itemp++;
                    for (Int_t i = 0; i < ngroups; i++)
                        ((Double_t *)data)[i] = pdata[i + 1].SO_angularmomentum_star[j][1];
                    Fhdf.write_dataset(head.headerdatainfo[itemp], ng, data, head.hdfpredtypeinfo[itemp]);
                    itemp++;
                    for (Int_t i = 0; i < ngroups; i++)
                        ((Double_t *)data)[i] = pdata[i + 1].SO_angularmomentum_star[j][2];
                    Fhdf.write_dataset(head.headerdatainfo[itemp], ng, data, head.hdfpredtypeinfo[itemp]);
                    itemp++;
                }
            }
...
void WriteProfiles(Options &opt, const Int_t ngroups, PropData *pdata)
{
    ...
    if (opt.iInclusiveHalo > 0)
        for (auto i = 1; i <= ng; i++) nhalos += (pdata[i].hostid == -1);
#ifdef USEHDF
    // H5File Fhdf;
    // H5std_string datasetname;
    // DataSpace dataspace;
    ...
    ((Double_t *)data)[0] = 0.0;
    for (auto i = 0; i < opt.profilenbins; i++) ((Double_t *)data)[i + 1] = opt.profile_bin_edges[i];
    // write header
    if (opt.ibinaryout == OUTBINARY)
    {
        Fout.open(fname, ios::out | ios::binary);
    ...
    else if (opt.ibinaryout == OUTHDF)
    {
        ...
        // Fhdf.write_dataset(datagroupnames.profile[itemp++], opt.profileradnormstring.size(),
        // opt.profileradnormstring);
    ...
    else
    {
        ...
        Fout << "Radial_bin_edges=0 ";
        for (auto i = 0; i < opt.profilenbins; i++) Fout << opt.profile_bin_edges[i] << " ";
        Fout << endl;
    ...
    if (opt.ibinaryout == OUTHDF)
    {
        ...
        for (Int_t i = 0; i < ngroups; i++) ((unsigned long *)data)[i] = pdata[i + 1].haloid;
        Fhdf.write_dataset(head.headerdatainfo[itemp], ngroups, data, head.hdfpredtypeinfo[itemp]);
        itemp++;
        if (opt.iprofilenorm == PROFILERNORMR200CRIT)
        {
            for (Int_t i = 0; i < ngroups; i++)
            {
                if (opt.iInclusiveHalo > 0)
                {
                    if (pdata[i + 1].hostid == -1)
                        ((Double_t *)data)[i] = pdata[i + 1].gR200c_excl;
                    else
                        ((Double_t *)data)[i] = pdata[i + 1].gR200c;
                }
                else
                    ((Double_t *)data)[i] = pdata[i + 1].gR200c;
            }
        ...
        Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
        itemp++;

#ifdef GASON
        for (Int_t i = 0; i < ngroups; i++)
            for (auto j = 0; j < opt.profilenbins; j++)
                ((unsigned int *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_npart_gas[j];
        Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
        itemp++;
#ifdef STARON
        for (Int_t i = 0; i < ngroups; i++)
            for (auto j = 0; j < opt.profilenbins; j++)
                ((unsigned int *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_npart_gas_sf[j];
        Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
        itemp++;
        for (Int_t i = 0; i < ngroups; i++)
            for (auto j = 0; j < opt.profilenbins; j++)
                ((unsigned int *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_npart_gas_nsf[j];
        Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
        itemp++;
#endif
#endif
        ...
        for (Int_t i = 0; i < ngroups; i++)
            for (auto j = 0; j < opt.profilenbins; j++)
                ((unsigned int *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_npart_star[j];
        Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
        itemp++;
#endif
        ...
        for (Int_t i = 0; i < ngroups; i++)
            for (auto j = 0; j < opt.profilenbins; j++)
                ((float *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_mass[j];
        Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
        itemp++;
#ifdef GASON
        for (Int_t i = 0; i < ngroups; i++)
            for (auto j = 0; j < opt.profilenbins; j++)
                ((float *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_mass_gas[j];
        Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
        itemp++;
#ifdef STARON
        for (Int_t i = 0; i < ngroups; i++)
            for (auto j = 0; j < opt.profilenbins; j++)
                ((float *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_mass_gas_sf[j];
        Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
        itemp++;
        for (Int_t i = 0; i < ngroups; i++)
            for (auto j = 0; j < opt.profilenbins; j++)
                ((float *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_mass_gas_nsf[j];
        Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
        itemp++;
#endif
#endif
        ...
        for (Int_t i = 0; i < ngroups; i++)
            for (auto j = 0; j < opt.profilenbins; j++)
                ((float *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_mass_star[j];
        Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
        itemp++;
#endif
        ::operator delete(data);
        ...
        if (opt.iInclusiveHalo > 0)
        {
            ...
            for (Int_t i = 0; i < nhalos; i++)
                for (auto j = 0; j < opt.profilenbins; j++)
                    ((unsigned int *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_npart_inclusive[j];
            Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
            itemp++;
#ifdef GASON
            for (Int_t i = 0; i < nhalos; i++)
                for (auto j = 0; j < opt.profilenbins; j++)
                    ((unsigned int *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_npart_inclusive_gas[j];
            Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
            itemp++;
#ifdef STARON
            for (Int_t i = 0; i < nhalos; i++)
                for (auto j = 0; j < opt.profilenbins; j++)
                    ((unsigned int *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_npart_inclusive_gas_sf[j];
            Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
            itemp++;
            for (Int_t i = 0; i < nhalos; i++)
                for (auto j = 0; j < opt.profilenbins; j++)
                    ((unsigned int *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_npart_inclusive_gas_nsf[j];
            Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
            itemp++;
#endif
#endif
            ...
            for (Int_t i = 0; i < nhalos; i++)
                for (auto j = 0; j < opt.profilenbins; j++)
                    ((unsigned int *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_npart_inclusive_star[j];
            Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
            itemp++;
#endif
            ...
            for (Int_t i = 0; i < nhalos; i++)
                for (auto j = 0; j < opt.profilenbins; j++)
                    ((float *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_mass_inclusive[j];
            Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
            itemp++;
#ifdef GASON
            for (Int_t i = 0; i < nhalos; i++)
                for (auto j = 0; j < opt.profilenbins; j++)
                    ((float *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_mass_inclusive_gas[j];
            Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
            itemp++;
#ifdef STARON
            for (Int_t i = 0; i < nhalos; i++)
                for (auto j = 0; j < opt.profilenbins; j++)
                    ((float *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_mass_inclusive_gas_sf[j];
            Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
            itemp++;
            for (Int_t i = 0; i < nhalos; i++)
                for (auto j = 0; j < opt.profilenbins; j++)
                    ((float *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_mass_inclusive_gas_nsf[j];
            Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
            itemp++;
#endif
#endif
            ...
            for (Int_t i = 0; i < nhalos; i++)
                for (auto j = 0; j < opt.profilenbins; j++)
                    ((float *)data)[i * opt.profilenbins + j] = pdata[i + 1].profile_mass_inclusive_star[j];
            Fhdf.write_dataset_nd(head.headerdatainfo[itemp], 2, dims.data(), data, head.hdfpredtypeinfo[itemp]);
            itemp++;
#endif
        }
...
void WriteHierarchy(Options &opt, const Int_t &ngroups, const Int_t &nhierarchy, const Int_t &nfield, Int_t *nsub,
                    Int_t *parentgid, Int_t *stype, int subflag)
{
    fstream       Fout;
    fstream       Fout2;
    ...
    if (subflag == 0)
    {
        ...
        else if (opt.ibinaryout == OUTHDF)
        {
            ...
            for (Int_t i = 1; i <= nfield; i++) Fout << nsub[i] << endl;
    }
    else if (subflag == 1)
    {
        if (opt.ibinaryout == OUTBINARY)
    ...
    if (subflag == 0)
    {
        ...
        else if (opt.ibinaryout == OUTHDF)
        {
            ...
            for (Int_t i = 1; i <= nfield; i++) Fout << parentgid[i] << " " << nsub[i] << endl;
    }
    else if (subflag == 1)
    {
        if (opt.ibinaryout == OUTBINARY)
        ...
        else if (opt.ibinaryout == OUTHDF)
        {
            ...
            for (Int_t i = nfield + 1; i <= ngroups; i++) Fout << parentgid[i] << " " << nsub[i] << endl;
    }
    // write everything, no distinction made between field and substructure
    else if (subflag == -1)
    {
...
void WriteCellValues(Options &opt, const Int_t nbodies, const Int_t ngrid, GridCell *grid, Coordinate *gvel,
                     Matrix *gveldisp)
{
    fstream Fout;
    char    fname[1000];
#ifdef USEMPI
    if (opt.gname == NULL)
        sprintf(fname, "%s.griddata.%d", opt.outname, ThisTask);
    else
        sprintf(fname, "%s.%d", opt.gname, ThisTask);
#else
    if (opt.gname == NULL)
        sprintf(fname, "%s.griddata", opt.outname);
    else
        sprintf(fname, "%s", opt.gname);
#endif
    if (opt.ibinaryout == OUTBINARY)
    {
        Fout.open(fname, ios::out | ios::binary);
    ...
    else
    {
        ...
        for (Int_t i = 0; i < ngrid; i++)
        {
            ...
            for (int j = 0; j < 3; j++)
                for (int k = 0; k < 3; k++) Fout << gveldisp[i](j, k) << " ";
            Fout << endl;
        }
    }
    Fout.close();
...
Int_t ReadPFOF(Options &opt, Int_t nbodies, Int_t *pfof)
{
    ...
    {
        Fin >> pfof[i];
        if (pfof[i] > ngroup) ngroup = pfof[i];
    }
    Fin.close();
    cout << "Done" << endl;
    return ngroup;
...
{ // old groupcat format
    ...
    // read group header info (number of groups, number of particles in all groups, total number of groups (in case
    // split across several files) and number of files
    ...
    for (Int_t i = 0; i < Ngroups; i++)
    {
        for (Int_t j = 0; j < numingroup[i]; j++) { pfof[idtoindex[pids[j + offset] - minid]] = i + 1; }
        offset += numingroup[i];
    }
    Fin.close();
    return Ngroups;
...
#else
        Fout << "C compiler is too old and config file output relies on std 11 implentation to write info. UPDATE YOUR "
                "COMPILER "
             << endl;
#endif
        Fout.close();
...
void PrintSimulationState(Options &opt)
{
    if (opt.iverbose)
    {
        ...
        if (opt.icosmologicalin)
        {
            double Hubble =
                opt.h * opt.H *
                sqrt(opt.Omega_k * pow(opt.a, -2.0) + opt.Omega_m * pow(opt.a, -3.0) + opt.Omega_r * pow(opt.a, -4.0) +
                     opt.Omega_Lambda + opt.Omega_de * pow(opt.a, -3.0 * (1 + opt.w_de)));
...
void WriteExtendedOutput(Options &opt, Int_t numgroups, Int_t nbodies, PropData *pdata, Particle *p, Int_t *pfof)
{
    ...
    for (Int_t i = 0; i < numgroups; i++)
    {
        ...
        ntaskspergroup[i]      = 0;
        for (Int_t j = 0; j < NProcs; j++) npartsofgroupintask[i][j] = 0;
#endif

        nfilespergroup[i] = 0;
        for (Int_t j = 0; j < opt.num_files; j++) npartsofgroupinfile[i][j] = 0;
    }
    ...
    for (Int_t i = 0; i < numgroups; i++)
    {
        ...
        for (Int_t j = 0; j < NProcs; j++)
            if (npartsofgroupintask[i][j] > 0) ntaskspergroup[i]++;
#endif
        for (Int_t j = 0; j < opt.num_files; j++)
            if (npartsofgroupinfile[i][j] > 0) nfilespergroup[i]++;
    }
...
#ifdef USEMPI
        if (NProcs > 1) MPI_Isend(&myturn, 1, MPI_INT, ThisTask + 1, ThisTask, MPI_COMM_WORLD, &rqst);
    }
    else
    {
        ...
        if (ThisTask != NProcs - 1) MPI_Isend(&myturn, 1, MPI_INT, ThisTask + 1, ThisTask, MPI_COMM_WORLD, &rqst);
    }
    delete[] filesofgroup;
    delete[] tasksofgroup;
    ...
    for (Int_t i = 0; i < nbodies; i++)
        if (p[i].GetOTask() != ThisTask) PartsToSend[p[i].GetOTask()][count[p[i].GetOTask()]++] = Particle(p[i]);
    ...
    for (Int_t i = 0; i < NProcs; i++)
        if (ntosendtotask[i] > localMax) localMax = ntosendtotask[i];
    ...
    if (bufferFlag)
    {
        ...
        for (int i = 1; i < NProcs; i++)
        {
            ...
            for (int jj = 0; jj < numBuffersToSend[dst] - 1; jj++)
            {
                ...
                MPI_Isend(&PartsToSend[dst][buffOffset], sizeof(Particle) * size, MPI_BYTE, dst, (int)(10000 + jj + 1),
                          MPI_COMM_WORLD, &rqst);
            }
            ...
            if (size > 0 && numBuffersToSend[dst] > 0)
            {
                ...
                MPI_Isend(&PartsToSend[dst][buffOffset], sizeof(Particle) * size, MPI_BYTE, dst,
                          (int)(10000 + numBuffersToSend[dst]), MPI_COMM_WORLD, &rqst);
            }
            ...
            for (int jj = 0; jj < numBuffersToRecv[src]; jj++)
            {
                ...
                MPI_Recv(&PartsToRecv[src][buffOffset], sizeof(Particle) * numInBuffer, MPI_BYTE, src,
                         (int)(10000 + jj + 1), MPI_COMM_WORLD, &status);
    ...
    else
    {
        for (Int_t i = 1; i < NProcs; i++)
        {
            int src = (ThisTask + NProcs - i) % NProcs;
            int dst = (ThisTask + i) % NProcs;
            MPI_Isend(PartsToSend[dst], ntosendtotask[dst] * sizeof(Particle), MPI_BYTE, dst, ThisTask, MPI_COMM_WORLD,
                      &rqst);
            MPI_Recv(PartsToRecv[src], ntorecievefromtask[src] * sizeof(Particle), MPI_BYTE, src, src, MPI_COMM_WORLD,
                     &status);
        }
    }
#endif
    ...
    for (Int_t i = 0; i < opt.num_files; i++) npartperfile[i] = 0;
    ...
    for (Int_t i = 0; i < NProcs; i++)
        for (Int_t j = 0; j < ntorecievefromtask[i]; j++)
            if (PartsToRecv[i][j].GetOTask() == ThisTask)
            {
                ...
                IdTopHost[PartsToRecv[i][j].GetOFile()][PartsToRecv[i][j].GetOIndex()] =
                    PartsToRecv[i][j].GetIdTopHost();
            }
#endif
