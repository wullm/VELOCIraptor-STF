...
/// like \ref FOFStreamwithprob search but here linking length adjusted by velocity offset, smaller lengths for larger
/// velocity offsets
...
//@{
/// this is minimum particle number size for a subsearch to proceed whereby substructure split up into CELLSPLITNUM new
/// cells
...
/// number below which just use PP calculation for potential, which occurs roughly at when n~2*log(n) (from scaling of
/// n^2 vs n ln(n) for PP vs tree and factor of 2 is for extra overhead in producing tree. For reasonable values of n
/// (>100) this occurs at ~100. Here to account for extra memory need for tree, we use n=3*log(n) or 150
...
#define CMVELREF 0
/// use the particle at potential minimum. Issues if too few particles used as particles will move in and out of deepest
...
struct UnbindInfo
{
    ...
    /// minimum number of particles to use to calculate reference frame if using particles around deepest potential well
    /// as reference frame
    Int_t Npotref;
    /// fraction of number of particles to use to calculate reference frame if using particles around deepest potential
    /// well as reference frame
    Double_t fracpotref;
...
/// Options structure stores useful variables that have user determined values which are altered by \ref GetArgs in \ref
/// ui.cxx
struct Options
{
    ///\name filenames
    ...
    /// period (comove)
    Double_t p;
    ///\name scale factor, Hubunit, h, cosmology, virial density. These are used if linking lengths are scaled or trying
    ///to define virlevel using the cosmology
    //@{
    Double_t a, H, h;
    ...
    /// to store number of each particle types so that if only searching one particle type, assumes order of gas, dark
    /// (halo, disk,bulge), star, special or sink for pfof tipsy style output
    Int_t numpart[NPARTTYPES];

    ///\name parameters that control the local and average volumes used to calculate the local velocity density and the
    ///mean field, also the size of the leafnode in the kd-tree used when searching the tree for fof neighbours
    //@{
    ...
    /// grid type, physical, physical+entropy splitting criterion, phase+entropy splitting criterion. Note that this
    /// parameter should not be changed from the default value
    int gridtype;
    ...
    int iiterflag;
    ///\name factors used to multiply the input values to find initial candidate particles and for mergering groups in
    ///interative search
    //@{
    ...
    //@}
    //@{
    ///\name factors used to check for halo mergers, large background substructures and store the velocity scale when
    ///searching for associated baryon substructures
    //@{
    ...
    ///\name extra stuff for halo merger check and identification of multiple halo core and flag for fully adaptive
    ///linking length using number density of candidate objects
    //@{
    ...
    /// factor by which a substructure s must be closer than in phase-space to merger with another substructure in sigma
    /// units
...
// for changing effective resolution when rescaling linking lengh
        ...
        if (opt.aperturenum > 0)
        {
            ...
            datastring = string("");
            for (auto &x : opt.aperture_names_kpc)
            {
                datastring += x;
                datastring += string(",");
            }
        ...
        if (opt.apertureprojnum > 0)
        {
            ...
            datastring = string("");
            for (auto &x : opt.aperture_proj_names_kpc)
            {
                datastring += x;
                datastring += string(",");
            }
        ...
        if (opt.iprofilecalc)
        {
            ...
            datastring = string("");
            for (auto &x : opt.profile_bin_edges)
            {
                datastring += to_string(x);
                datastring += string(",");
            }
            datainfo.push_back(datastring);
        ...
        if (opt.SOnum > 0)
        {
            ...
            datastring = string("");
            for (auto &x : opt.SOthresholds_names_crit)
            {
                datastring += x;
                datastring += string(",");
            }
...
struct PropData
{
    ...
    {
        num = gNFOF = gN6DFOF = 0;
        gmass = gsize = gRmbp = gmaxvel = gRmaxvel = gRvir = gR200m = gR200c = gRhalfmass = gMassTwiceRhalfmass =
            Efrac = Pot = T = 0.;
        gMFOF = gM6DFOF = 0;
        gM500c = gR500c = 0;
        gMBN98 = gRBN98 = 0;
    ...
    PropData &operator=(const PropData &p)
    {
        ...
        gposmbp    = p.gposmbp;
        gvelmbp    = p.gvelmbp;
        gposminpot = p.gposminpot;
        gvelminpot = p.gvelminpot;
        gmass      = p.gmass;
        gsize      = p.gsize;
        gMvir      = p.gMvir;
        gRvir      = p.gRvir;
        gRmbp      = p.gRmbp;
        gmaxvel = gmaxvel = p.gmaxvel;
        gRmaxvel          = p.gRmaxvel;
        gMmaxvel          = p.gMmaxvel;
        gM200c            = p.gM200c;
        gR200c            = p.gR200c;
        ...
        gM200c_excl = p.gM200c_excl;
        gR200c_excl = p.gR200c_excl;
        gM200m_excl = p.gM200m_excl;
        gR200m_excl = p.gR200m_excl;
        gMBN98_excl = p.gMBN98_excl;
        gRBN98_excl = p.gRBN98_excl;
        gJ          = p.gJ;
        gJ200c      = p.gJ200c;
        gJ200m      = p.gJ200m;
        gJBN98      = p.gJBN98;
    ...
    void AllocateProfiles(Options &opt)
    {
        if (opt.iprofilecalc)
        {
            ...
            for (auto i = 0; i < opt.profilenbins; i++)
                profile_npart_gas_sf[i] = profile_mass_gas_sf[i] = profile_npart_gas_nsf[i] = profile_mass_gas_nsf[i] =
                    0;
#endif
#endif
#ifdef STARON
            ...
            if (opt.iInclusiveHalo > 0)
            {
...
#endif
                for (auto i = 0; i < opt.profilenbins; i++)
                    profile_npart_inclusive_gas[i] = profile_mass_inclusive_gas[i] = 0;
#ifdef STARON
                for (auto i = 0; i < opt.profilenbins; i++)
                    profile_npart_inclusive_gas_sf[i]      = profile_mass_inclusive_gas_sf[i] =
                        profile_npart_inclusive_gas_nsf[i] = profile_mass_inclusive_gas_nsf[i] = 0;
#endif
#endif
#ifdef STARON
                ...
                for (auto i = 0; i < opt.profilenbins; i++)
                    profile_npart_inclusive_star[i] = profile_mass_inclusive_star[i] = 0;
#endif
            }
        }
    }
    ...
    void WriteBinary(fstream &Fout, Options &opt)
    {
        ...
        Fout.write((char *)&val, sizeof(val));
        for (int k = 0; k < 3; k++)
            for (int n = 0; n < 3; n++) val9[k * 3 + n] = gveldisp(k, n);
        ...
        Fout.write((char *)&val, sizeof(val));
        for (int k = 0; k < 3; k++)
            for (int n = 0; n < 3; n++) val9[k * 3 + n] = geigvec(k, n);
        Fout.write((char *)val9, sizeof(val) * 9);
        ...
        Fout.write((char *)&val, sizeof(val));
        for (int k = 0; k < 3; k++)
            for (int n = 0; n < 3; n++) val9[k * 3 + n] = RV_veldisp(k, n);
        Fout.write((char *)val9, sizeof(val) * 9);
        ...
        Fout.write((char *)&val, sizeof(val));
        for (int k = 0; k < 3; k++)
            for (int n = 0; n < 3; n++) val9[k * 3 + n] = RV_eigvec(k, n);
        Fout.write((char *)val9, sizeof(val) * 9);
        ...
        for (int k = 0; k < 3; k++)
            for (int n = 0; n < 3; n++) val9[k * 3 + n] = veldisp_gas(k, n);
        Fout.write((char *)val9, sizeof(val) * 9);
        ...
        Fout.write((char *)&val, sizeof(val));
        for (int k = 0; k < 3; k++)
            for (int n = 0; n < 3; n++) val9[k * 3 + n] = eigvec_gas(k, n);
        Fout.write((char *)val9, sizeof(val) * 9);
        ...
        for (int k = 0; k < 3; k++)
            for (int n = 0; n < 3; n++) val9[k * 3 + n] = veldisp_star(k, n);
        Fout.write((char *)val9, sizeof(val) * 9);
        ...
        Fout.write((char *)&val, sizeof(val));
        for (int k = 0; k < 3; k++)
            for (int n = 0; n < 3; n++) val9[k * 3 + n] = eigvec_star(k, n);
        Fout.write((char *)val9, sizeof(val) * 9);
        ...
        if (opt.iaperturecalc && opt.aperturenum > 0)
        {
            for (auto j = 0; j < opt.aperturenum; j++) { Fout.write((char *)&aperture_npart[j], sizeof(int)); }
#ifdef GASON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout.write((char *)&aperture_npart_gas[j], sizeof(int)); }
#ifdef STARON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout.write((char *)&aperture_npart_gas_sf[j], sizeof(int)); }
            for (auto j = 0; j < opt.aperturenum; j++) { Fout.write((char *)&aperture_npart_gas_nsf[j], sizeof(int)); }
#endif
#endif
#ifdef STARON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout.write((char *)&aperture_npart_star[j], sizeof(int)); }
#endif
            for (auto j = 0; j < opt.aperturenum; j++) { Fout.write((char *)&aperture_mass[j], sizeof(val)); }
#ifdef GASON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout.write((char *)&aperture_mass_gas[j], sizeof(val)); }
#ifdef STARON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout.write((char *)&aperture_mass_gas_sf[j], sizeof(val)); }
            for (auto j = 0; j < opt.aperturenum; j++) { Fout.write((char *)&aperture_mass_gas_nsf[j], sizeof(val)); }
#endif
#endif
#ifdef STARON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout.write((char *)&aperture_mass_star[j], sizeof(val)); }
#endif
            for (auto j = 0; j < opt.aperturenum; j++) { Fout.write((char *)&aperture_rhalfmass[j], sizeof(val)); }
#ifdef GASON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout.write((char *)&aperture_rhalfmass_gas[j], sizeof(val)); }
#ifdef STARON
...
#endif
#ifdef STARON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout.write((char *)&aperture_rhalfmass_star[j], sizeof(val)); }
#endif
            for (auto j = 0; j < opt.aperturenum; j++) { Fout.write((char *)&aperture_veldisp[j], sizeof(val)); }
#ifdef GASON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout.write((char *)&aperture_veldisp_gas[j], sizeof(val)); }
#ifdef STARON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout.write((char *)&aperture_veldisp_gas_sf[j], sizeof(val)); }
            for (auto j = 0; j < opt.aperturenum; j++)
            { Fout.write((char *)&aperture_veldisp_gas_nsf[j], sizeof(val)); }
#endif
#endif
#ifdef STARON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout.write((char *)&aperture_veldisp_star[j], sizeof(val)); }
#endif
#if defined(GASON) && defined(STARON)
            for (auto j = 0; j < opt.aperturenum; j++) { Fout.write((char *)&aperture_SFR_gas[j], sizeof(val)); }
#endif
        }
        if (opt.iaperturecalc && opt.apertureprojnum > 0)
        {
        ...
        if (opt.SOnum > 0)
        {
            for (auto j = 0; j < opt.SOnum; j++) { Fout.write((char *)&SO_mass[j], sizeof(int)); }
            for (auto j = 0; j < opt.SOnum; j++) { Fout.write((char *)&SO_radius[j], sizeof(int)); }
#ifdef GASON
            if (opt.iextragasoutput && opt.iextrahalooutput)
                for (auto j = 0; j < opt.SOnum; j++) { Fout.write((char *)&SO_mass_gas[j], sizeof(int)); }
#ifdef STARON
#endif
#endif
#ifdef STARON
            if (opt.iextrastaroutput && opt.iextrahalooutput)
                for (auto j = 0; j < opt.SOnum; j++) { Fout.write((char *)&SO_mass_star[j], sizeof(int)); }
#endif
        }
    ...
    void WriteAscii(fstream &Fout, Options &opt)
    {
        ...
        Fout << gsigma_v << " ";
        for (int k = 0; k < 3; k++)
            for (int n = 0; n < 3; n++) Fout << gveldisp(k, n) << " ";
        Fout << glambda_B << " ";
        for (int k = 0; k < 3; k++) Fout << gJ[k] << " ";
        Fout << gq << " ";
        Fout << gs << " ";
        for (int k = 0; k < 3; k++)
            for (int n = 0; n < 3; n++) Fout << geigvec(k, n) << " ";
        Fout << cNFW << " ";
        Fout << Krot << " ";
        Fout << T << " ";
        Fout << Pot << " ";
        ...
        Fout << RV_sigma_v << " ";
        for (int k = 0; k < 3; k++)
            for (int n = 0; n < 3; n++) Fout << RV_veldisp(k, n) << " ";
        Fout << RV_lambda_B << " ";
        for (int k = 0; k < 3; k++) Fout << RV_J[k] << " ";
        Fout << RV_q << " ";
        Fout << RV_s << " ";
        for (int k = 0; k < 3; k++)
            for (int n = 0; n < 3; n++) Fout << RV_eigvec(k, n) << " ";
        ...
        Fout << Rhalfmass_gas << " ";
        for (int k = 0; k < 3; k++)
            for (int n = 0; n < 3; n++) Fout << veldisp_gas(k, n) << " ";
        for (int k = 0; k < 3; k++) Fout << L_gas[k] << " ";
        Fout << q_gas << " ";
        Fout << s_gas << " ";
        for (int k = 0; k < 3; k++)
            for (int n = 0; n < 3; n++) Fout << eigvec_gas(k, n) << " ";
        Fout << Krot_gas << " ";
        Fout << Temp_mean_gas << " ";
#ifdef STARON
        Fout << Z_mean_gas << " ";
        ...
        Fout << Rhalfmass_star << " ";
        for (int k = 0; k < 3; k++)
            for (int n = 0; n < 3; n++) Fout << veldisp_star(k, n) << " ";
        for (int k = 0; k < 3; k++) Fout << L_star[k] << " ";
        Fout << q_star << " ";
        Fout << s_star << " ";
        for (int k = 0; k < 3; k++)
            for (int n = 0; n < 3; n++) Fout << eigvec_star(k, n) << " ";
        Fout << Krot_star << " ";
        Fout << t_mean_star << " ";
        Fout << Z_mean_star << " ";
        if (opt.iextrastaroutput)
        ...
        if (opt.iaperturecalc && opt.aperturenum > 0)
        {
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_npart[j] << " "; }
#ifdef GASON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_npart_gas[j] << " "; }
#ifdef STARON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_npart_gas_sf[j] << " "; }
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_npart_gas_nsf[j] << " "; }
#endif
#endif
#ifdef STARON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_npart_star[j] << " "; }
#endif
#ifdef HIGHRES
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_npart_interloper[j] << " "; }
#endif
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_mass[j] << " "; }
#ifdef GASON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_mass_gas[j] << " "; }
#ifdef STARON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_mass_gas_sf[j] << " "; }
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_mass_gas_nsf[j] << " "; }
#endif
#endif
#ifdef STARON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_mass_star[j] << " "; }
#endif
#ifdef HIGHRES
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_mass_interloper[j] << " "; }
#endif
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_rhalfmass[j] << " "; }
#ifdef GASON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_rhalfmass_gas[j] << " "; }
#ifdef STARON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_rhalfmass_gas_sf[j] << " "; }
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_rhalfmass_gas_nsf[j] << " "; }
#endif
#endif
#ifdef STARON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_rhalfmass_star[j] << " "; }
#endif
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_veldisp[j] << " "; }
#ifdef GASON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_veldisp_gas[j] << " "; }
#ifdef STARON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_veldisp_gas_sf[j] << " "; }
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_veldisp_gas_nsf[j] << " "; }
#endif
#endif
#ifdef STARON
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_veldisp_star[j] << " "; }
#endif
#if defined(GASON) && defined(STARON)
            for (auto j = 0; j < opt.aperturenum; j++) { Fout << aperture_SFR_gas[j] << " "; }
#endif
        }
        if (opt.iaperturecalc && opt.apertureprojnum > 0)
        {
            for (auto k = 0; k < 3; k++)
            {
                for (auto j = 0; j < opt.apertureprojnum; j++) { Fout << aperture_mass_proj[j][k] << " "; }
#ifdef GASON
                for (auto j = 0; j < opt.apertureprojnum; j++) { Fout << aperture_mass_proj_gas[j][k] << " "; }
#ifdef STARON
                for (auto j = 0; j < opt.apertureprojnum; j++) { Fout << aperture_mass_proj_gas_sf[j][k] << " "; }
                for (auto j = 0; j < opt.apertureprojnum; j++) { Fout << aperture_mass_proj_gas_nsf[j][k] << " "; }
#endif
#endif
#ifdef STARON
                for (auto j = 0; j < opt.apertureprojnum; j++) { Fout << aperture_mass_proj_star[j][k] << " "; }
#endif
                for (auto j = 0; j < opt.apertureprojnum; j++) { Fout << aperture_rhalfmass_proj[j][k] << " "; }
#ifdef GASON
                for (auto j = 0; j < opt.apertureprojnum; j++) { Fout << aperture_rhalfmass_proj_gas[j][k] << " "; }
#ifdef STARON
                for (auto j = 0; j < opt.apertureprojnum; j++) { Fout << aperture_rhalfmass_proj_gas_sf[j][k] << " "; }
                for (auto j = 0; j < opt.apertureprojnum; j++) { Fout << aperture_rhalfmass_proj_gas_nsf[j][k] << " "; }
#endif
#endif
#ifdef STARON
                for (auto j = 0; j < opt.apertureprojnum; j++) { Fout << aperture_rhalfmass_proj_star[j][k] << " "; }
#endif
#if defined(GASON) && defined(STARON)
                for (auto j = 0; j < opt.apertureprojnum; j++) { Fout << aperture_SFR_proj_gas[j][k] << " "; }
#endif
            }
        }
        if (opt.SOnum > 0)
        {
            for (auto j = 0; j < opt.SOnum; j++) { Fout << SO_mass[j] << " "; }
            for (auto j = 0; j < opt.SOnum; j++) { Fout << SO_radius[j] << " "; }
#ifdef GASON
            if (opt.iextragasoutput && opt.iextrahalooutput)
                for (auto j = 0; j < opt.SOnum; j++) { Fout << SO_mass_gas[j] << " "; }
#ifdef STARON
#endif
#endif
#ifdef STARON
            if (opt.iextrastaroutput && opt.iextrahalooutput)
                for (auto j = 0; j < opt.SOnum; j++) { Fout << SO_mass_star[j] << " "; }
#endif
#ifdef HIGHRES
            if (opt.iextrainterloperoutput && opt.iextrahalooutput)
                for (auto j = 0; j < opt.SOnum; j++) { Fout << SO_mass_interloper[j] << " "; }
#endif
        }
        if (opt.SOnum > 0 && opt.iextrahalooutput)
        {
...
struct PropDataHeader
{
    ...
    PropDataHeader(Options &opt)
    {
        ...
        if (sizeof(Double_t) == sizeof(double))
            hdfdesiredproprealtype.push_back(H5T_NATIVE_DOUBLE);
        else
            hdfdesiredproprealtype.push_back(H5T_NATIVE_FLOAT);

#endif
#ifdef USEADIOS
        vector<ADIOS_DATATYPES> desiredadiosproprealtype;
        if (sizeof(Double_t) == sizeof(double))
            desiredadiosproprealtype.push_back(ADIOS_DATATYPES::adios_double);
        else
            desiredadiosproprealtype.push_back(ADIOS_DATATYPES::adios_real);
#endif
        ...
        if (opt.iaperturecalc > 0 && opt.aperturenum > 0)
        {
...
#ifdef STARON
            for (auto i = 0; i < opt.aperturenum; i++)
                headerdatainfo.push_back(
                    (string("Aperture_npart_gas_sf_") + opt.aperture_names_kpc[i] + string("_kpc")));
            for (auto i = 0; i < opt.aperturenum; i++)
                headerdatainfo.push_back(
                    (string("Aperture_npart_gas_nsf_") + opt.aperture_names_kpc[i] + string("_kpc")));
#endif
#endif
#ifdef STARON
            for (auto i = 0; i < opt.aperturenum; i++)
            ...
            for (auto i = 0; i < opt.aperturenum; i++)
                headerdatainfo.push_back(
                    (string("Aperture_npart_interloper_") + opt.aperture_names_kpc[i] + string("_kpc")));
#endif
#ifdef USEHDF
            sizeval = hdfpredtypeinfo.size();
...
#ifdef USEADIOS
            sizeval = adiospredtypeinfo.size();
            for (int i = sizeval; i < headerdatainfo.size(); i++)
            ...
            for (auto i = 0; i < opt.aperturenum; i++)
                headerdatainfo.push_back(
                    (string("Aperture_mass_gas_sf_") + opt.aperture_names_kpc[i] + string("_kpc")));
            for (auto i = 0; i < opt.aperturenum; i++)
                headerdatainfo.push_back(
                    (string("Aperture_mass_gas_nsf_") + opt.aperture_names_kpc[i] + string("_kpc")));
#endif
#endif
#ifdef STARON
            for (auto i = 0; i < opt.aperturenum; i++)
            ...
            for (auto i = 0; i < opt.aperturenum; i++)
                headerdatainfo.push_back(
                    (string("Aperture_mass_interloper_") + opt.aperture_names_kpc[i] + string("_kpc")));
#endif
            ...
            for (auto i = 0; i < opt.aperturenum; i++)
                headerdatainfo.push_back(
                    (string("Aperture_rhalfmass_gas_") + opt.aperture_names_kpc[i] + string("_kpc")));
#ifdef STARON
            for (auto i = 0; i < opt.aperturenum; i++)
                headerdatainfo.push_back(
                    (string("Aperture_rhalfmass_gas_sf_") + opt.aperture_names_kpc[i] + string("_kpc")));
            for (auto i = 0; i < opt.aperturenum; i++)
                headerdatainfo.push_back(
                    (string("Aperture_rhalfmass_gas_nsf_") + opt.aperture_names_kpc[i] + string("_kpc")));
#endif
#endif
#ifdef STARON
            for (auto i = 0; i < opt.aperturenum; i++)
                headerdatainfo.push_back(
                    (string("Aperture_rhalfmass_star_") + opt.aperture_names_kpc[i] + string("_kpc")));
#endif
            ...
            for (auto i = 0; i < opt.aperturenum; i++)
                headerdatainfo.push_back(
                    (string("Aperture_veldisp_gas_") + opt.aperture_names_kpc[i] + string("_kpc")));
#ifdef STARON
            for (auto i = 0; i < opt.aperturenum; i++)
                headerdatainfo.push_back(
                    (string("Aperture_veldips_gas_sf_") + opt.aperture_names_kpc[i] + string("_kpc")));
            for (auto i = 0; i < opt.aperturenum; i++)
                headerdatainfo.push_back(
                    (string("Aperture_veldisp_gas_nsf_") + opt.aperture_names_kpc[i] + string("_kpc")));
#endif
#endif
#ifdef STARON
            for (auto i = 0; i < opt.aperturenum; i++)
                headerdatainfo.push_back(
                    (string("Aperture_veldisp_star_") + opt.aperture_names_kpc[i] + string("_kpc")));
#endif
#if defined(GASON) && defined(STARON)
            for (auto i = 0; i < opt.aperturenum; i++)
        ...
        if (opt.iaperturecalc > 0 && opt.apertureprojnum > 0)
        {
            for (auto k = 0; k < 3; k++)
            {
                ...
                for (auto i = 0; i < opt.apertureprojnum; i++)
                    headerdatainfo.push_back(projname + string("mass_") + opt.aperture_proj_names_kpc[i] +
                                             string("_kpc"));
#ifdef GASON
                for (auto i = 0; i < opt.apertureprojnum; i++)
                    headerdatainfo.push_back(projname + string("mass_gas_") + opt.aperture_proj_names_kpc[i] +
                                             string("_kpc"));
#ifdef STARON
                for (auto i = 0; i < opt.apertureprojnum; i++)
                    headerdatainfo.push_back(projname + string("mass_gas_sf_") + opt.aperture_proj_names_kpc[i] +
                                             string("_kpc"));
                for (auto i = 0; i < opt.apertureprojnum; i++)
                    headerdatainfo.push_back(projname + string("mass_gas_nsf_") + opt.aperture_proj_names_kpc[i] +
                                             string("_kpc"));
#endif
#endif
#ifdef STARON
                for (auto i = 0; i < opt.apertureprojnum; i++)
                    headerdatainfo.push_back(projname + string("mass_star_") + opt.aperture_proj_names_kpc[i] +
                                             string("_kpc"));
#endif

                for (auto i = 0; i < opt.apertureprojnum; i++)
                    headerdatainfo.push_back(projname + string("rhalfmass_") + opt.aperture_proj_names_kpc[i] +
                                             string("_kpc"));
#ifdef GASON
                for (auto i = 0; i < opt.apertureprojnum; i++)
                    headerdatainfo.push_back(projname + string("rhalfmass_gas_") + opt.aperture_proj_names_kpc[i] +
                                             string("_kpc"));
#ifdef STARON
                for (auto i = 0; i < opt.apertureprojnum; i++)
                    headerdatainfo.push_back(projname + string("rhalfmass_gas_sf_") + opt.aperture_proj_names_kpc[i] +
                                             string("_kpc"));
                for (auto i = 0; i < opt.apertureprojnum; i++)
                    headerdatainfo.push_back(projname + string("rhalfmass_gas_nsf_") + opt.aperture_proj_names_kpc[i] +
                                             string("_kpc"));
#endif
#endif
#ifdef STARON
                for (auto i = 0; i < opt.apertureprojnum; i++)
                    headerdatainfo.push_back(projname + string("rhalfmass_star_") + opt.aperture_proj_names_kpc[i] +
                                             string("_kpc"));
#endif
#if defined(GASON) && defined(STARON)
                for (auto i = 0; i < opt.apertureprojnum; i++)
                    headerdatainfo.push_back(projname + string("SFR_gas_") + opt.aperture_proj_names_kpc[i] +
                                             string("_kpc"));
#endif
            }
#ifdef USEHDF
        ...
        if (opt.SOnum > 0)
        {
            ...
            if (opt.iextragasoutput && opt.iextrahalooutput)
            {
                ...
                {
                    headerdatainfo.push_back(
                        (string("SO_Mass_gas_") + opt.SOthresholds_names_crit[i] + string("_rhocrit")));
#ifdef USEHDF
                    // predtypeinfo.push_back(desiredproprealtype[0]);
                    hdfpredtypeinfo.push_back(hdfdesiredproprealtype[0]);
#endif
            ...
            if (opt.iextrastaroutput && opt.iextrahalooutput)
            {
                ...
                {
                    headerdatainfo.push_back(
                        (string("SO_Mass_star_") + opt.SOthresholds_names_crit[i] + string("_rhocrit")));
#ifdef USEHDF
                    // predtypeinfo.push_back(desiredproprealtype[0]);
                    hdfpredtypeinfo.push_back(hdfdesiredproprealtype[0]);
#endif
            ...
            if (opt.iextrainterloperoutput && opt.iextrahalooutput)
            {
                ...
                {
                    headerdatainfo.push_back(
                        (string("SO_Mass_interloper_") + opt.SOthresholds_names_crit[i] + string("_rhocrit")));
#ifdef USEHDF
                    // predtypeinfo.push_back(desiredproprealtype[0]);
                    hdfpredtypeinfo.push_back(hdfdesiredproprealtype[0]);
#endif
        ...
        if (opt.SOnum > 0 && opt.iextrahalooutput)
        {
            ...
            if (opt.iextragasoutput)
            {
                ...
                {
                    headerdatainfo.push_back(
                        (string("SO_Lx_gas_") + opt.SOthresholds_names_crit[i] + string("_rhocrit")));
                    headerdatainfo.push_back(
                        (string("SO_Ly_gas_") + opt.SOthresholds_names_crit[i] + string("_rhocrit")));
                    headerdatainfo.push_back(
                        (string("SO_Lz_gas_") + opt.SOthresholds_names_crit[i] + string("_rhocrit")));
                    for (auto k = 0; k < 3; k++)
                    {
#ifdef USEHDF
                        // predtypeinfo.push_back(desiredproprealtype[0]);
            ...
            if (opt.iextrastaroutput)
            {
                ...
                {
                    headerdatainfo.push_back(
                        (string("SO_Lx_star_") + opt.SOthresholds_names_crit[i] + string("_rhocrit")));
                    headerdatainfo.push_back(
                        (string("SO_Ly_star_") + opt.SOthresholds_names_crit[i] + string("_rhocrit")));
                    headerdatainfo.push_back(
                        (string("SO_Lz_star_") + opt.SOthresholds_names_crit[i] + string("_rhocrit")));
                    for (auto k = 0; k < 3; k++)
                    {
#ifdef USEHDF
                        // predtypeinfo.push_back(desiredproprealtype[0]);
...
/*! Structures stores profile info of the data writen by the \ref PropData profiles data structures,
    specifically the \ref PropData::WriteProfileBinary, \ref PropData::WriteProfileAscii, \ref PropData::WriteProfileHDF
   routines
*/
...
struct ProfileDataHeader
{
    ...
    ProfileDataHeader(Options &opt)
    {
        ...
        sizeval = adiospredtypeinfo.size();
        for (int i = sizeval; i < headerdatainfo.size(); i++)
            adiospredtypeinfo.push_back(ADIOS_DATATYPES::adios_unsigned_integer);
#endif
        ...
        if (opt.iInclusiveHalo > 0)
        {
            ...
            sizeval = adiospredtypeinfo.size();
            for (int i = sizeval; i < headerdatainfo.size(); i++)
                adiospredtypeinfo.push_back(ADIOS_DATATYPES::adios_real);
#endif
...
struct StrucLevelData
{
    ...
    void Initialize()
    {
        for (Int_t i = 1; i <= nsinlevel; i++)
        {
            gidhead[i]           = NULL;
            gidparenthead[i]     = NULL;
            giduberparenthead[i] = NULL;
        }
    }
    ~StrucLevelData()
    {
