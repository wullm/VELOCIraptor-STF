...
int RAMSES_fortran_read(fstream &F, RAMSESFLOAT &f)
{
    ...
    F.read((char *)&dummy, sizeof(dummy));
    byteoffset += sizeof(int);
    F.read((char *)&f, sizeof(RAMSESFLOAT));
    byteoffset += dummy;
    F.read((char *)&dummy, sizeof(dummy));
    byteoffset += sizeof(int);
    return byteoffset;
...
Int_t RAMSES_get_nbodies(char *fname, int ptype, Options &opt)
{
    ...
    if (opt.partsearchtype == PSTGAS || opt.partsearchtype == PSTALL ||
        (opt.partsearchtype == PSTDARK && opt.iBaryonSearch))
    {
    ...
    {
        nusetypes   = 4;
        usetypes[0] = RAMSESGASTYPE;
        usetypes[1] = RAMSESDMTYPE;
        usetypes[2] = RAMSESSTARTYPE;
        usetypes[3] = RAMSESBHTYPE;
    }
    else if (ptype == PSTDARK)
    {
        nusetypes   = 1;
        usetypes[0] = RAMSESDMTYPE;
    }
    ...
    {
        nusetypes   = 1;
        usetypes[0] = RAMSESSTARTYPE;
    }
    else if (ptype == PSTBH)
    {
        nusetypes   = 1;
        usetypes[0] = RAMSESBHTYPE;
    }
    ...
    // read header info
    // this is based on a ramsestotipsy fortran code that does not detail everything in the header block but at least
    // its informative. The example has
    /*
    read(10)ncpu
    read(10)ndim
    read(10)nx,ny,nz
    ...
    */
    ...
    // reopen to get number of amr cells might need to alter to read grid information and what cells have no so-called
    // son cells
    if (opt.partsearchtype == PSTGAS || opt.partsearchtype == PSTALL ||
        (opt.partsearchtype == PSTDARK && opt.iBaryonSearch))
    {
...
void ReadRamses(Options &opt, vector<Particle> &Part, const Int_t nbodies, Particle *&Pbaryons, Int_t nbaryons)
{
    ...
    if (ireadtask[ThisTask] >= 0)
    {
        ...
        header[ifirstfile].BoxSize *=
            header[ifirstfile].scale_l / 3.086e21 / header[ifirstfile].aexp * header[ifirstfile].HubbleParam / 100.0;
        getline(Finfo, stringbuf);
        ...
        // to scale mpi domains correctly need to store in opt.lengthinputconversion the box size in comoving little h
        // value opt.lengthinputconversion= opt.p*opt.h/opt.a;
        ...
        mscale = header[ifirstfile].scale_d *
                 (header[ifirstfile].scale_l * header[ifirstfile].scale_l * header[ifirstfile].scale_l) / 1.988e+33;
        ...
        cout << "Particle system contains " << nbodies << " particles (of interest) at is at time " << opt.a
        ...
        // grab from the first particle file the dimensions of the arrays and also the number of cpus (should be number
        // of files)
    ...
    if (opt.partsearchtype != PSTGAS)
    {
        ...
        if (ireadtask[ThisTask] >= 0)
        {
            inreadsend = 0;
#endif
            // read particle files consists of positions,velocities, mass, id, and level (along with ages and met if
            // some flags set)
...
*/
                            ...
                            if (opt.partsearchtype == PSTALL)
                            {
                                ...
                                Pbuf[ibufindex] = Particle(
                                    mtemp * mscale, xtemp[0] * lscale, xtemp[1] * lscale, xtemp[2] * lscale,
                                    vtemp[0] * opt.velocityinputconversion + Hubbleflow * xtemp[0],
                                    vtemp[1] * opt.velocityinputconversion + Hubbleflow * xtemp[1],
                                    vtemp[2] * opt.velocityinputconversion + Hubbleflow * xtemp[2], count2, typeval);
                            ...
                            else if (opt.partsearchtype == PSTDARK)
                            {
                                if (!(typeval == STARTYPE || typeval == BHTYPE))
                                {
                                    ...
                                    Pbuf[ibufindex] = Particle(
                                        ...
                                        vtemp[0] * opt.velocityinputconversion + Hubbleflow * xtemp[0],
                                        vtemp[1] * opt.velocityinputconversion + Hubbleflow * xtemp[1],
                                        vtemp[2] * opt.velocityinputconversion + Hubbleflow * xtemp[2], count2,
                                        DARKTYPE);
                                    Pbuf[ibufindex].SetPID(idval);
#ifdef EXTRAINPUTINFO
                                    if (opt.iextendedoutput)
                                ...
                                else if (opt.iBaryonSearch)
                                {
                                    ...
                                    if (ibuf == ThisTask)
                                    {
                                        if (k == RAMSESSTARTYPE)
                                            Nlocalbaryon[2]++;
                                        else if (k == RAMSESSINKTYPE)
                                            Nlocalbaryon[3]++;
                                    }
                                    MPIAddParticletoAppropriateBuffer(ibuf, ibufindex, ireadtask, BufSize, Nbuf, Pbuf,
                                                                      Nlocalbaryon[0], Pbaryons, Nreadbuf, Preadbuf);
#else
                                Pbaryons[bcount2] = Particle(
                                    mtemp * mscale, xtemp[0] * lscale, xtemp[1] * lscale, xtemp[2] * lscale,
                                    vtemp[0] * opt.velocityinputconversion + Hubbleflow * xtemp[0],
                                    vtemp[1] * opt.velocityinputconversion + Hubbleflow * xtemp[1],
                                    vtemp[2] * opt.velocityinputconversion + Hubbleflow * xtemp[2], count2, typeval);
                    ...
                    {
                        MPI_Allgather(Nreadbuf, opt.nsnapread, MPI_Int_t, mpi_nsend_readthread, opt.nsnapread,
                                      MPI_Int_t, mpi_comm_read);
        ...
        {
            MPIReceiveParticlesFromReadThreads(opt, Pbuf, Part.data(), readtaskID, irecv, mpi_irecvflag,
                                               Nlocalthreadbuf, mpi_request, Pbaryons);
        }
#endif
    }
        ...
        (opt.partsearchtype == PSTDARK && opt.iBaryonSearch))
    {
        ...
        if (ireadtask[ThisTask] >= 0)
        {
            ...
            for (i = 0; i < opt.num_files; i++)
                if (ireadfile[i])
                {
                    // then apparently read ngridlevels, which appears to be an array storing the number of grids at a
                    // given level
                    ...
                    if (orderingstring == string("bisection"))
                        RAMSES_fortran_skip(Famr[i], 5);
                    else
                        RAMSES_fortran_skip(Famr[i], 4);
                    ...
                    for (k = 0; k < header[i].nboundary + 1; k++)
                    {
                        for (j = 0; j < header[i].nlevelmax; j++)
                        {
                            ...
                            if (chunksize > 0)
                            {
                                ...
                                // read son index to determine if a cell in a specific grid is at the highest resolution
                                // and needs to be represented by a particle
                            ...
                            RAMSES_fortran_skip(Fhydro[i]);
                            // then read hydro for other variables (first is density, then velocity, then pressure, then
                            // metallicity )
                            if (chunksize > 0)
                            {
                                ...
                                for (idim = 0; idim < header[i].twotondim; idim++)
                                {
                                    for (ivar = 0; ivar < header[i].nvarh; ivar++)
                                    {
                                        RAMSES_fortran_read(
                                            Fhydro[i],
                                            &hydrotempchunk[idim * chunksize * header[i].nvarh + ivar * chunksize]);
                                        for (igrid = 0; igrid < chunksize; igrid++)
                                        {
                                            // once we have looped over all the hydro data then can start actually
                                            // storing it into the particle structures
                                            if (ivar == header[i].nvarh - 1)
                                            {
                                                // if cell has no internal cells or at maximum level produce a particle
                                                if (icellchunk[idim * chunksize + igrid] == 0 ||
                                                    j == header[i].nlevelmax - 1)
                                                {
                                                    // first suggestion is to add some jitter to the particle positions
                                                    double dx = pow(0.5, j);
                                                    int    ix, iy, iz;
                                                    // below assumes three dimensions with 8 corners (? maybe cells) per
                                                    // grid
                                                    iz = idim / 4;
                                                    iy = (idim - (4 * iz)) / 2;
                                                    ix = idim - (2 * iy) - (4 * iz);
                                                    ...
                                                    xpos[0] =
                                                        ((((float)rand() / (float)RAND_MAX) * header[i].BoxSize * dx) +
                                                         (header[i].BoxSize *
                                                          (xtempchunk[igrid] + (double(ix) - 0.5) * dx)) -
                                                         (header[i].BoxSize * dx / 2.0));
                                                    xpos[1] =
                                                        ((((float)rand() / (float)RAND_MAX) * header[i].BoxSize * dx) +
                                                         (header[i].BoxSize * (xtempchunk[igrid + 1 * chunksize] +
                                                                               (double(iy) - 0.5) * dx)) -
                                                         (header[i].BoxSize * dx / 2.0));
                                                    xpos[2] =
                                                        ((((float)rand() / (float)RAND_MAX) * header[i].BoxSize * dx) +
                                                         (header[i].BoxSize * (xtempchunk[igrid + 2 * chunksize] +
                                                                               (double(iz) - 0.5) * dx)) -
                                                         (header[i].BoxSize * dx / 2.0));
#ifdef USEMPI
                                                    // determine processor this particle belongs on based on its spatial
                                                    // position
                                                    ibuf      = MPIGetParticlesProcessor(xpos[0], xpos[1], xpos[2]);
                                                    ibufindex = ibuf * BufSize + Nbuf[ibuf];
#endif

                                                    vpos[0] = hydrotempchunk[idim * chunksize * header[i].nvarh +
                                                                             1 * chunksize + igrid];
                                                    vpos[1] = hydrotempchunk[idim * chunksize * header[i].nvarh +
                                                                             2 * chunksize + igrid];
                                                    vpos[2] = hydrotempchunk[idim * chunksize * header[i].nvarh +
                                                                             3 * chunksize + igrid];
                                                    mtemp   = dx * dx * dx *
                                                            hydrotempchunk[idim * chunksize * header[i].nvarh +
                                                                           0 * chunksize + igrid];
                                                    // the self energy P/rho is given by
                                                    utemp = hydrotempchunk[idim * chunksize * header[i].nvarh +
                                                                           4 * chunksize + igrid] /
                                                            hydrotempchunk[idim * chunksize * header[i].nvarh +
                                                                           0 * chunksize + igrid] /
                                                            (header[i].gamma_index - 1.0);
                                                    rhotemp = hydrotempchunk[idim * chunksize * header[i].nvarh +
                                                                             0 * chunksize + igrid] *
                                                              rhoscale;
                                                    Ztemp = hydrotempchunk[idim * chunksize * header[i].nvarh +
                                                                           5 * chunksize + igrid];
                                                    if (opt.partsearchtype == PSTALL)
                                                    {
#ifdef USEMPI
                                                        Pbuf[ibufindex] =
                                                            Particle(mtemp * mscale, xpos[0] * lscale, xpos[1] * lscale,
                                                                     xpos[2] * lscale,
                                                                     vpos[0] * opt.velocityinputconversion +
                                                                         Hubbleflow * xpos[0],
                                                                     vpos[1] * opt.velocityinputconversion +
                                                                         Hubbleflow * xpos[1],
                                                                     vpos[2] * opt.velocityinputconversion +
                                                                         Hubbleflow * xpos[2],
                                                                     count2, GASTYPE);
                                                        Pbuf[ibufindex].SetPID(idval);
#ifdef GASON
                                                        ...
                                                        {
                                                            Pbuf[ibufindex].SetInputFileID(i);
                                                            Pbuf[ibufindex].SetInputIndexInFile(
                                                                idim * chunksize * header[i].nvarh + 0 * chunksize +
                                                                igrid + ninputoffset);
                                                        }
#endif
                                                        // ensure that store number of particles to be sent to the
                                                        // threads involved with reading snapshot files
                                                        Nbuf[ibuf]++;
                                                        MPIAddParticletoAppropriateBuffer(
                                                            ibuf, ibufindex, ireadtask, BufSize, Nbuf, Pbuf, Nlocal,
                                                            Part.data(), Nreadbuf, Preadbuf);
#else
                                                    Part[count2] = Particle(
                                                        mtemp * mscale, xpos[0] * lscale, xpos[1] * lscale,
                                                        xpos[2] * lscale,
                                                    ...
                                                    {
                                                        Part[count2].SetInputFileID(i);
                                                        Part[count2].SetInputIndexInFile(
                                                            idim * chunksize * header[i].nvarh + 0 * chunksize + igrid +
                                                            ninputoffset);
                                                    }
#endif
                                                    ...
                                                    else if (opt.partsearchtype == PSTDARK && opt.iBaryonSearch)
                                                    {
                                                        ...
                                                        MPIAddParticletoAppropriateBuffer(
                                                            ibuf, ibufindex, ireadtask, BufSize, Nbuf, Pbuf,
                                                            Nlocalbaryon[0], Pbaryons, Nreadbuf, Preadbuf);
#else
                                                    Pbaryons[bcount2] = Particle(
                                                        mtemp * mscale, xpos[0] * lscale, xpos[1] * lscale,
                                                    ...
                                                    if (opt.iextendedoutput)
                                                    {
                                                        ...
                                                        Pbaryons[bcount2].SetInputIndexInFile(
                                                            idim * chunksize * header[i].nvarh + 0 * chunksize + igrid +
                                                            ninputoffset);
                                                    }
#endif
